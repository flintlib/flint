...
void
nmod_sparse_mat_sub(nmod_sparse_mat_t C, const nmod_sparse_mat_t A, const nmod_sparse_mat_t B)
{
    slong i;
    ...
    for (i = 0; i < C->r; i++)
    {
        ...
        while(j < A->row_nnz[i] && k < B->row_nnz[i]) {
            ...
            if(Be[k].col == col) Ce[nnz].val = nmod_sub(Ce[nnz].val, Be[k++].val, C->mod);
            if(Ce[nnz].val != UWORD(0)) ++nnz;
        }
        /* Add remainder of A row */
        for(; j<A->row_nnz[i]; ++j, ++nnz) 
            Ce[nnz] = Ae[j];
        /* Subtract remainder of B row */
        for(; k<B->row_nnz[i]; ++k, ++nnz) 
            Ce[nnz] = Be[k], Ce[nnz].val = nmod_neg(Ce[nnz].val, C->mod);
