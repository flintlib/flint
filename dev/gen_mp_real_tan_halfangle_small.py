#!/usr/bin/env python3
"""Body emitter for dev/tune_mp_real.py (not run directly).
...
Only r = _mp_real_tan_opt_r[n] is supported (the r = 0 dispatch): the
...
def emit_mulhi(o, res, a, b, n, indent):
    pre = " " * indent
    if n == 2:
        o(pre + "_mp_real_mulhi_2x2_sloppy(%s + 1, %s, %s[1], %s[0], %s[1], %s[0]);"
          % (res, res, a, a, b, b))
    else:
...
def emit_sqrhi(o, res, a, n, indent):
    pre = " " * indent
    if n == 2:
        o(pre + "_mp_real_sqrhi_2x2(%s + 1, %s, %s[1], %s[0]);" % (res, res, a, a))
    else:
...
def emit(o, n, r_const=None, fn_name=None, series_name=None,
    ...
    wn = n + 1
    if fn_name is None:
        fn_name = "_mp_real_tan_halfangle_%d" % n
    if series_name is None:
        series_name = "_mp_real_tan_rs_opt_%d" % n
    ...
    o("    slong num, nc;")
    if r_const is None:
        o("    int r = _mp_real_tan_opt_r[%d], c;" % n)
    else:
    ...
    o("")
    o("    nn_srcptr tab;")
    o("    tab = _mp_real_atans_tab(%d, r, &nc);" % n)
    o("")
    o("    TMP_START;")
    o("    used = TMP_ALLOC(MP_REAL_BITWISE_REDUCE_USED_ALLOC(r)"
      " * sizeof(slong));")
    o("")
    for k in range(n - 1):
        o("    t[%d] = (x[%d] >> 1) | (x[%d] << (FLINT_BITS - 1));" % (k, k, k + 1))
    o("    t[%d] = x[%d] >> 1;" % (n - 1, n - 1))
    o("    num = _mp_real_bitwise_reduce(t, %d, r, 1, tab, nc, used);" % n)
    o("    _mp_real_tan_rotate_%d(wx, wy, used, num);" % n)
    ...
    o("            c = 2;")
    o("")
    o("        flint_mpn_tdiv_qr(R, rem, N, %d, S, %d);" % (2 * n, n))
    o("")
    o("        if (ysin != NULL)")
    o("        {")
