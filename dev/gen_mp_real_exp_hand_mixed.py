...
(n, r), used by dev/tune_mp_real.py for the per-size kernels.
...
def emit(o, n, r, name):
    ...
    for k in range(N - 2, 1, -1):
        ...
        if cur == 1:
            ...
            o("    _mp_real_mulhi_2x1(&h, &l, X2, X1, v);")
            o("    add_ssaaaa(w1, w0, UWORD(0x%016x), UWORD(0x%016x), h, l);"
              % (c[1], c[0]))
        elif cur == 2 and prev == 2:
            o("    _mp_real_mulhi_2x2_sloppy(&h, &l, X2, X1, w1, w0);")
            o("    add_ssaaaa(w1, w0, UWORD(0x%016x), UWORD(0x%016x), h, l);"
              % (c[1], c[0]))
        elif cur == 3 and prev == 2:
            o("    _mp_real_mulhi_3x2(&A[2], &A[1], &A[0], x[%d], x[%d], x[%d],"
              % (n - 1, n - 2, n - 3))
            o("        w1, w0);")
            for i in range(3):
