#!/usr/bin/env python3
"""Body emitter for dev/tune_mp_real.py (not run directly): per-size
...
# dev/gen_mp_real_exp_rs_hard.py (WINNERS_OPT), whose term count spends
...
def gen(n, opt=False, fn_name=None, series_name=None, r_const=None,
    ...
    o = L.append
    if fn_name is None:
        fn_name = ("_mp_real_exp_bitwise_rs_opt_%d" % n) if opt \
            else ("_mp_real_exp_bitwise_rs_%d" % n)
    if series_name is None and opt:
        series_name = "_mp_real_exp_rs_opt_%d" % n
    ...
    o("")
    o("    nn_srcptr tab;")
    o("    tab = _mp_real_exp_logs_tab(%d, r, &nc);" % wn)
    o("")
    for k in range(wn):
        o("    t%d = x[%d];" % (k, k))
    o("")
    ...
    o("    for (; bj < num; bj++) \\")
    o("    { \\")
    o("        nn_srcptr Lq = tab + used[bj] * nc + (nc - %d); \\"
      % wn)
    if wn == 1:
        o("        t0 -= Lq[0]; \\")
    else:
    ...
    if n <= 2:
        ...
        o("   band (see _mp_real_exp_reduce) */")
    ...
    if opt:
        ...
        o("    _mp_real_exp_rs(ys, NULL, ts, %d);" % wn)
    for k in range(wn + 1):
        o("    y%d = ys[%d];" % (k, k))
    o("")
    o("    j = 0;")
    ...
    o("    if (j < num)")
    o("        _mp_real_exp_recon(ys, sh, %d, used, j, num);" % (wn + 1))
    o("")
    for k in range(wn + 1):
        o("    res[%d] = ys[%d];" % (k, k))
    o("}")
...
out = ["/* Generated by dev/gen_mp_real_exp_bitwise_small.py -- do not edit.",
       "",
       "   Register implementations of _mp_real_exp_bitwise_rs for",
...
static int
_mp_real_exp_bitwise_rs_opt(nn_ptr res, nn_srcptr x, slong n)
{
    switch (n)
    {
        case 1: _mp_real_exp_bitwise_rs_opt_1(res, x); return 1;
        case 2: _mp_real_exp_bitwise_rs_opt_2(res, x); return 1;
        case 3: _mp_real_exp_bitwise_rs_opt_3(res, x); return 1;
        case 4: _mp_real_exp_bitwise_rs_opt_4(res, x); return 1;
        case 5: _mp_real_exp_bitwise_rs_opt_5(res, x); return 1;
        default: return 0;
    }
}

static void
_mp_real_exp_bitwise_rs_small(nn_ptr res, nn_srcptr x, slong n, int r)
{
    switch (n)
    {
        case 1: _mp_real_exp_bitwise_rs_1(res, x, r); break;
        case 2: _mp_real_exp_bitwise_rs_2(res, x, r); break;
        case 3: _mp_real_exp_bitwise_rs_3(res, x, r); break;
        case 4: _mp_real_exp_bitwise_rs_4(res, x, r); break;
        case 5: _mp_real_exp_bitwise_rs_5(res, x, r); break;
        case 6: _mp_real_exp_bitwise_rs_6(res, x, r); break;
        default: _mp_real_exp_bitwise_rs_7(res, x, r); break;
    }
}
""")
print("\n".join(out).rstrip("\n"))
...
if __name__ == "__main__":
    ...
    sys.exit("this module is a body-emitter library for "
             "dev/tune_mp_real.py; run that instead")
