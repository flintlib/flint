...
the bitwise log, atan and sin/cos, as dev/gen_mp_real_exp_hand_mixed.py does for exp.
...
def emit_sqrhigh(o, dst, src, L):
    ...
    if L == 1:
        ...
        o("    _mp_real_sqrhi_2x2(&%s[1], &%s[0], %s[1], %s[0]);"
          % (dst, dst, src, src))
    else:
...
def emit_mulhigh(o, dst, a, b, L):
    ...
    if L == 1:
        ...
        o("    _mp_real_mulhi_2x2_sloppy(&%s[1], &%s[0], %s[1], %s[0], %s[1], %s[0]);"
          % (dst, dst, a, a, b, b))
    else:
...
def emit(o, func, n, r, name):
    ...
    for k in range(N - 2, -1, -1):
        ...
        if cur == 1:
            ...
            o("    _mp_real_mulhi_2x1(&h, &l, z[%d], z[%d], v);" % (n - 1, n - 2))
            if sub:
            ...
            o("    _mp_real_mulhi_2x2_sloppy(&h, &l, z[%d], z[%d], w1, w0);"
              % (n - 1, n - 2))
            if sub:
                o("    sub_ddmmss(w1, w0, UWORD(0x%016x), UWORD(0x%016x), h, l);"
                  % (c[1], c[0]))
...
def emit_chain(o, func, n, r, zname):
    ...
    for k in range(N - 2, -1, -1):
        ...
        if cur == 1:
            ...
            o("    _mp_real_mulhi_2x1(&h, &l, %s[%d], %s[%d], %sv);"
              % (zname, n - 1, zname, n - 2, pre))
            o("    sub_ddmmss(%sw1, %sw0, UWORD(0x%016x), UWORD(0x%016x), h, l);"
              % (pre, pre, c[1], c[0]))
        elif cur == 2:
            o("    _mp_real_mulhi_2x2_sloppy(&h, &l, %s[%d], %s[%d], %sw1, %sw0);"
