...
register implementations of the _mp_real_atan_bitwise_rs vectoring for n = 1..7,
mirroring the log1p small path (dev/gen_mp_real_log1p_bitwise_small.py).
...
# src/mp_real/trig_rs_opt_hand.inc).  The hand series is so much cheaper
...
def gen_one(o, n, opt=False, fn_name=None, series_name=None,
    ...
    if fn_name is None:
        fn_name = ("_mp_real_atan_bitwise_rs_opt_%d" % n) if opt \
            else ("_mp_real_atan_bitwise_rs_%d" % n)
    if series_name is None and opt:
        series_name = "_mp_real_atan_rs_opt_%d" % n
    ...
    for j in range(n):
        o("    a%d = 0;" % j)
    o("")
    o("#define AP(ii) (tab + (ii) * nc + (nc - %d))" % n)
    o("")
    ...
    for j in range(n):
        o("        nd[%d] = y%d;" % (n + j, j))
    o("        flint_mpn_tdiv_qr(t, nd, nd, %d, S, %d);" % (2 * n, n + 1))
    o("    }")
    o("    else")
    o("    {")
    for j in range(n):
    ...
    if opt:
        ...
        o("    _mp_real_atan_rs(q, NULL, t, %d);" % n)
    o("")
    # res = acc + atan(t)
