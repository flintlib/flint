...
register implementations of the _mp_real_log1p_bitwise_rs reduction for
n = 3..7, mirroring the exp small path (dev/
gen_mp_real_exp_bitwise_small.py).
...
def gen_one(o, n, opt=False, fn_name=None, series_name=None,
    ...
    if fn_name is None:
        fn_name = ("_mp_real_log1p_bitwise_rs_opt_%d" % n) if opt \
            else ("_mp_real_log1p_bitwise_rs_%d" % n)
    if series_name is None and opt:
        series_name = "_mp_real_atanh_rs_opt_%d" % n
    ...
    o("")
    o("    nn_srcptr tab;")
    o("    tab = _mp_real_exp_logs_tab(%d, r, &nc);" % n)
    o("")
    for j in range(n):
        o("    d%d = x[%d];" % (j, j))
    for j in range(n):
    ...
    for j in range(n):
        o("    a%d = 0;" % j)
    o("")
    o("#define LP(ii) (tab + (ii) * nc + (nc - %d))" % n)
    o("")
    ...
    if opt:
        ...
        o("    _mp_real_atanh_rs(y, NULL, t, %d);" % n)
    o("")
    # res = acc + 2 atanh(t)
    o("    %s(%s," % (ADD[n],
