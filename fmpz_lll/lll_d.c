...
fmpz_lll_d(fmpz_mat_t B, double delta, double eta)
{
    ...
    do
        d_mat_entry(appSP, i, i) = _d_vec_norm(appB->rows[i], n);
    while ((d_mat_entry(appSP, i, i) <= 0.0) && (++i < d)); /* Check if this should be D_EPS and not 0.0 */
    ...
    if (zeros < d - 1)
        d_mat_entry(r, i, i) = d_mat_entry(appSP, i, i);
    ...
    while (kappa < d)
    {
        ...
        if (babai_ok == -1)
        {
            num_failed_fast++;
            heuristic_fail =
                fmpz_lll_check_babai_heuristic_d(kappa, B, mu, r, s, appB,
        ...
        if (tmp <= s[kappa - 1])
        {
            alpha[kappa] = kappa;
            tmp =
                d_mat_entry(mu, kappa, kappa - 1) * d_mat_entry(r, kappa,
                                                                kappa - 1);
        ...
        else
        {
            ...
            for (i = 0; i <= kappa2; i++)
                appSPtmp[i] = d_mat_entry(appSP, kappa2, i);

            for (i = kappa2 + 1; i <= kappamax; i++)
                appSPtmp[i] = d_mat_entry(appSP, i, kappa2);

            for (i = kappa2; i > kappa; i--)
            {
                for (j = 0; j < kappa; j++)
                    d_mat_entry(appSP, i, j) = d_mat_entry(appSP, i - 1, j);
                d_mat_entry(appSP, i, kappa) = appSPtmp[i - 1];

                for (j = kappa + 1; j <= i; j++)
                    d_mat_entry(appSP, i, j) =
                        d_mat_entry(appSP, i - 1, j - 1);

                for (j = kappa2 + 1; j <= kappamax; j++)
                    d_mat_entry(appSP, j, i) = d_mat_entry(appSP, j, i - 1);
            }

            for (i = 0; i < kappa; i++)
                d_mat_entry(appSP, kappa, i) = appSPtmp[i];
            d_mat_entry(appSP, kappa, kappa) = appSPtmp[kappa2];

            for (i = kappa2 + 1; i <= kappamax; i++)
                d_mat_entry(appSP, i, kappa) = appSPtmp[i];

            if (d_mat_entry(r, kappa, kappa) <= 0.0)
            {
                zeros++;
                kappa++;
                d_mat_entry(appSP, kappa, kappa) =
