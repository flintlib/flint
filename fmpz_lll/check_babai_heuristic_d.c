...
int
fmpz_lll_check_babai_heuristic_d(int kappa, fmpz_mat_t B, d_mat_t mu,
                                 d_mat_t r, double *s, d_mat_t appB, int *expo,
                                 d_mat_t appSP, int a, int zeros, int kappamax,
                                 int n, double delta, double eta)
{
        ...
        for (j = aa; j < kappa; j++)
        {
            if (d_mat_entry(appSP, kappa, j) != d_mat_entry(appSP, kappa, j))
            {
                ...
                d_mat_entry(appSP, kappa, j) =
                    fmpz_lll_heuristic_dot(appB->rows[kappa], appB->rows[j], n,
                                           B, kappa, j, expo[kappa] + expo[j]);
            }
    ...
    if (d_mat_entry(appSP, kappa, kappa) != d_mat_entry(appSP, kappa, kappa))
    {
        ...
        d_mat_entry(appSP, kappa, kappa) =
            fmpz_lll_heuristic_dot(appB->rows[kappa], appB->rows[kappa], n, B,
                                   kappa, kappa, 2 * expo[kappa]);
    }
