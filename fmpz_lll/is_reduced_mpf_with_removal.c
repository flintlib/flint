...
int
fmpz_lll_is_reduced_mpf_with_removal(const fmpz_mat_t B, const fmpz_lll_t fl,
                                     ...
                                     mp_bitcnt_t prec)
{
    if (fl->rt == Z_BASIS)
    {
        ...
        mpf_init2(tmp, prec);
        mpf_init2(rtmp, prec);
        mpf_init2(mpf_gs_B, prec);

        fmpz_mat_get_mpf_mat(appB, B);

        fmpz_get_mpf(mpf_gs_B, gs_B);
        ...
        if (newd == 0 && mpf_cmp(mpf_mat_entry(mu, 0, 0), mpf_gs_B) < 0)
        {
            mpf_mat_clear(appB);
            mpf_mat_clear(Q);
            mpf_mat_clear(mu);
            mpf_clears(tmp, rtmp, mpf_gs_B, '\0');
            return 0;
        }
        ...
        for (i = 1; i < d; i++)
        {
            ...
            if (i >= newd && mpf_cmp(mpf_mat_entry(mu, i, i), mpf_gs_B) < 0)    /* check removals */
            {
                mpf_mat_clear(appB);
                mpf_mat_clear(Q);
                mpf_mat_clear(mu);
    ...
    else
    {
        ...
        for (i = 1; i < d; i++)
        {
            ...
            if (i >= newd && mpf_cmp(mpf_mat_entry(r, i, i), mpf_gs_B) < 0) /* check removals */
            {
                mpf_mat_clear(r);
                mpf_mat_clear(mu);
                _mpf_vec_clear(s, d);
