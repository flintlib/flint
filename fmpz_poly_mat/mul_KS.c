    ...
    const fmpz_poly_mat_t B)
{
    ...
    {
        fmpz_poly_mat_t T;
        fmpz_poly_mat_init(T, A->r, B->c);
        fmpz_poly_mat_mul_KS(T, A, B);
    ...
    for (i = 0; i < A->r; i++)
        for (j = 0; j < A->c; j++)
            fmpz_poly_bit_pack(fmpz_mat_entry(AA, i, j), fmpz_poly_mat_entry(A, i, j), bit_size);

    for (i = 0; i < B->r; i++)
        for (j = 0; j < B->c; j++)
            fmpz_poly_bit_pack(fmpz_mat_entry(BB, i, j), fmpz_poly_mat_entry(B, i, j), bit_size);
    ...
    for (i = 0; i < C->r; i++)
        for (j = 0; j < C->c; j++)
            if (signs)
                fmpz_poly_bit_unpack_unsigned(fmpz_poly_mat_entry(C, i, j), fmpz_mat_entry(CC, i, j), bit_size);
            else
                fmpz_poly_bit_unpack(fmpz_poly_mat_entry(C, i, j), fmpz_mat_entry(CC, i, j), bit_size);
