...
#define FLINT_COMPINV_NEWTON_CUTOFF 10
...
_fmpz_poly_compinv_series_newton(fmpz * Qinv, const fmpz * Q, long n)
{
    ...
    else
    {
        ...
        Qprime = _fmpz_vec_init(n);
        T = _fmpz_vec_init(n);
        U = _fmpz_vec_init(n);
        V = _fmpz_vec_init(n);

        _fmpz_poly_derivative(T, Q, n);
        _fmpz_poly_inv_series(Qprime, T, n - 1);
        fmpz_zero(Qprime + n - 1);
        ...
        _fmpz_poly_compinv_series_lagrange(Qinv, Q, k);
        ...
        for (i--; i >= 0; i--)
        {
            ...
            _fmpz_poly_compose_series(U, Qprime, k, Qinv, k, k);
            fmpz_zero(T + 1);
            _fmpz_poly_mullow(V, T, k, U, k, k);
            _fmpz_vec_sub(Qinv, Qinv, V, k);
        }

        free(a);
        _fmpz_vec_clear(Qprime, n);
        _fmpz_vec_clear(T, n);
        _fmpz_vec_clear(U, n);
        _fmpz_vec_clear(V, n);
    }
...
fmpz_poly_compinv_series_newton(fmpz_poly_t Qinv, const fmpz_poly_t Q, long n)
{
    ...
    {
        printf("exception: fmpz_poly_compinv_series_newton: input must have "
    ...
    if (Qinv != Q)
    {
        ...
        _fmpz_poly_compinv_series_newton(Qinv->coeffs, Qcopy, n);
    }
    else
    {
        fmpz_poly_t t;
        fmpz_poly_init2(t, n);
        _fmpz_poly_compinv_series_newton(t->coeffs, Qcopy, n);
