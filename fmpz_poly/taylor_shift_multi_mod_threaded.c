...
void
_fmpz_vec_multi_mod_ui_threaded(ulong_ptr * residues, fmpz * vec, slong len,
    ulong_srcptr primes, slong num_primes, int crt)
{
    mod_ui_arg_t * args;
    slong i, num_threads;
    ...
    for (i = 0; i < num_threads + 1; i++)
    {
        ...
        args[i].primes = (ulong_ptr) primes;
...
void
_fmpz_poly_multi_taylor_shift_threaded(ulong_ptr * residues, slong len,
        const fmpz_t c, ulong_srcptr primes, slong num_primes)
{
...
_fmpz_poly_taylor_shift_multi_mod(fmpz * poly, const fmpz_t c, slong len)
{
    ...
    residues = flint_malloc(sizeof(ulong_ptr) * num_primes);
