...
/*
    Benchmarks for the q-adic multiplication routine.

    We consider the set-up with p = 17, N = 2^i, i = 0, ..., 19, 
    and compute A*B mod p^N, where 
...
main(void)
{
    ...
    printf("Benchmark for q-adic multiplication.\n");
    fflush(stdout);
...
for (l = 0; l < len; l++)
{
    ...
    for (i = 0; i < d; i++)
    {
        ...
        fmpz_set_ui(f, 5 + 2*i);
        fmpz_pow_ui(b->coeffs + i, f, 3 * n);
        fmpz_mod(b->coeffs + i, b->coeffs + i, pow);
