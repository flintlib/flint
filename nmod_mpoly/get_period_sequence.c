...
void get_period_sequence(fmpz_t *zeroCoefficients, long *coefficients, ulong *exponents, ulong length, ulong monomial, int pow, ulong *primes, int numOfPrimes){  
   ...
   for(i = 0; i<numOfPrimes; i++){
      ...
      nmod_mpoly_init2(poly, primes[i], length, (long) 3, (ulong) 21);
      nmod_mpoly_init2(polyTotal, primes[i], length, (long) 3, (ulong) 21);
      nmod_mpoly_init2(tempPoly, primes[i], length, (long) 3, (ulong) 21);
