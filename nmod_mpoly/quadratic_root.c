    ...
    const ulong * cmpmask)
{
    ...
    while (heap_len > 1)
    {
        ...
        */
        ...
        if (bits <= FLINT_BITS)
        {
            if (!mpoly_monomial_halves(Qexps + N*Qlen, exp, N, mask))
            {
                mcmp = -1;
                goto try_less;
            }
        ...
        {
            if (!mpoly_monomial_halves_mp(Qexps + N*Qlen, exp, N, bits))
            {
                mcmp = -1;
                goto try_less;
            }
        ...
        have_root = 0; /* z^2+z+1=0 insoluble over F2 */
    ...
    try_less:
        ...
        if (bits <= FLINT_BITS)
        {
            if (!mpoly_monomial_divides(Qexps + Qlen*N, exp, Aexps + N*0, N, mask))
                goto no_solution;
        }
        else
        {
    ...
    const nmod_mpoly_ctx_t ctx)
{
    ...
    if (ctx->mod.n != 2)
    {
        ...
        nmod_mpoly_scalar_addmul_ui(t2, t1, B, nmod_neg(c, ctx->mod), ctx);
        success = nmod_mpoly_sqrt(t1, t2, ctx);
        if (success)
        {
            nmod_mpoly_sub(t2, t1, A, ctx);
            nmod_mpoly_scalar_mul_ui(Q, t2, nmod_inv(2, ctx->mod), ctx);
        }
