...
{ 
   ...
   ulong j, bits1 = 0, bits2 = 0, b, b_old;

   nmod_mpoly_heap_t * heap = 
      (nmod_mpoly_heap_t *) malloc((len2 + 1)*sizeof(nmod_mpoly_heap_t));
   nmod_mpoly_entry_t * entries = 
      (nmod_mpoly_entry_t *) malloc(len2*sizeof(nmod_mpoly_entry_t));
   ...
   /* 
      Insert only the first of these entries into the heap - 
      we inject the others later 
   ...
   */
   while (n)
   {
      ...
      /* chain starting from top of heap */
      nmod_mpoly_entry_t * chain = heap[1].entry; 
	  ...
	  {                   
        prev = next;
		  ...
		  } else if (b == 2) /* input coeffs fit in a small and are unsigned */
		  {
           ...
           /* n2 + cy cannot overflow */
		     add_ssaaaa(cy, accum1, (mp_limb_t) 0, accum1, (mp_limb_t) 0, n2 + cy); 
		 ...
		 {			
           ...
           add_ssaaaa(cy, accum0, (mp_limb_t) 0, accum0, (mp_limb_t) 0, n1);
           /* n2 + cy cannot overflow */
		     add_ssaaaa(accum2, accum1, accum2, accum1, (mp_limb_t) 0, n2 + cy); 
       }
             
	    /* inject next entry poly1[0]*poly2[j+1] */
       if (next->i1 == 0 && next->i2 < len2 - 1) 
	    nmod_mpoly_heap_insert(heap, &n, next + 1, 
                  poly1->exps[0] + poly2->exps[next->i2 + 1]);
