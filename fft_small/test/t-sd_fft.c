...
void test_v2_fft(sd_fft_ctx_t Q, ulong minL, ulong maxL, ulong ireps, flint_rand_t state)
{
    ...
    for (ulong L = minL; L <= maxL; L++)
    {
        ...
        sd_fft_ctx_set_data(Q, (double*) flint_aligned_alloc(32,
                                     sd_fft_ctx_data_size(Q)*sizeof(double)));
        ...
        for (ulong rep = 0; rep < nreps; rep++)
        {
            ...
            for (i = 0; i < itrunc; i++)
                sd_fft_ctx_set_index(Q, i, X[i]);

            sd_fft_ctx_fft_trunc(Q, itrunc, otrunc);
            ...
            for (int check_reps = 0; check_reps < 3; check_reps++)
            {
                ...
                if (!vec1d_same_mod(y, sd_fft_ctx_get_fft_index(Q, i), Q->p, Q->pinv))
                {
            ...
            sd_fft_ctx_fft_trunc(Q, trunc, trunc);
            sd_fft_ctx_ifft_trunc(Q, trunc);
