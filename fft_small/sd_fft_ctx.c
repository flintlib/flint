...
void sd_fft_ctx_init_prime(sd_fft_ctx_t Q, ulong pp)
{
    ...
    for (ulong j = 0; j < N; j++)
    {
        ...
        Q->w2s[jr] = vec1d_reduce_0n_to_pmhn((slong)wi, Q->p, Q->p*0.5);
...
/*
    if (Q->w2s == NULL)
        sd_fft_ctx_init_prime(Q, Q->mod.n);
*/
    FLINT_ASSERT(Q->w2s != NULL);
    ...
    memcpy(Q->w2s, oldw2s, n_pow2(Q->wtab_depth-1)*sizeof(double));
    ...
    while (Q->wtab_depth < k)
    {
        ...
        vec8d w = vec8d_set_d(vec1d_reduce_0n_to_pmhn(ww, Q->p, 0.5*Q->p));
        vec8d n    = vec8d_set_d(Q->p);
        vec8d hn   = vec8d_set_d(0.5*Q->p);
        vec8d ninv = vec8d_set_d(Q->pinv);
        double* wptr = Q->w2s;
        ulong j = 0; do {
            vec8d x = vec8d_load_aligned(wptr + j);
            x = vec8d_mulmod2(x, w, n, ninv);
            x = vec8d_reduce_pm1n_to_pmhn(x, n, hn);
            vec8d_store_aligned(wptr + N/2 + j, x);
        } while (j += 8, j < N/2);
        FLINT_ASSERT(j == N/2);
        Q->wtab_depth++;
    }
