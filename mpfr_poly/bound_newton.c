...
/*
   ...
   If we plot these values on a graph, we are looking for a set of two 
   parallel lines which bound all the points between them, L1 bounding them 
   above, L2 bounding them below. If the two lines are vertically separated 
   by <= prec then the function returns 1 and the slope and y-intercept of L1. 
   If not, the function returns 0.

   Such lines are found as follows. Firstly, parallel horizontal lines L1 and 
   L2 are drawn which bound all the points. Both lines are then rotated 
   (remaining) parallel, in the direction which allows them to come closer, 
   until they can be rotated no more without one of the points crossing the 
   line.
...
int
_mpfr_poly_bound_newton(double *inter, double *slope,
                        mpfr * poly, long len, mpfr_prec_t prec)
{
    ...
    return (min_d - (double) min_i * slope2 <= *inter - prec) ? 0 : 1;
}
