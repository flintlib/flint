...
void
_mpfr_poly_mul_classical(mpfr * res, mpfr * in1, long len1, mpfr * in2,
                         long len2, mpfr_prec_t prec)
{
    mpfr *temp;
    long i;
...
void
mpfr_poly_mul_classical(mpfr_poly_t res, mpfr_poly_t poly1, mpfr_poly_t poly2)
{
    long len_out;
    ...
    if (res == poly1 || res == poly2)
    {
        ...
        if (poly1->length >= poly2->length)
            _mpfr_poly_mul_classical(temp->coeffs, poly1->coeffs,
                                     poly1->length, poly2->coeffs,
                                     poly2->length, prec);
        else
            _mpfr_poly_mul_classical(temp->coeffs, poly2->coeffs,
    ...
    else
    {
        ...
        if (poly1->length >= poly2->length)
            _mpfr_poly_mul_classical(res->coeffs, poly1->coeffs, poly1->length,
                                     poly2->coeffs, poly2->length, prec);
        else
            _mpfr_poly_mul_classical(res->coeffs, poly2->coeffs, poly2->length,
                                     poly1->coeffs, poly1->length, prec);
    }
