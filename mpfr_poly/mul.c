...
*/
void
_mpfr_poly_filter(mpfr * poly, long len, double s, mpfr_prec_t prec,
                  mpfr_prec_t filter_bits)
{
    long i;
...
*/
void
_mpfr_poly_mul_inplace(mpfr * poly1, long len1,
                       mpfr * poly2, long len2, mpfr_prec_t prec, double s,
                       mpfr_prec_t fb)
{
...
*/
void
_mpfr_poly_shift_scale(mpfr * res, mpfr * poly, long len,
...
void
_mpfr_poly_mul_scale(mpfr * res, mpfr * poly1, long len1, mpfr * poly2,
                     long len2, double s1, double i1, double i2, long w,
                     mpfr_prec_t prec, mpfr_prec_t fb)
{
    mpfr *t1, *t2;
    ...
    for (i = w2; i < len2; i += w2)
    {
        ...
        _mpfr_vec_add(res + len1 - w1 + i, res + len1 - w1 + i, t1,
                      w1 + l2 - 1);
    }
...
void
mpfr_poly_mul(mpfr_poly_t res, mpfr_poly_t poly1, mpfr_poly_t poly2,
              mpfr_prec_t fb)
{
