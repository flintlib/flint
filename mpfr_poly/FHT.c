...
void
_mpfr_poly_FHT_recursive(mpfr * coeffs, long n, mpfr_prec_t prec,
                         mpfr * costw, mpfr * sintw, long stride)
{
    long i, len2;
    mpfr_t temp, temp2;
    ...
    if (len2 != 1)
    {
        for (i = 1; i < len2 / 4; i++)
        {
            ...
            mpfr_mul(coeffs + 2 * len2 - i, coeffs + 2 * len2 - i, cos,
                     GMP_RNDN);

            mpfr_sub(coeffs + 2 * len2 - i, coeffs + len2 + i,
                     coeffs + 2 * len2 - i, GMP_RNDN);
            ...
            mpfr_mul(coeffs + 3 * len2 / 2 + i, coeffs + 3 * len2 / 2 + i, sin,
                     GMP_RNDN);
            mpfr_mul(coeffs + 3 * len2 / 2 - i, coeffs + 3 * len2 / 2 - i, cos,
                     GMP_RNDN);

            mpfr_sub(coeffs + 3 * len2 / 2 + i, coeffs + 3 * len2 / 2 - i,
                     coeffs + 3 * len2 / 2 + i, GMP_RNDN);
        ...
        if (len2 != 2)
        {
            ...
            mpfr_add(temp, coeffs + 5 * len2 / 4, coeffs + 7 * len2 / 4,
                     GMP_RNDN);
            mpfr_sub(coeffs + 7 * len2 / 4, coeffs + 5 * len2 / 4,
                     coeffs + 7 * len2 / 4, GMP_RNDN);
            mpfr_swap(temp, coeffs + 5 * len2 / 4);
            mpfr_mul(coeffs + 5 * len2 / 4, coeffs + 5 * len2 / 4, sin,
                     GMP_RNDN);
            mpfr_mul(coeffs + 7 * len2 / 4, coeffs + 7 * len2 / 4, sin,
                     GMP_RNDN);
        }
    }
    ...
    _mpfr_poly_FHT_recursive(coeffs + len2, n - 1, prec, costw, sintw,
                             stride * 2);
}

void
_mpfr_poly_FHT(mpfr * coeffs, long n, mpfr_prec_t prec)
{
    mpfr *cos, *sin;
    long i, length = (1L << n);
    mpfr_t t1, t2;
