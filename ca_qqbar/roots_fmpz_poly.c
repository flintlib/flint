...
void
ca_qqbar_roots_fmpz_poly(ca_qqbar_ptr res, const fmpz_poly_t poly, int flags)
{
    ...
    if (flags & CA_QQBAR_ROOTS_IRREDUCIBLE)
    {
        ...
        for (prec = CA_QQBAR_DEFAULT_PREC; ; prec *= 2)
        {
            ...
            for (i = 0; i < d; i++)
            {
                if (_ca_qqbar_validate_uniqueness(croots + i, poly, croots + i, prec))
                    checked++;
                else
                    break;
            }
            ...
            if (checked == d)
            {
                ...
                {
                    if (fmpz_is_one(c))
                        fmpz_poly_set(CA_QQBAR_POLY(res + i), poly);
                    else
                        fmpz_poly_scalar_divexact_fmpz(CA_QQBAR_POLY(res + i), poly, c);
                    acb_set(CA_QQBAR_ENCLOSURE(res + i), croots + i);
                }
    ...
    else
    {
        ...
        for (i = 0; i < fac->num; i++)
        {
            ...
            ca_qqbar_roots_fmpz_poly(out, fac->p + i, CA_QQBAR_ROOTS_IRREDUCIBLE);
            e = fac->exp[i];
            ...
            if (e > 1)
            {
                for (j = facd - 1; j >= 0; j--)
                {
                    ca_qqbar_set(out + j * e, out + j);
                    for (k = 1; k < e; k++)
                        ca_qqbar_set(out + j * e + k, out + j * e);
                }
            }
