...
void
ca_qqbar_scalar_op(ca_qqbar_t res, const ca_qqbar_t x, const fmpz_t a, const fmpz_t b, const fmpz_t c)
{
    fmpz_poly_t H;
    fmpz_t one, Gden;
    fmpz G[2];
    ...
    if (fmpz_is_zero(c))
    {
        flint_printf("ca_qqbar_scalar_op: division by zero\n");
        flint_abort();
    }
    ...
    _fmpq_poly_compose2(H->coeffs, CA_QQBAR_POLY(x)->coeffs, one, d + 1, G, Gden, 2);
