...
int main()
{
    ...
    for (iter = 0; iter < 1000; iter++)
    {
        ...
        ca_qqbar_roots_fmpz_poly(r, poly, 0);

        for (i = 0; i < d; i++)
        {
            prec = 2 * CA_QQBAR_DEFAULT_PREC;

            ca_qqbar_get_acb(x, r + i, prec);
            ...
            if (!acb_contains_zero(y))
            {
                ...
                flint_printf("r = "); ca_qqbar_print(r + i); flint_printf("\n\n");
