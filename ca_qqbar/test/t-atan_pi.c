...
int main()
{
    ...
    for (iter = 0; iter < 1000; iter++)
    {
        ...
        ca_qqbar_tan_pi(x, p, q);
        res = ca_qqbar_atan_pi(&p2, &q2, x);
        if (res)
            ca_qqbar_tan_pi(y, p2, q2);

        if (!res || !ca_qqbar_equal(x, y) || n_gcd(FLINT_ABS(p2), q2) != 1 || !(-(slong) q2 < 2 * p2 && 2 * p2 < (slong) q2))
        {
            flint_printf("FAIL!\n");
