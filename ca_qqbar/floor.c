...
void
ca_qqbar_floor(fmpz_t res, const ca_qqbar_t x)
{
    if (ca_qqbar_is_rational(x))
    {
        fmpz_cdiv_q(res, CA_QQBAR_COEFFS(x), CA_QQBAR_COEFFS(x) + 1);
        fmpz_neg(res, res);
    }
    else
    {
        ...
        arb_floor(v, acb_realref(CA_QQBAR_ENCLOSURE(x)), CA_QQBAR_DEFAULT_PREC);
        if (!arb_get_unique_fmpz(res, v))
        {
            ...
            if (!arb_get_unique_fmpz(res, v))
            {
                ...
                if (!arb_get_unique_fmpz(res, v))
                {
                    flint_printf("ca_qqbar_floor: either floor(x) or floor(x+1/2) should evaluate numerically\n");
                    flint_abort();
                }
