...
ca_qqbar_is_root_of_unity(slong * p, const ca_qqbar_t x)
{
    ...
    n = fmpz_poly_is_cyclotomic(CA_QQBAR_POLY(x));
    ...
    {
        if (p != NULL) *p = (ca_qqbar_sgn_im(x) > 0) ? 1 : 2;
    }
    else if (n == 4)
    {
        if (p != NULL) *p = (ca_qqbar_sgn_im(x) > 0) ? 1 : 3;
    }
    else
    {
        if (p != NULL)
        {
            ...
            {
                flint_printf("ca_qqbar_is_root_of_unity: unexpected precision issue\n");
                flint_abort();
            }
...
void
ca_qqbar_root_of_unity(ca_qqbar_t res, slong p, ulong q)
{
    fmpq_t t;
    ulong a, b;
    slong prec;
    ...
    if (q == 0)
    {
        flint_printf("ca_qqbar_root_of_unity: q = 0\n");
        flint_abort();
    }
    ...
    {
        ca_qqbar_i(res);
        ca_qqbar_conj(res, res);
    }
    else
    {
        fmpz_poly_cyclotomic(CA_QQBAR_POLY(res), b);
        fmpq_mul_2exp(t, t, 1);

        for (prec = CA_QQBAR_DEFAULT_PREC / 2; ; prec *= 2)
        {
            arb_sin_cos_pi_fmpq(acb_imagref(CA_QQBAR_ENCLOSURE(res)),
                                acb_realref(CA_QQBAR_ENCLOSURE(res)),
                                t, prec);
