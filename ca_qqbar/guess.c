...
#define CA_QQBAR_GUESS_TRY_SMALLER 1
#define CA_QQBAR_GUESS_CHECK 2

int
ca_qqbar_guess(ca_qqbar_t res, const acb_t z, slong max_deg, slong max_bits, int flags, slong prec)
{
    acb_ptr zpow;
    int found;
    ...
    if (max_deg > 8 && (flags & CA_QQBAR_GUESS_TRY_SMALLER))
    {
        if (ca_qqbar_guess(res, z, max_deg / 4, max_bits, flags, prec))
            return 1;
    }
    ...
    if (_ca_qqbar_acb_lindep(poly->coeffs, zpow, max_deg + 1, 1, prec))
    {
        ...
        for (i = 0; i < fac->num; i++)
        {
            ...
            if (fac_bits <= max_bits)
            {
                ...
                if (acb_contains_zero(z2))
                {
                    ...
                    if (acb_rel_accuracy_bits(z) >= CA_QQBAR_DEFAULT_PREC - 3)
                    {
                        for (prec2 = CA_QQBAR_DEFAULT_PREC / 2; prec2 < 2 * prec; prec2 *= 2)
                        {
                            acb_set(z2, z);
                            acb_get_mag(rad, z);
                            ...
                            if (_ca_qqbar_validate_existence_uniqueness(z2, fac->p + i, z2, 2 * prec2))
                            {
                                fmpz_poly_set(CA_QQBAR_POLY(res), fac->p + i);
                                acb_set(CA_QQBAR_ENCLOSURE(res), z2);
                                found = 1;
                                break;
                            }
                        }
                    ...
                    roots = ca_qqbar_vec_init(deg);

                    ca_qqbar_roots_fmpz_poly(roots, fac->p + i, CA_QQBAR_ROOTS_IRREDUCIBLE);

                    for (j = 0; j < deg; j++)
                    {
                        ca_qqbar_get_acb(z2, roots + j, prec);
