...
*/
static void
ideal_mixed_erfi(ca_field_t K, slong i, slong j, slong index_i, ca_ctx_t ctx)
{
    ...
    if (ca_check_equal(ix, y, ctx) == T_TRUE)
    {
        ...
        fmpz_mpoly_init(poly, CA_FIELD_MCTX(K, ctx));
        if (Fi == CA_Erf)
            fmpz_mpoly_set_linear2_three_term_si(poly, 1, j, index_i, 1, i, 0, CA_FIELD_MCTX(K, ctx));
        else
            fmpz_mpoly_set_linear2_three_term_si(poly, -1, j, index_i, 1, i, -1, CA_FIELD_MCTX(K, ctx));
        _ca_field_ideal_insert_clear_mpoly(K, poly, CA_FIELD_MCTX(K, ctx), ctx);
    }
        ...
        for (i = 0; i < len; i++)
        {
            ...
            if (Fi == CA_Erf || Fi == CA_Erfc || Fi == CA_Erfi)
            {
                for (j = i + 1; j < len; j++)
                {
                    ...
                    {
                        if (have_i && (Fj == CA_Erfi && (Fi == CA_Erf || Fi == CA_Erfc)))
                        {
                            ideal_mixed_erfi(K, i, j, index_i, ctx);
                            continue;
                        }

                        if (have_i && (Fi == CA_Erfi && (Fj == CA_Erf || Fj == CA_Erfc)))
                        {
                            ideal_mixed_erfi(K, j, i, index_i, ctx);
                            continue;
                        }
