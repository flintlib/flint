...
void
_fmpq_sub(fmpz_t rnum, fmpz_t rden, const fmpz_t op1num, const fmpz_t op1den,
            const fmpz_t op2num, const fmpz_t op2den)
{
    if (fmpz_equal(op1den, op2den))
    {
        fmpz_sub(rnum, op1num, op2num);
...
{
    _fmpq_sub(&res->num, &res->den, &op1->num, &op1->den,
            &op2->num, &op2->den);
}
