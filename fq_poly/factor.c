                 ...
                 const fq_poly_t input, int algorithm, const fq_ctx_t ctx)
{
    ...
    if (len <= 1)
    {
        ...
        {
            fq_set(leading_coeff, input->coeffs);
        }
    }

    fq_poly_get_coeff(leading_coeff, input, fq_poly_degree(input), ctx);

    fq_poly_init(monic_input);
    fq_poly_make_monic(monic_input, input, ctx);

    if (len == 2)
    {
        fq_poly_factor_insert(result, monic_input, 1, ctx);
        fq_poly_clear(monic_input);
        fq_set(leading_coeff, input->coeffs + 1);
        return;
    }
    ...
    for (i = 0; i < sqfree_factors->num; i++)
    {
        ...
        fq_poly_factor_pow(factors, sqfree_factors->exp[i]);
        fq_poly_factor_concat(result, factors, ctx);
        fq_poly_factor_clear(factors);
    }

    fq_poly_factor_clear(sqfree_factors);
    ...
    else
    {
        ...
        for (i = 0; i < def_res->num; i++)
        {
            ...
            else
            {
                ...
                fq_poly_factor_pow(t, def_res->exp[i]);
                fq_poly_factor_concat(result, t, ctx);
                fq_poly_factor_clear(t);
            }
            fq_poly_clear(pol);
        }

        fq_poly_factor_clear(def_res);
    }
}
