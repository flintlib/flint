                      ...
                      mp_limb_t n, n_ecm_t n_ecm_inf)
{
    ...
    arrx = _nmod_vec_init(maxj + 1);
    arrz = _nmod_vec_init(maxj + 1);
    ...
    /* arr[2] = 2Q0 */
    n_factor_ecm_double(arrx + 2, arrz + 2, arrx[1], arrz[1], n, n_ecm_inf);

    /* arr[3] = 3Q0 */
    n_factor_ecm_add(arrx + 3, arrz + 3, arrx[2], arrz[2], arrx[1], arrz[1],
                     arrx[1], arrz[1], n, n_ecm_inf);
    ...
    for (j = 5; j <= maxj; j += 2)
    {
        ...
        n_factor_ecm_add(arrx + j, arrz + j, arrx[j - 2], arrz[j - 2], arrx[2],
                         arrz[2], arrx[j - 4], arrz[j - 4], n, n_ecm_inf);
    }
    ...
    for (i = mmin; i <= mmax; i ++)
    {
        for (j = 1; j <= maxj; j+=2)
        {
            ...
            {
                a = n_mulmod_preinv(Rx, arrz[j], n, n_ecm_inf->ninv, n_ecm_inf->normbits);
                b = n_mulmod_preinv(Rz, arrx[j], n, n_ecm_inf->ninv, n_ecm_inf->normbits);
                a = n_submod(a, b, n);
