                                   ...
                                   mp_limb_t k, mp_limb_t n, n_ecm_t n_ecm_inf)
{
    ...
    while (1)
    {
        ...
        {
            n_factor_ecm_add(&x1, &z1, x1, z1, x2, z2, x0, z0, n, n_ecm_inf);   /* Q <- P + Q */
            n_factor_ecm_double(&x2, &z2, x2, z2, n, n_ecm_inf);                /* P <- 2 * P */
        }
        else
        {   
            n_factor_ecm_add(&x2, &z2, x1, z1, x2, z2, x0, z0, n, n_ecm_inf);   /* P <- P + Q */
            n_factor_ecm_double(&x1, &z1, x1, z1, n, n_ecm_inf);                /* Q <- 2 * Q */        
        }
