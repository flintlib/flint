                 ...
                 mp_limb_t n, n_ecm_t n_ecm_inf)
{
    ...
    u = n_mulmod_preinv(u, v, n, n_ecm_inf->ninv, n_ecm_inf->normbits); /* u = (x2 - z2) * (x1 + z1) */
    v = n_submod(x1, z1, n);            /* v = (x1 - z1) */
    w = n_addmod(x2, z2, n);    /* w = (x2 + z2) */
    v = n_mulmod_preinv(v, w, n, n_ecm_inf->ninv, n_ecm_inf->normbits); /* v = (x1 - z1) * (x2 + z2) */
    w = n_addmod(u, v, n);      /* w = 2 * (x1 * x2 - z1 * z2) */
    v = n_submod(v, u, n);          /* v = 2 * (x2 * z1 - x1 * z2) */
    w = n_mulmod_preinv(w, w, n, n_ecm_inf->ninv, n_ecm_inf->normbits); /* w = 4 * (x1 * x2 - z1 * z2)^2 */
    v = n_mulmod_preinv(v, v, n, n_ecm_inf->ninv, n_ecm_inf->normbits); /* v = 4 * (x2 * z1 - x1 * z2)^2 */
    *x = n_mulmod_preinv(z0, w, n, n_ecm_inf->ninv, n_ecm_inf->normbits);   /* x = 4 * z0 * (x1 * x2 - z1 * z2)^2 */
    *z = n_mulmod_preinv(x0, v, n, n_ecm_inf->ninv, n_ecm_inf->normbits);   /* z = 4 * x0 * (x2 * z1 - x1 * z2)^2 */
}
