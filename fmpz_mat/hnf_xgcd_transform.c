...
void fmpz_mat_hnf_xgcd_transform(fmpz_mat_t H, fmpz_mat_t U, const fmpz_mat_t A)
{
    ...
    for (j = 0, k = 0, l = (A->c - A->r)*(A->c > A->r); A->c - j != l; j++, k++)
    {
        for (i = k + 1; i != A->r; i++)
        {
            ...
            for (j2 = 0; j2 < U->c; j2++)
            {
                fmpz_mul(b, u, fmpz_mat_entry(U, i, j2));
                fmpz_addmul(b, v, fmpz_mat_entry(U, i - 1, j2));
                fmpz_mul(fmpz_mat_entry(U, i - 1, j2), r1d, fmpz_mat_entry(U, i - 1, j2));
                fmpz_submul(fmpz_mat_entry(U, i - 1, j2), r2d, fmpz_mat_entry(U, i, j2));
                fmpz_set(fmpz_mat_entry(U, i, j2), b);
            }
            for (j2 = j; j2 < A->c; j2++)
            {
        ...
        fmpz_mat_swap_rows(U, NULL, A->r - 1, k);
        if (fmpz_sgn(fmpz_mat_entry(H, k, j)) < 0)
        {
            for (j2 = 0; j2 < U->c; j2++)
                fmpz_neg(fmpz_mat_entry(U, k, j2),
