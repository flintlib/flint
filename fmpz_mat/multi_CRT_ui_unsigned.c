    ...
    fmpz_comb_t comb, fmpz_comb_temp_t temp)
{
    ...
    for (i = 0; i < fmpz_mat_nrows(mat); i++)
    {
        for (j = 0; j < fmpz_mat_ncols(mat); j++)
        {
            ...
            fmpz_multi_CRT_ui_unsigned(fmpz_mat_entry(mat, i, j),
                r, comb, temp);
        }
    }
...
void
fmpz_mat_multi_CRT_ui_unsigned(fmpz_mat_t mat, nmod_mat_t * const residues,
                long nres)
{
    fmpz_comb_t comb;
    ...
    fmpz_mat_multi_CRT_ui_unsigned_precomp(mat, residues, nres, comb, temp);
