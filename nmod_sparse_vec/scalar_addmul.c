...
void nmod_sparse_vec_scalar_addmul_nmod(nmod_sparse_vec_t w, const nmod_sparse_vec_t u, const nmod_sparse_vec_t v, mp_limb_t c, nmod_t mod)
{
    ...
    else if (c == UWORD(1)) nmod_sparse_vec_add(w, u, v, mod);
    else if (c + UWORD(1) == mod.n) nmod_sparse_vec_sub(w, u, v, mod);
    else if (unnz == 0) nmod_sparse_vec_scalar_mul_nmod(w, v, c, mod);
    else
    {
        ...
        while ((k = _nmod_sparse_vector_merge_descend (&we, &ue, &ve, u, v)) >= 0)
        {
            switch(k)
            {
                ...
                case 1: we->val = nmod_mul(ve->val, c, mod); break;
                default: we->val = nmod_add (ue->val, nmod_mul(ve->val, c, mod), mod);
