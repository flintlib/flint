...
/* A = A * X^pow */
static int _fq_nmod_mpoly_pmul(fq_nmod_mpoly_t A, const fq_nmod_mpoly_t X,
            const fmpz_t pow, fq_nmod_mpoly_t T, const fq_nmod_mpoly_ctx_t ctx)
{
    ulong p;
    ...
    if (!fmpz_fits_si(pow))
    {
        if (!fq_nmod_mpoly_pow_fmpz(T, X, pow, ctx))
        {
               ...
               const fq_nmod_mpoly_ctx_t ctxB, const fq_nmod_mpoly_ctx_t ctxAC)
{
    ...
    regs   = (fq_nmod_mpoly_struct *) TMP_ALLOC(nvars*sizeof(fq_nmod_mpoly_struct));
    for (i = 0; i < nvars; i++)
        fq_nmod_mpoly_init(regs + i, ctxAC);
    fq_nmod_mpoly_init(temp, ctxAC);
...
HornerForm:
    ...
    else if (totalcounts == 1)
    {
        ...
        if (!fq_nmod_mpoly_pow_fmpz(regs + rp, C[v], Buexp + nvars*f + v, ctxAC))
        {
            success = 0;
        }
...
HornerForm2:
        ...
        if (rtypes[rp - 1] == -WORD(1) && rtypes[rp] == -WORD(1))
        {
            ...
            if (!_fq_nmod_mpoly_pmul(regs + rp - 1, C[(stack + sp)->v_var],
                                             (stack + sp)->v_exp, temp, ctxAC))
            {
                success = 0;
            }
            fq_nmod_mpoly_add(temp, regs + rp - 1, regs + rp, ctxAC);
            fq_nmod_mpoly_swap(temp, regs + rp - 1, ctxAC);
        }
        ...
        else if (rtypes[rp - 1] != -WORD(1) && rtypes[rp] == -WORD(1))
        {
            ...
            if (!fq_nmod_mpoly_pow_fmpz(temp, C[(stack + sp)->v_var],
    ...
    else
    {
        ...
        if (!_fq_nmod_mpoly_pmul(regs + rp, C[(stack + sp)->v_var],
                                             (stack + sp)->v_exp, temp, ctxAC))
        {
            success = 0;
        }
...
cleanup:
    ...
    for (i = 0; i < nvars; i++)
        fq_nmod_mpoly_clear(regs + i, ctxAC);
    fq_nmod_mpoly_clear(temp, ctxAC);
