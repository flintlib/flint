...
void qsieve_ll_linalg_init(qs_t qs_inf)
{
    ...
    qs_inf->small = malloc(qs_inf->small_primes*sizeof(mp_limb_t));
    qs_inf->factor = malloc(qs_inf->max_factors*sizeof(fac_t));
    qs_inf->matrix = malloc((qs_inf->buffer_size + qs_inf->qsort_rels)*sizeof(la_col_t));
    qs_inf->unmerged = qs_inf->matrix + qs_inf->buffer_size;
    qs_inf->Y_arr = malloc(qs_inf->buffer_size*sizeof(fmpz));
    qs_inf->curr_rel = qs_inf->relation
                     = malloc(2*qs_inf->buffer_size*qs_inf->max_factors*sizeof(long));
    qs_inf->qsort_arr = malloc(qs_inf->qsort_rels*sizeof(la_col_t *));
    ...
    qs_inf->prime_count = malloc(qs_inf->num_primes*sizeof(long));
