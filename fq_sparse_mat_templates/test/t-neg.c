...
main(void)
{
    ...
    for (rep = 0; rep < 1000; rep++)
    {
        ...
        TEMPLATE(T, sparse_mat_randtest) (D, state, 0, c, ctx);

        TEMPLATE(T, sparse_mat_sub) (C, A, B, ctx);
        TEMPLATE(T, sparse_mat_neg) (B, B, ctx);
        TEMPLATE(T, sparse_mat_add) (D, A, B, ctx);

        if (!TEMPLATE(T, sparse_mat_equal) (C, D, ctx))
        {
            flint_printf("FAIL\n");
            abort();
        }

        TEMPLATE(T, sparse_mat_neg) (C, B, ctx);
        TEMPLATE(T, sparse_mat_neg) (B, B, ctx);

        if (!TEMPLATE(T, sparse_mat_equal) (C, B, ctx))
        {
            flint_printf("FAIL\n");
            abort();
        }
