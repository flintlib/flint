				...
				ulong len1, mp_srcptr poly2, ulong len2, ulong start, nmod_t mod)
{
   ...
   } else // Ordinary case
   {
	  ...
	  if (2*bits + log_len <= FLINT_BITS)
	  {
	     ...
	     if (len2 != 1)
		 {
            ...
            for (i = m - len2 + 1; i < len1 - 1; i++)
	        {
	           ...
	           mpn_addmul_1(res + n, poly2 + n - i, len2 + i - n, poly1[i]);
	        }
		 }
	  ...
	  } else
	  {
	     ...
	     {
		    n = FLINT_MAX(i + 1, start);
	        _nmod_vec_scalar_addmul(res + n, poly2 + n - i, len2 + i - n, mod, poly1[i]);
	     }
	  }
   } 
}
...
{
   ulong len_out = poly1->length + poly2->length - 1;
   ...
   if (res == poly1 || res == poly2)
   {
	   ...
	   if (poly1->length >= poly2->length)
		  _nmod_poly_mulhigh_classical(temp->coeffs, poly1->coeffs, poly1->length, 
		                                 poly2->coeffs, poly2->length, start, poly1->mod);
	   else
		  _nmod_poly_mulhigh_classical(temp->coeffs, poly2->coeffs, poly2->length, 
		                                 poly1->coeffs, poly1->length, start, poly1->mod);		   
   ...
   } else
   {
	   ...
	   if (poly1->length >= poly2->length)
		  _nmod_poly_mulhigh_classical(res->coeffs, poly1->coeffs, poly1->length, 
		                                 poly2->coeffs, poly2->length, start, poly1->mod);
	   else
		  _nmod_poly_mulhigh_classical(res->coeffs, poly2->coeffs, poly2->length, 
		                                 poly1->coeffs, poly1->length, start, poly1->mod);		   
   }
