...
void
_nmod_poly_sinh_series(ulong_ptr f, ulong_srcptr h, slong n, nmod_t mod)
{
    ulong_ptr g = _nmod_vec_init(n);
...
nmod_poly_sinh_series(nmod_poly_t g, const nmod_poly_t h, slong n)
{
    ...
    if (h_len > 0 && h->coeffs[0] != UWORD(0))
        flint_throw(FLINT_ERROR, "Requires constant term 0 in nmod_poly_sqrt_series\n");
