...
void _nmod_poly_divrem_newton_n_preinv(ulong_ptr Q, ulong_ptr R, ulong_srcptr A,
                                        slong lenA, ulong_srcptr B, slong lenB,
                                      ...
                                      const nmod_poly_t Binv)
{
    ...
    if (lenB == 0)
    {
        if (nmod_poly_modulus(B) == 1)
        {
            ...
            flint_throw(FLINT_DIVZERO, "nmod_poly_divrem_newton_n_preinv\n");
    }
    ...
    if (lenA > 2 * lenB - 2)
        flint_throw(FLINT_ERROR, "nmod_poly_divrem_newton_n_preinv\n");
