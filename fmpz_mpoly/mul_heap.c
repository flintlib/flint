...
void
fmpz_mpoly_mul_heap(fmpz_mpoly_t res, fmpz_mpoly_t poly1, fmpz_mpoly_t poly2)
{
    ...
    for (i = 0; i < len2; i++)
    {
        ...
        if (b > bits2)
            bits2 = b;
        if (fmpz_sgn(poly2->coeffs + i) < 0)
            signed_c = 1;
    }

    b = bits1 + bits2 + FLINT_BIT_COUNT(len2);
    if (b < FLINT_BITS - 2)
        b = 0;                  /* output will fit in a small fmpz */
    else if (!signed_c && bits1 <= FLINT_BITS - 2 && bits2 <= FLINT_BITS - 2)
        b = 2;                  /* unsigned <= 3 limbs */
    else
        b = 3;
    ...
    /* 
       Insert only the first of these entries into the heap - we inject the 
       others later; we do this manually as the heap_insert can't handle an 
       empty heap
     */
    ...
    while (n)
    {
        ...
        /*
           Chain all heap elements with the same exp together 
           and remove from heap
         */
        while (1)
        {
            prev = next;
        ...
        do
        {
            ...
            else if (b == 2)    /* input coeffs fit in a small and are unsigned */
            {
                ...
                add_ssaaaa(cy, accum0, (mp_limb_t) 0, accum0, (mp_limb_t) 0,
                           n1);
            ...
            else                /* general case */
            {
                if (!COEFF_IS_MPZ(c1) && !COEFF_IS_MPZ(c2))
                {
                    ...
                    add_ssaaaa(cy, accum0, (mp_limb_t) 0, accum0,
                               (mp_limb_t) 0, n1);
                    add_ssaaaa(accum2, accum1, accum2, accum1, (mp_limb_t) 0,
                               cy);
                    add_ssaaaa(accum2, accum1, accum2, accum1, n3, n2);
                }
                else
                    fmpz_addmul(temp, poly1->coeffs + next->i1,
                                poly2->coeffs + next->i2);
            }

            if (next->i1 == 0 && next->i2 < len2 - 1)   /* inject next entry poly1[1]*poly2[j+1] */
                fmpz_mpoly_heap_insert(heap, &n, next + 1,
                                       poly1->exps[0] + poly2->exps[next->i2 +
                                                                    1]);
            ...
            prev->i1++;
            if (prev->i1 < len1)
                fmpz_mpoly_heap_insert(heap, &n, prev,
                                       poly1->exps[prev->i1] +
                                       poly2->exps[prev->i2]);
        } while (next);
        ...
        else
        {
            ...
            if (accum2 != (mp_limb_t) 0)
                size = 3;       /* normalise */
            else if (accum1 != (mp_limb_t) 0)
                size = 2;
            else
            {
                _fmpz_demote(res->coeffs + len_out);
                if (accum0 != (mp_limb_t) 0)
                    size = 1;
                else
                {
                    fmpz_swap(res->coeffs + len_out, temp); /* set coeff */
                    fmpz_zero(temp);    /* zero temp for next coefficient */
                    if (fmpz_is_zero(res->coeffs + len_out))
                        continue;
                    else
                    {
