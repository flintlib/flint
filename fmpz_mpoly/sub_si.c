                 ...
                 const fmpz_mpoly_t poly2, slong c, const fmpz_mpoly_ctx_t ctx)
{
   ...
   if (c != 0)
   {
      ...
      if (mpoly_monomial_is_zero(poly2->exps + (len2 - 1)*N, N))
      {
         ...
         if (c > 0)
            fmpz_sub_ui(poly1->coeffs + len2 - 1, 
                                          poly2->coeffs + len2 - 1, (ulong) c);
         else
            fmpz_add_ui(poly1->coeffs + len2 - 1, 
                                         poly2->coeffs + len2 - 1, (ulong) -c);
      ...
      } else
      {
         ...
         fmpz_set_si(poly1->coeffs + len2, c);
         fmpz_neg(poly1->coeffs + len2, poly1->coeffs + len2);
