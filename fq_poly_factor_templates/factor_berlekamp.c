...
static void
TEMPLATE(T, to_mat_col)(TEMPLATE(T, mat_t) mat, slong col, TEMPLATE(T, poly_t) poly, const TEMPLATE(T, ctx_t) ctx)
{
    slong i;

    for (i = 0; i < poly->length; i++)
        TEMPLATE(T, set)(TEMPLATE(T, mat_entry)(mat, i, col), poly->coeffs + i, ctx);
...
static void
TEMPLATE(T, mat_col_to_shifted)(TEMPLATE(T, poly_t) poly, TEMPLATE(T, mat_t) mat, slong col, slong * shift,
                      const TEMPLATE(T, ctx_t) ctx)
{
    ...
    for (i = 0, j = 0; j < rows; j++)
    {
        ...
        {
            TEMPLATE(T, set)(poly->coeffs + j, TEMPLATE(T, mat_entry)(mat, i, col), ctx);
            i++;
        }
    }
...
static void
__TEMPLATE(T, poly_factor_berlekamp)(TEMPLATE(T, poly_factor_t) factors, flint_rand_t state,
                           const TEMPLATE(T, poly_t) f, const TEMPLATE(T, ctx_t) ctx)
{
    ...
    for (i = 1; i < nullity; i++)
    {
        ...
        while (!TEMPLATE(T, is_zero)(TEMPLATE(T, mat_entry)(matrix, row, col), ctx))
        {
            row++;
            col++;
        }
        ...
        while (1)
        {
            do
            {
                ...
                for (i = 1; i < nullity; i++)
                {
                    ...
                    TEMPLATE(T, TEMPLATE(poly_scalar_mul, T))(b, basis[i], mul, ctx);
            ...
            while (TEMPLATE(T, poly_is_zero)(factor, ctx) ||
                   (factor->length < 2 && TEMPLATE(T, is_one)(factor->coeffs, ctx)));
            ...
            if (fmpz_cmp_ui(p, 3) > 0)
                TEMPLATE(T, poly_powmod_fmpz_binexp)(power, factor, pow, f, ctx);
            else
                         ...
                         const TEMPLATE(T, ctx_t) ctx)
{
    ...
    {
        __TEMPLATE(T, poly_factor_berlekamp)(factors, r, sq_free->poly + i, ctx);
    }
    flint_randclear(r);
