...
main(void)
{
    ...
    for (iter = 0; iter < flint_test_multiplier(); iter++)
    {
        ...
        switch (n_randint(state, 4))
        {
            ...
            case 2:
                if (fmpz_is_even(TEMPLATE(T, ctx_prime)(ctx)))
                    ...
                    TEMPLATE(T, poly_factor_with_cantor_zassenhaus)(res, lead, pol1, ctx);
                break;
            case 3:
                TEMPLATE(T, poly_factor_with_kaltofen_shoup)(res, lead, pol1, ctx);
                break;
        ...
        TEMPLATE(T, TEMPLATE(poly_scalar_mul, T))(product, product, lead, ctx);
    ...
    for (iter = 0; iter < flint_test_multiplier(); iter++)
    {
        ...
        for (i = 0; i < res->num; i++)
        {
            ...
            {
                if (TEMPLATE(T, poly_equal)(res->poly + i, res2->poly + j, ctx) &&
