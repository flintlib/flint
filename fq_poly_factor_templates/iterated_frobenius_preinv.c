                                            ...
                                            const TEMPLATE(T, ctx_t) ctx)
{
   ...
   if (TEMPLATE(CAP_T, POLY_ITERATED_FROBENIUS_CUTOFF)(ctx, v->length))
   {
       ...
       TEMPLATE(T, poly_powmod_fmpz_sliding_preinv)(rop[1], rop[0], q, 0, v, vinv, ctx);
       TEMPLATE(T, poly_precompute_matrix)(HH, rop[1], v, vinv, ctx);
       for (i = 2; i < n; i++)
           TEMPLATE(T, poly_compose_mod_brent_kung_precomp_preinv)(rop[i], rop[i - 1],
                                                                   HH, v, vinv, ctx);
   ...
   {
       for (i = 1; i < n; i++)
           TEMPLATE(T, poly_powmod_fmpz_sliding_preinv)(rop[i], rop[i - 1], q, 0, v, vinv, ctx);
