...
mpz_mock_ptr * mpz_free_arr = NULL;
mpz_mock_ptr * mpz_arr = NULL;
ulong mpz_num = 0;
ulong mpz_alloc = 0;
...
mpz_mock_ptr _fmpz_new_mpz(void)
{
    ...
    {
        z = flint_malloc(sizeof(__mpz_mock_struct));

        if (mpz_num == mpz_alloc) /* store pointer to prevent gc cleanup */
        {
            mpz_alloc = FLINT_MAX(64, mpz_alloc * 2);
            mpz_arr = flint_realloc(mpz_arr, mpz_alloc * sizeof(mpz_mock_ptr));
        }
        mpz_arr[mpz_num++] = z;
