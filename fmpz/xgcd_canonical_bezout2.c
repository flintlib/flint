...
void
fmpz_xgcd_canonical_bezout2(fmpz_t d, fmpz_t a, fmpz_t b, const fmpz_t f, const fmpz_t g)
{
    /* check aliasing */
    ...
    if (!COEFF_IS_MPZ(*f) && !COEFF_IS_MPZ(*g))  /* both are small */
    {
        ...
        else if (fn < gn)
        {
            ...
            *d = n_gcdinv((ulong *) a, fn, gn); /* Done with (1) */
        ...
        {
            /* Like previous case, but with the switch
             * f <-> g and a <-> b */
            slong k, tmp;

            *d = n_gcdinv((ulong *) b, gn, fn);
