...
TEST_FUNCTION_START(nmod_mpoly_get_set_term_exp_si, state)
{
    slong i, j, k;
    int result;
    ...
    for (i = 0; i < 100 * flint_test_multiplier(); i++)
    {
        ...
        for (j = 0; j < 10; j++)
        {
            ...
            nmod_mpoly_set_term_exp_ui(f, index, (ulong *) exp1, ctx);
            ...
            nmod_mpoly_get_term_exp_si(exp2, f, index, ctx);
            ...
            for (k = 0; k < nvars; k++)
            {
                result = result
                 && exp1[k] == exp2[k]
                 && exp1[k] == nmod_mpoly_get_term_var_exp_si(f, index, k, ctx);
            }
