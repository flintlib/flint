...
typedef struct
{
    nn_ptr valA;
    nn_ptr valB;
    const n_poly_struct * Ax;
    const n_poly_struct * Bx;
    slong lenA;
    slong lenB;
    slong stride;
    slong blen;
...
_geom_eval_worker(slong widx, void * argsv)
{
    ...
    for (i = widx; i < A->lenA + A->lenB; i += A->nworkers)
    {
        ...
        for (k = 0; k < P->length; k++)
            tmp[k] = nmod_mul(P->coeffs[k], A->spow[k], A->mod);
              ...
              slong M, ulong m, double * dbuf, sd_fft_ctx_t Q, nmod_t mod)
{
    ...
    for (k = 0; k < M; k++)
        vs[k] = vec1d_reduce_to_0n(dbuf[k], Q->p, Q->pinv);
}
...
_dft_eval_worker(slong widx, void * argsv)
{
    ...
    for (i = widx; i < A->lenA + A->lenB; i += A->nworkers)
    {
        ...
        _dft_evaluate(out, P->coeffs, P->length, A->spow,
            A->M, A->m, dbuf, A->Q, A->mod);
    }
}

/* res_y(A, B) by evaluation at roots of unity. Returns 0 when the
...
static int
_n_bpoly_mod_resultant_dft(n_poly_struct * res,
                           const n_poly_struct * Ax, slong lenA,
                           const n_poly_struct * Bx, slong lenB,
                           slong npoints, slong maxlen, nmod_t mod)
{
    ...
    /* A primitive N-th root of unity w must exist, which is what the
       transforms of fft_small need of the modulus too. The geometric
    ...
    if (use_ifft)
    {
        ...
        for (k = 0; k < npoints; k++)
            res->coeffs[k] =
                nmod_mul(vec1d_reduce_to_0n(d[k], Q->p, Q->pinv), Ninv, mod);
    }
    else
    {
        ...
        _nmod_vec_set((nn_ptr) res->coeffs, resc, npoints);
    }
...
/* Where the subresultant PRS works throughout with polynomials in x of the
   ...
   less are better served by a direct formula and are not expected here. */
#define MULTIPOINT_CUTOFF 36
#define MULTIPOINT_CUTOFF_SMALL 16
#define MULTIPOINT_CUTOFF_POINTS 4096

int
n_bpoly_mod_resultant_cutoff(slong lenA, slong lenB, slong npoints)
{
...
/* res_y(A, B), where A and B are seen as polynomials in y of lengths lenA and
   lenB with coefficients in (Z/pZ)[x]. The modulus must be prime, and the
   leading coefficients in y must be nonzero, the degree bound and the
   specialisation property below both reading the degrees in y off the two
   lengths. Returns 0, leaving res untouched, when the field is too small to
   hold a geometric progression of the required length; the transform path is
   tried first and has its own conditions. */
int
_n_bpoly_mod_resultant(n_poly_t res, const n_poly_struct * Ax, slong lenA,
                       const n_poly_struct * Bx, slong lenB, nmod_t mod)
{
    nmod_geometric_progression_t G;
...
#if FLINT_HAVE_FFT_SMALL
    if (_n_bpoly_mod_resultant_dft(res, Ax, lenA, Bx, lenB,
            npoints, maxlen, mod))
        return 1;
#endif
    ...
    n_poly_fit_length(res, npoints);
    res->length = npoints;
    _nmod_vec_set(res->coeffs, resc, npoints);
    _n_poly_normalise(res);
...
int
n_bpoly_mod_resultant(n_poly_t res, const n_bpoly_t A, const n_bpoly_t B,
                      nmod_t mod)
{
    slong lenA = A->length;
    slong lenB = B->length;
    ...
    if (lenA >= lenB)
        return _n_bpoly_mod_resultant(res, A->coeffs, lenA, B->coeffs, lenB, mod);

    if (!_n_bpoly_mod_resultant(res, B->coeffs, lenB, A->coeffs, lenA, mod))
        return 0;

    /* res(A, B) = (-1)^(deg A deg B) res(B, A) */
    if (((lenA | lenB) & 1) == 0)
        _nmod_vec_neg(res->coeffs, res->coeffs, res->length, mod);
