...
TEST_FUNCTION_START(fmpz_mod_poly_factor_is_irreducible, state)
{
    int iter;
    fmpz_mod_ctx_t ctx;
    ...
    for (iter = 0; iter < 10 * flint_test_multiplier(); iter++)
    {
        ...
        while ((!fmpz_mod_poly_is_irreducible(poly1, ctx)) || (poly1->length < 2));
        ...
        for (i = 0; i < num; i++)
        {
            ...
            while ((!fmpz_mod_poly_is_irreducible(poly2, ctx)) || (poly2->length < 2));

            fmpz_mod_poly_mul(poly1, poly1, poly2, ctx);
        }

        if (fmpz_mod_poly_is_irreducible(poly1, ctx))
        {
