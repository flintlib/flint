...
static ulong
random_modulus_u52(flint_rand_t state)
{
    ulong bits, n;
    ...
    switch (n_randint(state, 8))
    {
        ...
        case 3:
            ...
            break;
        case 4:
            /* around 2^26, the switch between the two microkernels */
            n = (UWORD(1) << 26) - 8 + n_randint(state, 17);
            break;
        case 5:
...
*/
static int
mul_window_check_u52(const nmod_mat_t A, const nmod_mat_t B,
                     const nmod_mat_t D, flint_rand_t state)
{
    ...
    ok = nmod_mat_mul_u52(WC, WA, WB) && nmod_mat_equal(WC, D);
...
TEST_FUNCTION_START(nmod_mat_mul_u52, state)
{
    ...
    for (i = 0; i < 300 * flint_test_multiplier(); i++)
    {
        ...
        modulus = random_modulus_u52(state);
        ...
        if (ret)
        {
            ...
            if (n_randint(state, 2)
                    && !mul_window_check_u52(A, B, D, state))
            ...
            {
                nmod_mat_set(D, A);
                nmod_mat_mul_u52(D, D, B);
                if (!nmod_mat_equal(C, D))
                    TEST_FUNCTION_FAIL("aliasing C = A: m: %wd, mod: %wu\n",
                                       m, modulus);

                nmod_mat_set(D, B);
                nmod_mat_mul_u52(D, A, D);
                if (!nmod_mat_equal(C, D))
...
#if FLINT_BITS == 64
    /* a modulus above 2^52 must be declined */
    {
        nmod_mat_t A, B, C;
        ...
        if (nmod_mat_mul_u52(C, A, B) != 0)
            TEST_FUNCTION_FAIL("mul_u52 accepted a modulus of 2^52 + 1\n");
