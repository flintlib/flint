...
static void
randfull_max_fp50(nmod_mat_t mat, flint_rand_t state, int mixed)
{
    slong i, j;
    ulong n = mat->mod.n;
...
static ulong
random_modulus_fp50(flint_rand_t state)
{
    ulong bits, n;
    ...
    switch (n_randint(state, 8))
    {
        ...
        case 1:
            ...
            bits = 2 + n_randint(state, 49);
            n = (UWORD(1) << bits) - 1 - n_randint(state, 16);
            break;
        case 2:
            /* just above a power of two */
            bits = 1 + n_randint(state, 49);
            n = (UWORD(1) << bits) + 1 + n_randint(state, 16);
            break;
        case 3:
            /* the largest moduli, up to 2^50 - 1 */
            n = TWO50 - 1 - n_randint(state, 1000);
            break;
        case 4:
            /* around 2^45, where the in-loop folds start */
            bits = 44 + n_randint(state, 6);
            n = n_randbits(state, bits);
            break;
        case 5:
            /* the range where this method is meant to be used */
            bits = 31 + n_randint(state, 20);
            n = n_randbits(state, bits);
            break;
        case 6:
            /* powers of two */
...
TEST_FUNCTION_START(nmod_mat_mul_fp50, state)
{
    ...
    for (i = 0; i < 300 * flint_test_multiplier(); i++)
    {
        ...
        modulus = random_modulus_fp50(state);
        ...
        switch (n_randint(state, 4))
        {
            case 0:
                ...
                break;
            case 1:
                randfull_max_fp50(A, state, 0);
                randfull_max_fp50(B, state, 0);
                break;
            case 2:
                randfull_max_fp50(A, state, 1);
                randfull_max_fp50(B, state, 1);
                break;
            default:
        ...
        if (ret)
        {
            ...
            {
                nmod_mat_set(D, A);
                nmod_mat_mul_fp50(D, D, B);
                if (!nmod_mat_equal(C, D))
                    TEST_FUNCTION_FAIL("aliasing C = A: m: %wd, mod: %wu\n",
                                       m, modulus);

                nmod_mat_set(D, B);
                nmod_mat_mul_fp50(D, A, D);
                if (!nmod_mat_equal(C, D))
    ...
    /* a modulus out of range must be declined */
    {
        ...
        if (nmod_mat_mul_fp50(C, A, B) != 0)
            TEST_FUNCTION_FAIL("mul_fp50 accepted a modulus of 2^50\n");
