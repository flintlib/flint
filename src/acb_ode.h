...
extern "C" {
        ...
        slong acb_ode_group_length(const acb_ode_group_struct * grp);
        ...
        void acb_ode_sol_init(acb_ode_sol_struct * sol, slong nshifts,
                              slong nlogs, slong npts);

        void acb_ode_sol_clear(acb_ode_sol_struct * sol);

        void acb_ode_sol_reset(acb_ode_sol_struct * sol);

        void acb_ode_sol_fit_length(acb_ode_sol_struct * sol, slong len,
                                    slong nder);

        void acb_ode_sol_unit_ini(acb_ode_sol_struct * sol, slong i0,
                                  const acb_ode_shift_struct * shifts);

        acb_poly_struct * acb_ode_sol_sum_ptr(
                const acb_ode_sol_struct * sol, slong j, slong k);
        ...
        void _acb_ode_sol_value(acb_poly_struct * val, acb_srcptr expo,
...
#define acb_ode_WANT_SERIES 1
        ...
        typedef struct
        {
                ...
                /* solution group */
                acb_t expo;
                acb_ode_shift_struct * sing_shifts;
                acb_poly_t ind;
        ...
        typedef acb_ode_sum_context_struct acb_ode_sum_context_t[1];

        void acb_ode_sum_context_init(
                acb_ode_sum_context_struct * ctx,
                slong dop_len, slong npts, slong nsols, slong nder);
        void acb_ode_sum_context_clear(acb_ode_sum_context_struct * ctx);

        void acb_ode_sum_ordinary(acb_ode_sum_context_struct * ctx);
        void acb_ode_sum_canonical_basis(acb_ode_sum_context_struct * ctx);
        void acb_ode_sum_highest(acb_ode_sum_context_struct * ctx);
        void acb_ode_sum_group(acb_ode_sum_context_struct * ctx, const acb_ode_group_struct * grp);


        void _acb_ode_sum_precompute(acb_ode_sum_context_struct * ctx);
        void _acb_ode_sum_reset(acb_ode_sum_context_struct * ctx);

        void acb_ode_sum_divconquer(
                acb_ode_sum_context_struct * ctx, slong nterms);

        void acb_ode_sum_value(acb_poly_struct * val, slong nfrobshifts,
        ...
        typedef void (* acb_ode_sum_worker_t)(
                acb_ode_sum_context_struct * ctx, slong nterms);

        void _acb_ode_fundamental_matrix(
                acb_mat_struct * mat,
        ...
        int acb_ode_fundamental_matrix(
                acb_mat_struct * mat,
