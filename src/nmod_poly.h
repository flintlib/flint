...
NMOD_POLY_INLINE void nmod_poly_set_mod(nmod_poly_t poly, const nmod_t mod) { poly->mod = mod; }

NMOD_POLY_INLINE void _nmod_poly_set_length(nmod_poly_t poly, slong len) { poly->length = len; }
...
NMOD_POLY_INLINE slong nmod_poly_length(const nmod_poly_t poly) { return poly->length; }
NMOD_POLY_INLINE slong nmod_poly_degree(const nmod_poly_t poly) { return poly->length - 1; }
NMOD_POLY_INLINE mp_limb_t nmod_poly_modulus(const nmod_poly_t poly) { return poly->mod.n; }
...
NMOD_POLY_INLINE void nmod_poly_swap(nmod_poly_t poly1, nmod_poly_t poly2) { FLINT_SWAP(nmod_poly_struct, *poly1, *poly2); }

NMOD_POLY_INLINE void nmod_poly_zero(nmod_poly_t res) { res->length = 0; }
...
NMOD_POLY_INLINE void nmod_poly_truncate(nmod_poly_t poly, slong len) { nmod_poly_set_trunc(poly, poly, len); }
...
NMOD_POLY_INLINE int nmod_poly_is_zero(const nmod_poly_t poly) { return (poly->length == 0); }
NMOD_POLY_INLINE int nmod_poly_is_one(const nmod_poly_t poly) { return (poly->mod.n == 0) || (poly->length == 1 && poly->coeffs[0] == 1); }

/* bogus for non-prime modulus */
NMOD_POLY_INLINE int nmod_poly_is_unit(const nmod_poly_t poly) { return (poly->length == 1) && poly->coeffs[0] != 0; }
NMOD_POLY_INLINE int nmod_poly_is_gen(const nmod_poly_t poly) { return (poly->mod.n == 0) || (poly->length == 2 && poly->coeffs[0] == 0 && poly->coeffs[1] == 1); }
NMOD_POLY_INLINE int nmod_poly_is_monic(const nmod_poly_t poly) { return (poly->length && poly->coeffs[(poly->length - 1)] == 1); }

/* Randomisation  ************************************************************/

void nmod_poly_randtest(nmod_poly_t poly, flint_rand_t state, slong len);
NMOD_POLY_INLINE void nmod_poly_randtest_not_zero(nmod_poly_t poly, flint_rand_t state, slong len) { do { nmod_poly_randtest(poly, state, len); } while (nmod_poly_is_zero(poly)); }
...
NMOD_POLY_INLINE ulong nmod_poly_get_coeff_ui(const nmod_poly_t poly, slong j) { return (j >= poly->length) ? 0 : poly->coeffs[j]; }
...
void nmod_poly_scalar_mul_nmod(nmod_poly_t res, const nmod_poly_t poly, mp_limb_t c);
...
void _nmod_poly_bit_unpack(mp_ptr res, slong len, mp_srcptr mpn, flint_bitcnt_t bits, nmod_t mod);
...
void _nmod_poly_mullow(mp_ptr res, mp_srcptr poly1, slong len1, mp_srcptr poly2, slong len2, slong trunc, nmod_t mod);
...
void _nmod_poly_divrem_basecase_preinv1(mp_ptr Q, mp_ptr R, mp_srcptr A, slong A_len, mp_srcptr B, slong B_len, mp_limb_t invB, nmod_t mod);
...
void _nmod_poly_evaluate_fmpz(fmpz_t rop, mp_srcptr poly, const slong len, const fmpz_t c);
void nmod_poly_evaluate_fmpz(fmpz_t rop, const nmod_poly_t poly, const fmpz_t c);
...
void _nmod_poly_evaluate_nmod_vec(mp_ptr ys, mp_srcptr coeffs, slong len, mp_srcptr xs, slong n, nmod_t mod);
void nmod_poly_evaluate_nmod_vec(mp_ptr ys, const nmod_poly_t poly, mp_srcptr xs, slong n);

void _nmod_poly_evaluate_nmod_vec_iter(mp_ptr ys, mp_srcptr coeffs, slong len, mp_srcptr xs, slong n, nmod_t mod);
...
void _nmod_poly_evaluate_nmod_vec_fast(mp_ptr ys, mp_srcptr coeffs, slong len, mp_srcptr xs, slong n, nmod_t mod);
void nmod_poly_evaluate_nmod_vec_fast(mp_ptr ys, const nmod_poly_t poly, mp_srcptr xs, slong n);

void nmod_mat_one_addmul(nmod_mat_t dest, const nmod_mat_t mat, mp_limb_t c);
...
void _nmod_poly_interpolate_nmod_vec_fast(mp_ptr poly, mp_srcptr xs, mp_srcptr ys, slong len, nmod_t mod);
...
void _nmod_poly_precompute_matrix(nmod_mat_t A, mp_srcptr poly1, mp_srcptr poly2, slong len2, mp_srcptr poly2inv, slong len2inv, nmod_t mod);
void nmod_poly_precompute_matrix(nmod_mat_t A, const nmod_poly_t poly1, const nmod_poly_t poly2, const nmod_poly_t poly2inv);

void _nmod_poly_compose_mod_brent_kung_precomp_preinv_worker(void * arg_ptr);
void _nmod_poly_compose_mod_brent_kung_precomp_preinv(mp_ptr res, mp_srcptr poly1, slong len1, const nmod_mat_t A, mp_srcptr poly3, slong len3, mp_srcptr poly3inv, slong len3inv, nmod_t mod);
void nmod_poly_compose_mod_brent_kung_precomp_preinv(nmod_poly_t res, const nmod_poly_t poly1, const nmod_mat_t A, const nmod_poly_t poly3, const nmod_poly_t poly3inv);

void _nmod_poly_compose_mod_brent_kung_preinv(mp_ptr res, mp_srcptr poly1, slong len1, mp_srcptr poly2, mp_srcptr poly3, slong len3, mp_srcptr poly3inv, slong len3inv, nmod_t mod);
void nmod_poly_compose_mod_brent_kung_preinv(nmod_poly_t res, const nmod_poly_t poly1, const nmod_poly_t poly2, const nmod_poly_t poly3, const nmod_poly_t poly3inv);

void _nmod_poly_compose_mod_brent_kung_vec_preinv_worker(void * arg_ptr);
void _nmod_poly_compose_mod_brent_kung_vec_preinv(nmod_poly_struct * res, const nmod_poly_struct * polys, slong len1, slong l, mp_srcptr g, slong glen, mp_srcptr poly, slong len, mp_srcptr polyinv,slong leninv, nmod_t mod);
void nmod_poly_compose_mod_brent_kung_vec_preinv(nmod_poly_struct * res, const nmod_poly_struct * polys, slong len1, slong n, const nmod_poly_t g, const nmod_poly_t poly, const nmod_poly_t polyinv);
...
NMOD_POLY_INLINE slong nmod_poly_hamming_weight(const nmod_poly_t A) { return _nmod_poly_hamming_weight(A->coeffs, A->length); }
...
slong _nmod_poly_hgcd_recursive(mp_ptr * M, slong * lenM, mp_ptr A, slong * lenA, mp_ptr B, slong * lenB, mp_srcptr a, slong lena, mp_srcptr b, slong lenb, mp_ptr P, nmod_t mod, int flag, nmod_poly_res_t res);
...
slong _nmod_poly_xgcd_euclidean(mp_ptr res, mp_ptr s, mp_ptr t, mp_srcptr poly1, slong len1, mp_srcptr poly2, slong len2, nmod_t mod);
void nmod_poly_xgcd_euclidean(nmod_poly_t G, nmod_poly_t S, nmod_poly_t T, const nmod_poly_t A, const nmod_poly_t B);

slong _nmod_poly_xgcd_hgcd(mp_ptr G, mp_ptr S, mp_ptr T, mp_srcptr A, slong lenA, mp_srcptr B, slong lenB, nmod_t mod);
...
mp_limb_t _nmod_poly_resultant_hgcd(mp_srcptr A, slong lenA, mp_srcptr B, slong lenB, nmod_t mod);
mp_limb_t nmod_poly_resultant_hgcd(const nmod_poly_t A, const nmod_poly_t B);

mp_limb_t _nmod_poly_resultant(mp_srcptr A, slong lenA, mp_srcptr B, slong lenB, nmod_t mod);
...
int nmod_poly_find_distinct_nonzero_roots(mp_limb_t * roots, const nmod_poly_t P);
...
NMOD_POLY_INLINE slong _nmod_poly_multi_crt_local_size(const nmod_poly_multi_crt_t CRT) { return CRT->localsize; }
...
NMOD_POLY_INLINE const mp_limb_t * nmod_berlekamp_massey_points(const nmod_berlekamp_massey_t B) { return B->points->coeffs; }

NMOD_POLY_INLINE slong nmod_berlekamp_massey_point_count(const nmod_berlekamp_massey_t B) { return B->points->length; }

NMOD_POLY_INLINE const nmod_poly_struct * nmod_berlekamp_massey_V_poly(const nmod_berlekamp_massey_t B) { return B->V1; }
NMOD_POLY_INLINE const nmod_poly_struct * nmod_berlekamp_massey_R_poly(const nmod_berlekamp_massey_t B) { return B->R1; }
