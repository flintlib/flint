...
TEST_FUNCTION_START(acb_ode_sum_divconquer, state)
{
    ...
    for (slong iter = 0; iter < 10 * flint_test_multiplier(); iter++)
    {
        ...
        slong dop_order = 2;
        acb_ode_sum_context_init(ctx, dop_order + 1,
                                 n_randint(state, 5),
                                 dop_order,
                                 1 + n_randint(state, 3));
        ...
        acb_ode_sum_divconquer(ctx, nterms);
        ...
        for (slong i = 0; i < ctx->npts; i++)
        {
            ...
            {
                if (!acb_poly_overlaps(ref + m, acb_ode_sol_sum_ptr(ctx->sol + m, i, 0)))
                {
                    flint_printf("FAIL\n\n");
                    flint_printf("i = %wd, m = %wd, a = %{acb}\n", i, m, ctx->pts + i);
                    for (slong j = 0; j < 2; j++)
                        flint_printf("sum[%wd] = %{acb_poly}\n", j,
                                     acb_ode_sol_sum_ptr(ctx->sol + m, i, 0));
