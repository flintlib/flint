...
TEST_FUNCTION_START(ca_poly_inv_series, state)
{
    slong iter;
    ...
    for (iter = 0; iter < 100 * 0.1 * flint_test_multiplier(); iter++)
    {
        ...
        {
            ca_poly_set(B, A, ctx);
            ca_poly_inv_series(B, B, n1, ctx);
        }
        else
        {
            ca_poly_inv_series(B, A, n1, ctx);
        }

        ca_poly_mullow(AB, A, B, n1, ctx);
        ca_poly_one(one, ctx);
        ca_poly_truncate(one, n1, ctx);

        if (!(A->length == 0 || ca_check_is_zero(A->coeffs, ctx) != T_FALSE))
        {
            if (ca_poly_check_equal(AB, one, ctx) == T_FALSE)
            {
                flint_printf("FAIL\n\n");
