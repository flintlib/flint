...
TEST_FUNCTION_START(nmod_poly_mulmid_classical, state)
{
    slong iter;
    ...
    for (iter = 0; iter < 200 * flint_test_multiplier(); iter++)
    {
        ...
        if (aliasing == 0)
        {
            nmod_poly_mulmid_classical(d, a, b, nlo, nhi);
        }
        else if (aliasing == 1)
        {
            nmod_poly_set(d, a);
            nmod_poly_mulmid_classical(d, d, b, nlo, nhi);
        }
        else if (aliasing == 2)
        {
            nmod_poly_set(d, b);
            nmod_poly_mulmid_classical(d, a, d, nlo, nhi);
        }
        else if (aliasing == 3)
        {
            nmod_poly_mulmid_classical(d, a, a, nlo, nhi);
        }
        else if (aliasing == 4)
        {
            nmod_poly_set(d, a);
            nmod_poly_mulmid_classical(d, d, d, nlo, nhi);
        }

        result = (nmod_poly_equal(c, d));
        if (!result)
        {
            flint_printf("FAIL: nmod_poly_mulmid_classical\n");
