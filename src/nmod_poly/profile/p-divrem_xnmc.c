...
void sample_generic(void * arg, ulong count)
{
    ...
    for (i = 0; i < (slong)count; i++)
    {
        ...
        qr = _nmod_vec_init(length);
        for (j = 0; j < length; j++)
            qr[j] = n_randint(state, modn);

        c = _get_c_from_ctype(info->ctype, state, modn);

        prof_start();
        _nmod_poly_divrem_xnmc(qr, qr, length, n, c, mod);
        prof_stop();
...
void sample_precomp(void * arg, ulong count)
{
    ...
    for (i = 0; i < (slong)count; i++)
    {
        ...
        _nmod_poly_divrem_xnmc_precomp(qr, qr, length, n, c, c_precomp, modn);
        prof_stop();
...
void sample_precomp_lazy(void * arg, ulong count)
{
    ...
    for (i = 0; i < (slong)count; i++)
    {
        ...
        prof_start();
        const ulong c_precomp = n_mulmod_precomp_shoup(c, modn);
        _nmod_poly_divrem_xnmc_precomp_lazy(qr, qr, length, n, c, c_precomp, modn);
        prof_stop();
