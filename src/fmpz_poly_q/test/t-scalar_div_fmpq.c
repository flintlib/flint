...
TEST_FUNCTION_START(fmpz_poly_q_scalar_div_fmpq, state)
{
    int i, result;
    ...
    for (i = 0; i < 10 * flint_test_multiplier(); i++)
    {
        ...
        fmpz_poly_q_scalar_div_fmpq(a, b, y);
        fmpz_poly_q_scalar_div_fmpq(b, b, y);
    ...
    for (i = 0; i < 10 * flint_test_multiplier(); i++)
    {
        ...
        fmpz_poly_q_scalar_div_fmpq(c, a, y);
        fmpz_poly_q_scalar_div_fmpq(d, b, y);
        fmpz_poly_q_add(d, c, d);

        fmpz_poly_q_add(c, a, b);
        fmpz_poly_q_scalar_div_fmpq(c, c, y);
