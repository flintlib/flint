...
gr_sparse_vec_randtest(gr_sparse_vec_t vec, double density, slong len, flint_rand_t state, gr_ctx_t ctx)
{
    ...
    if (nnz > vec->alloc)
    {
        vec->inds = flint_realloc(vec->inds, nnz*sizeof(slong));
    }
    for (i = 0; i < nnz; i++)
    {
        vec->inds[i] = n_randint(state, len);
    }
    qsort(vec->inds, nnz, sizeof(slong), int_sorter);
    /* Contract the col list to remove duplicates */
    j = 0;
    for (i = 0; i < nnz; i++)
    {
        if (i == 0 || vec->inds[j-1] != vec->inds[i])
        {
    ...
    /* Make alloc exactly the new nnz to get inds and entries back in sync */
    vec->inds = flint_realloc(vec->inds, nnz*sizeof(slong));
    if (vec->alloc > nnz)
    {
