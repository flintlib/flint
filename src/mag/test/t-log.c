...
TEST_FUNCTION_START(mag_log, state)
{
    ...
    for (iter = 0; iter < 100000 * 0.1 * flint_test_multiplier(); iter++)
    {
        ...
        else
        {
            ...
            {
                arf_log(z, x, 3 * MAG_BITS, ARF_RND_UP);
                arf_set_ui(z2, 2);
                arf_log(z2, z2, 3 * MAG_BITS, ARF_RND_UP);
                arf_addmul_fmpz(z, z2, n, 3 * MAG_BITS, ARF_RND_UP);
            }
        }
    ...
    for (iter = 0; iter < 100000 * 0.1 * flint_test_multiplier(); iter++)
    {
        ...
        else
        {
            ...
            {
                arf_log(z, x, 3 * MAG_BITS, ARF_RND_DOWN);
                arf_set_ui(z2, 2);
                arf_log(z2, z2, 3 * MAG_BITS, ARF_RND_DOWN);
                arf_addmul_fmpz(z, z2, n, 3 * MAG_BITS, ARF_RND_DOWN);
            }
        }
