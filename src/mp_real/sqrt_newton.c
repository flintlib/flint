...
void
_mp_real_rsqrt_ui_newton_basecase(nn_ptr Y, ulong a, slong n)
{
    nn_ptr U;
    slong nU;
    TMP_INIT;
...
void
_mp_real_rsqrt_newton_basecase(nn_ptr Q, nn_srcptr A, slong An, slong n)
{
    nn_ptr W, U, bq, br;
    nn_srcptr V;
...
/* Reciprocal square roots by third-order Newton steps: each step starts
   ...
   wrapped into a control limb (the construction of _mp_real_inv_newton);
...
#ifndef MP_REAL_RSQRT_UI_NEWTON_CUTOFF
#define MP_REAL_RSQRT_UI_NEWTON_CUTOFF 24
#endif

void
_mp_real_rsqrt_ui_newton(nn_ptr Y, ulong a, slong n)
{
    ...
    if (n <= MP_REAL_RSQRT_UI_NEWTON_CUTOFF)
    {
        _mp_real_rsqrt_ui_newton_basecase(Y, a, n);
        return;
    }
    ...
    Yhi = Y + n - m;
    _mp_real_rsqrt_ui_newton(Yhi, a, m);
...
#ifndef MP_REAL_RSQRT_NEWTON_CUTOFF
#define MP_REAL_RSQRT_NEWTON_CUTOFF 20
#endif

void
_mp_real_rsqrt_newton(nn_ptr Q, nn_srcptr A, slong An, slong n)
{
    ...
    if (n <= MP_REAL_RSQRT_NEWTON_CUTOFF)
    {
        _mp_real_rsqrt_newton_basecase(Q, A, An, n);
        return;
    }

    /* 3m >= n + 4: |u| < 8 B^-m, tail 160 B^-3m T far below B^-n T */
    m = (n + 4 + 2) / 3;
    _mp_real_rsqrt_newton(Q + n - m, A, An, m);
...
void
_mp_real_sqrt_newton_rsqrtmul(nn_ptr Q, nn_srcptr A, slong An, slong n)
{
    nn_ptr T;
    nn_srcptr A2;
    slong A2n;
    ...
    _mp_real_rsqrt_newton(Q, A, An, n);
...
#ifndef MP_REAL_SQRT_NEWTON_CUTOFF
#define MP_REAL_SQRT_NEWTON_CUTOFF 12
#endif

/* Karp-Markstein square root: S + T * (A - S^2) / 2 with S = T * A,
   T ~= 1/sqrt(A) carried at half precision (_mp_real_rsqrt_newton) */
void
_mp_real_sqrt_newton(nn_ptr Q, nn_srcptr A, slong An, slong n)
{
    ...
    if (n <= MP_REAL_SQRT_NEWTON_CUTOFF)
    {
        _mp_real_sqrt_newton_rsqrtmul(Q, A, An, n);
        return;
    }
