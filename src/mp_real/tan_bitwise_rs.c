...
/* R = floor-or-slightly-below of (2^(128n-1) - 1) / S for S with the
   ...
   runs _mp_real_inv_newton with one guard limb; its two-sided error
   ...
   starts. */
#ifndef MP_REAL_TRIG_RECIP_NEWTON_CUTOFF
#define MP_REAL_TRIG_RECIP_NEWTON_CUTOFF 40
#endif

static void
_mp_real_recip_bn(nn_ptr R, nn_ptr h, nn_srcptr N, nn_srcptr S, slong n)
{
    if (n < MP_REAL_TRIG_RECIP_NEWTON_CUTOFF)
    {
...
#define MP_REAL_TAN_ROTATE_NMAX 7

/* The tangent half-angle reconstruction.

   The angle is reduced exactly as for _mp_real_sin_cos_bitwise_rs: with the
...
void
_mp_real_tan_halfangle_mid(nn_ptr ysin, nn_ptr ycos, nn_ptr ytan,
    nn_srcptr x, slong n, int r, void (*series)(nn_ptr, nn_srcptr))
{
    ...
    mpn_rshift(t, x, n, 1);
    num = _mp_real_bitwise_reduce(t, n, r, 1, tab, nc, used);
    ...
    if (n <= MP_REAL_TAN_ROTATE_NMAX)
    {
        _mp_real_tan_rotate_tab[n](wx, wy, used, num);
    }
    else
    {
    ...
    if (series != NULL || (n >= MP_REAL_TAN_TAPERED_MIN
                            && (n <= MP_REAL_TAN_TAPERED_MAX
                                || (n <= MP_REAL_TAN_RS_MAX
                                    && _mp_real_series_rs_tan_ok(n, (flint_bitcnt_t) r)))))
    {
        /* u = tan(t'): the per-size series, or the chunked one of
           series_rs.c (common-denominator rectangular splitting with a
           Horner tail), which beats the sine and cosine series below
           by 1.1-1.45 times in the whole kernel up to 600 limbs */
        if (series != NULL)
            series(u, t);
        else
            _mp_real_series_rs_tan(u, t, n, (flint_bitcnt_t) r);
        /* the top n + 1 limbs of wx u and wy u: a mulhigh of the low
           n limbs (a lower bound within one ulp) plus u times the
           units limb */
        N[n] = 0;
        flint_mpn_mulhigh_n(N, wx, u, n);
        if (wx[n])
            N[n] = mpn_add_n(N, N, u, n);
        mpn_add_n(T, wy, N, wn);            /* TT (T[n] = 0) */
        N[n] = 0;
        flint_mpn_mulhigh_n(N, wy, u, n);
        if (wy[n])
            N[n] = mpn_add_n(N, N, u, n);
        mpn_sub_n(D, wx, N, wn);            /* DE */
    }
    else
    {
        /* sin t' and g = 1 - cos t' of the residual, factored out
           into _mp_real_sin_cos_reduced (which also owns the tuned
           series-vs-sqrt choice and adds the bit-burst modes for
           large n / r); r >= 32 here per _mp_real_tan_halfangle */
        _mp_real_sin_cos_reduced(ss, cc, NULL, t, n, (flint_bitcnt_t) r, 0);
...
/* smallest n at which each reduction parameter becomes optimal for
   the shared half-angle path of _mp_real_sin_cos_bitwise_rs and
   _mp_real_tan_bitwise_rs beyond the range of the tapered tangent
   series: interleaved timings of every ladder value at 17 sizes from
   84 to 1300 limbs (x86-64; the optimum flat to a few percent over
   one or two ladder steps).  The earlier table from
   src/mp_real/tune/tune-bitwise-r.c chose r one or two steps too
   large in places (2-9% slower). */
static const int _mp_real_trig_bitwise_rs_r_tab[] =
    {32, 64, 128, 192, 256, 320, 384, 448, 512};
static const short _mp_real_trig_bitwise_rs_n_tab[] =
    {1, 11, 40, 130, 330, 520, 700, 950, 1600};

int
_mp_real_trig_bitwise_rs_default_r(slong n)
{
    slong j;
    ...
    /* the tangent series of series_rs.c (common-denominator chunks,
       Horner tail up to 80 limbs; measured optima of the whole kernel,
       the timings flat within a few percent around these) */
    if (n <= MP_REAL_TAN_RS_MAX)
    {
        static const short tap_n[] = {13, 16, 24, 32, 48, 64, 80, 112, 144,
                                      176, 224, 480, 560};
        static const short tap_r[] = {36, 40, 48, 56, 72, 80, 96, 128, 160,
                                      192, 256, 320, 384};
        for (j = 0; j + 1 < (slong) (sizeof(tap_n) / sizeof(short))
                && n >= tap_n[j + 1]; j++)
            ;
        return tap_r[j];
    }
    ...
    for (j = 0; j + 1 < (slong) (sizeof(_mp_real_trig_bitwise_rs_n_tab)
            / sizeof(short))
            && n >= _mp_real_trig_bitwise_rs_n_tab[j + 1]; j++)
        ;
    return _mp_real_trig_bitwise_rs_r_tab[j];
}

int
_mp_real_tan_halfangle(nn_ptr ysin, nn_ptr ycos, nn_ptr ytan,
    nn_srcptr x, slong n, int r)
{
    ...
    /* fully specialized per-size implementations (trig_opt_<n>.c,
       emitted and tuned by dev/tune_mp_real.py) */
    if (r == 0 && n <= 12)
    {
        static void (* const tab[])(nn_ptr, nn_ptr, nn_ptr, nn_srcptr) = {
            NULL,
            _mp_real_trig_opt_1, _mp_real_trig_opt_2, _mp_real_trig_opt_3,
            _mp_real_trig_opt_4, _mp_real_trig_opt_5, _mp_real_trig_opt_6,
            _mp_real_trig_opt_7, _mp_real_trig_opt_8, _mp_real_trig_opt_9,
            _mp_real_trig_opt_10, _mp_real_trig_opt_11, _mp_real_trig_opt_12
        };
    ...
    if (r == 0)
        r = _mp_real_trig_bitwise_rs_default_r(n);
    r = FLINT_MAX(r, 32);
    ...
    _mp_real_tan_halfangle_mid(ysin, ycos, ytan, x, n, r, NULL);
    return 1;
}

FLINT_FORCE_INLINE void
_mp_real_tan_bitwise_rs_noerr(nn_ptr res, nn_srcptr x, slong n, int r)
{
    int ok;
    FLINT_ASSERT(n >= 1);
    ...
    ok = _mp_real_tan_halfangle(NULL, NULL, res, x, n, r);
    FLINT_ASSERT(ok);
    (void) ok;
}

void
_mp_real_tan_bitwise_rs(nn_ptr res, ulong * err, nn_srcptr x, slong n, int r)
{
    _mp_real_tan_bitwise_rs_noerr(res, x, n, r);
    if (err != NULL)
        *err = 8 * (r ? (ulong) r : (ulong) _mp_real_trig_bitwise_rs_default_r(n)) + 256;
}
