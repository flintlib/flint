...
/* _mp_real_neglog_newton and _mp_real_atan_newton agree with arb within
...
TEST_FUNCTION_START(mp_real_newton, state)
{
    slong iter;
    ...
    for (iter = 0; iter < 200 + 100 * flint_test_multiplier(); iter++)
    {
        ...
        if (which == 0)
        {
            ...
            arb_neg(r, r);
            _mp_real_neglog_newton_tune(y, &test_err_out, x, n, forward, N);
            bound = TEST_ERR(DOC_NEGLOG_NEWTON_MAX_ERR);
        }
        else
        {
            arb_atan(r, xa, FLINT_BITS * n + 64);
            _mp_real_atan_newton_tune(y, &test_err_out, x, n, forward, N);
            bound = TEST_ERR(DOC_ATAN_NEWTON_MAX_ERR);
        }
        ...
        if (!(u <= bound))
            TEST_FUNCTION_FAIL("%s: n = %wd, forward = %d, N = %wd: "
                "%.17g ulps (err %wu)\nx = %{ulong*}\ny = %{ulong*}\n",
                which ? "atan" : "neglog", n, forward, N, u, test_err_out, x, n, y, n);

        /* the public entries */
        if (which == 0)
            _mp_real_neglog_newton(y, &test_err_out, x, n);
        else
            _mp_real_atan_newton(y, &test_err_out, x, n);
        u = _newton_ulps(y, n, r);
        if (!(u <= bound))
        ...
        if (which == 0)
        {
            ...
            mp_real_init(rb);
            arb_init(ra);
            _mp_real_neglog_agm_tune(y, &test_err_out, x, n, N);
            u = _newton_ulps(y, n, r);
            if (!(u <= bound))
                TEST_FUNCTION_FAIL("neglog_agm: n = %wd, N = %wd: %g ulps\nx = %{ulong*}\n",
                    n, N, u, x, n);
            _mp_real_neglog_agm_ball(rb, x, n, N);
            mp_real_get_arb(ra, rb);
        ...
        /* the ball worker contains the truth */
        {
            ...
            arb_init(ra);
            if (which == 0)
                _mp_real_neglog_newton_ball(rb, x, n, forward, N);
            else
                _mp_real_atan_newton_ball(rb, x, n, forward, N);
            mp_real_get_arb(ra, rb);
            if (!arb_overlaps(ra, r))
