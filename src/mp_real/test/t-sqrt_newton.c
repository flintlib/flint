...
/* Check _mp_real_rsqrt_ui_newton, _mp_real_rsqrt_newton and
   _mp_real_sqrt_newton against basecase references carrying XTRA extra
...
TEST_FUNCTION_START(mp_real_sqrt_newton, state)
{
    slong iter;
    ...
    for (iter = 0; iter < 300 * flint_test_multiplier(); iter++)
    {
        ...
        _mp_real_rsqrt_newton_basecase(R, A, an, n + XTRA);

        if (!check(Q, n + 2, R, bound))
            TEST_FUNCTION_FAIL("rsqrt: n = %wd, an = %wd\n", n, an);

        _mp_real_sqrt_newton(Q, A, an, n);
        _mp_real_sqrt_newton_rsqrtmul(R, A, an, n + XTRA);
        ...
        _mp_real_rsqrt_ui_newton(Q, a1, n);
        _mp_real_rsqrt_ui_newton_basecase(R, a1, n + XTRA);
