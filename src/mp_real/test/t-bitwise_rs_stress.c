...
TEST_FUNCTION_START(mp_real_bitwise_rs_stress, state)
{
    slong iter;
    ...
    for (iter = 0; iter < 200 * flint_test_multiplier(); iter++)
    {
        ...
        _mp_real_exp_logs_ensure(n, FLINT_MIN((slong) reff + 2, imax));
        ...
        for (j = 0; j < nsum; j++)
        {
            slong i = 1 + n_randint(state, FLINT_MIN((slong) reff + 2,
                imax));
            nn_srcptr Li = _mp_real_exp_logs_entry(i, n);
        ...
        if (iter % 5 == 0 && reff <= imax)
        {
            ...
            nn_srcptr Lr = _mp_real_exp_logs_entry(
                FLINT_MIN((slong) reff, _mp_real_exp_logs_max_index()), n);
        ...
        _mp_real_exp_bitwise_rs(res, &test_err_out, x, n, r);
        ...
        if (u > TEST_ERR(DOC_EXP_BITWISE_RS_MAX_ERR(n, r)))
           ...
           attempts a duplicated factor at the reduction limit) ---- */
        {
            ...
            _mp_real_log1p_bitwise_rs(res, &test_err_out, P, n, rl);
            ...
            if (u > TEST_ERR(DOC_LOG1P_BITWISE_RS_MAX_ERR(n, rl)))
        ...
        if (FLINT_BITS == 64 || n >= 2)
        {
            ...
            _mp_real_atans_ensure(n, FLINT_MIN((slong) rteff + 2, imax));
            ...
            for (j = 0; j < nsum; j++)
            {
                ...
                nn_srcptr Ai = _mp_real_atans_entry(i, n);
            ...
            _mp_real_sin_cos_bitwise_rs(ys, yc, &test_err_out, x, n, rt);
            ...
            if (u > TEST_ERR(DOC_SIN_COS_BITWISE_RS_MAX_ERR(n, rt)))
           ...
           must rediscover the factor set ---- */
        {
            ...
            /* t = floor(WY 2^(b n) / WX): WY < WX so t < 1.  In
               ...
               (The vectoring loop inside _mp_real_atan_bitwise_rs has
            ...
            _mp_real_atan_bitwise_rs(ya, &test_err_out, x, n, rt);
            ...
            if (u > TEST_ERR(DOC_ATAN_BITWISE_RS_MAX_ERR(n, rt)))
       ...
       thread-exit path over freshly rebuilt tables as well */
    {
        ...
        if (_mp_real_atans_max_index() < 1)
            TEST_FUNCTION_FAIL("angle table not warmed\n");

        _mp_real_exp_logs_clear();
        ...
        _mp_real_exp_bitwise_rs(y, &test_err_out, x, n, 64);
        fmpz_set_ui_array(f, x, n);
        arb_set_fmpz(xa, f);
        arb_mul_2exp_si(xa, xa, -FLINT_BITS * n);
        arb_exp(exact, xa, prec);
        u = ulp_error_vs_arb(y, n + 1, exact, n);
        if (u > TEST_ERR(DOC_EXP_BITWISE_RS_MAX_ERR(n, 64)))
            TEST_FUNCTION_FAIL("exp after cleanup: ulp = %f\n", u);

        _mp_real_tan_bitwise_rs(y, &test_err_out, x, n, 64);
        arb_tan(exact, xa, prec);
        u = ulp_error_vs_arb(y, n + 1, exact, n);
        if (u > TEST_ERR(DOC_TAN_BITWISE_RS_MAX_ERR(n, 64)))
