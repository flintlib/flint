...
TEST_FUNCTION_START(mp_real_tab_floor, state)
{
    slong iter;
    ...
    for (iter = 0; iter < 2 + flint_test_multiplier() / 3; iter++)
    {
        ...
        for (i = 0; i <= rc; i++)
        {
            _mp_real_tab_entry_exact(exact, 0, (ulong) i, nc);
            if (mpn_cmp(exact, _mp_real_exp_logs_entry(i, nc), nc) != 0)
        ...
        for (i = 0; i <= rc; i++)
        {
            _mp_real_tab_entry_exact(exact, 1, (ulong) i, nc);
            if (mpn_cmp(exact, _mp_real_atans_entry(i, nc), nc) != 0)
