...
/* every algorithm of _mp_real_sin_cos_reduced agrees with arb within
    ...
    flint_rand_t state)
{
    ...
    arb_set_d(bnd, TEST_ERR(DOC_SIN_COS_REDUCED_MAX_ERR));
...
TEST_FUNCTION_START(mp_real_sin_cos_reduced, state)
{
    slong iter;
    ...
    for (iter = 0; iter < 20 + 20 * flint_test_multiplier(); iter++)
    {
        ...
        for (alg = 0; alg <= 4; alg++)
        {
            if (alg == 1 || alg == 2)
                if (r < 32)
                    continue;
            _mp_real_sin_cos_reduced(ysin, yg, &test_err_out, t, wn, r, (int) alg);
    ...
    /* targeted regimes that random sampling misses */
    {
            ...
            c++)
        {
            ...
            _mp_real_sin_cos_reduced(ysin, yg, &test_err_out, t, wn, r, cases[c].alg);
            if (_check(ysin, yg, t, wn, state) != 1)
