...
/* Check _mp_real_inv_newton and _mp_real_div_newton against basecase
...
TEST_FUNCTION_START(mp_real_div_newton, state)
{
    slong iter;
    ...
    for (iter = 0; iter < 300 * flint_test_multiplier(); iter++)
    {
        ...
        _mp_real_inv_newton_basecase(R, A, an, n + XTRA);

        if (!check(Q, n + 2, R, bound))
            TEST_FUNCTION_FAIL("inv: n = %wd, an = %wd\n", n, an);

        _mp_real_div_newton(Q, A, bn, A, an, n);
        _mp_real_div_newton_invmul(R, A, bn, A, an, n + XTRA);
