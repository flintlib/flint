...
/* _mp_real_sin_cos_diophantine agrees with arb within its budget for x
...
TEST_FUNCTION_START(mp_real_sin_cos_diophantine, state)
{
    slong iter;
    ...
    for (iter = 0; iter < 20 + 5 * flint_test_multiplier(); iter++)
    {
        ...
        _mp_real_atan_gauss_ensure(num, nv);

        arb_init(l); fmpz_init(f); fmpz_init(g); fmpq_init(q);
        fmpq_set_si(q, _mp_real_gaussian_primes[2 * j + 1],
            _mp_real_gaussian_primes[2 * j]);
        ...
        if (!arb_get_unique_fmpz(f, l))
            TEST_FUNCTION_FAIL("floor not determined: j = %wd, nv = %wd\n",
                j, nv);
        fmpz_set_ui_array(g, _mp_real_atan_gauss_entry(j, nv), nv + 1);
        if (!fmpz_equal(f, g))
    ...
    for (iter = 0; iter < 60 + 60 * flint_test_multiplier(); iter++)
    {
        ...
        _mp_real_sin_cos_diophantine(ys, yc, &test_err_out, x, n);
        u = FLINT_MAX(_ulps_sc(ys, n + 1, s, n), _ulps_sc(yc, n + 1, c, n));
        if (u > TEST_ERR(DOC_SIN_COS_DIOPHANTINE_MAX_ERR))
            TEST_FUNCTION_FAIL("n = %wd, zz = %wd, ulp = %f\n", n, zz, u);

        /* one output only, and the tangent */
        _mp_real_sin_cos_diophantine(ys, NULL, &test_err_out, x, n);
        _mp_real_sin_cos_diophantine(NULL, yc, &test_err_out, x, n);
        u = FLINT_MAX(_ulps_sc(ys, n + 1, s, n), _ulps_sc(yc, n + 1, c, n));
        if (u > TEST_ERR(DOC_SIN_COS_DIOPHANTINE_MAX_ERR))
            TEST_FUNCTION_FAIL("single: n = %wd, zz = %wd, ulp = %f\n", n, zz, u);
        {
            ...
            arb_init(ta);
            arb_tan(ta, xa, prec);
            _mp_real_tan_diophantine(ys, &test_err_out, x, n);
            u = _ulps_sc(ys, n + 1, ta, n);
            if (u > TEST_ERR(DOC_SIN_COS_DIOPHANTINE_MAX_ERR))
                TEST_FUNCTION_FAIL("tan: n = %wd, zz = %wd, ulp = %f\n", n, zz, u);
            if (n_randint(state, 2))
            {
                _mp_real_tan_diophantine_tune(ys, &test_err_out, x, n, 2 + n_randint(state, 19),
        ...
        for (k = 0; k < 3; k++)
        {
            ...
            _mp_real_sin_cos_diophantine_tune(ys, yc, &test_err_out, x, n, num, w);
            u = FLINT_MAX(_ulps_sc(ys, n + 1, s, n), _ulps_sc(yc, n + 1, c, n));
            if (u > TEST_ERR(DOC_SIN_COS_DIOPHANTINE_MAX_ERR))
