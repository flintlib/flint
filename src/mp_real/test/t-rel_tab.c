...
TEST_FUNCTION_START(mp_real_rel_tab, state)
{
    slong i, g, j, k;
    slong prec = 800;
    ...
    for (g = 0; g < 2; g++)
    {
        ...
        {
            const mp_real_rel_static_struct * st = _mp_real_rel_static + i;
            const mp_real_rel_struct * t;
            arb_ptr alpha;

            if (st->gaussian != g)
                continue;

            if (!_mp_real_rel_table_is_cached(g, st->num))
                TEST_FUNCTION_FAIL("precomputed table not reported cached: g = %wd, num = %wd\n", g, st->num);
            t = _mp_real_rel_table(g, st->num);
            if (t->rows != st->rows || t->d != st->d || !t->is_static)
                TEST_FUNCTION_FAIL("table mismatch: g = %wd, num = %wd\n", g, st->num);

            alpha = _arb_vec_init(st->num);
            if (g)
                _mp_real_atan_gauss_vec_arb(alpha, st->num, prec);
            else
        ...
        /* a generated table */
        {
            ...
            const mp_real_rel_struct * t;
            arb_ptr alpha;
            int was = _mp_real_rel_table_is_cached(g, num);

            t = _mp_real_rel_table(g, num);
            if (!_mp_real_rel_table_is_cached(g, num) || t->num != num)
            ...
            if (g)
                _mp_real_atan_gauss_vec_arb(alpha, num, prec);
            else
    ...
    {
        slong num;
        for (num = 2; num <= _mp_real_machin_table_max(g); num++)
        {
            const mp_real_machin_struct * mt = _mp_real_machin_table(g, num);
            arb_ptr y;
            fmpz_t p, q;
            ...
            for (i = 0; i < mt->num; i++)
            {
                ...
                arb_init(s); arb_init(r);
                {
                    ...
                    for (j = 0; j < mt->num; j++)
                    {
                        _mp_real_machin_get_c(cc, mt, i, j);
                ...
                else
                {
                    fmpq_t fr;
                    fmpq_init(fr);
                    fmpq_set_si(fr, _mp_real_gaussian_primes[2 * i + 1], _mp_real_gaussian_primes[2 * i]);
