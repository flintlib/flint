...
static void
_set_int(mp_real_hypgeom_int_struct * x, nn_ptr * d, const fmpz_t v)
{
    fmpz_t a;
    slong n = fmpz_size(v);
    ...
    const fmpz * Q, slong Qlen, const fmpz * R, slong Rlen)
{
    ...
    t->c = flint_malloc(tot * sizeof(mp_real_hypgeom_int_struct));
...
TEST_FUNCTION_START(mp_real_hypgeom_series, state)
{
    slong iter;
    ...
    for (iter = 0; iter < 30 * flint_test_multiplier(); iter++)
    {
        ...
        {
            /* pi = S^-1 / sqrt(10005) */
            mp_real_hypgeom_series_int64(x, -1, 1, 13591409,
        ...
        {
            mp_real_hypgeom_series_int64(x, 1, 1, 1497, 2160, l2P, 4, l2Q, 3,
                l2R, 3, n);
