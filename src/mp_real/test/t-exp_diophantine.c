...
/* _mp_real_exp_diophantine agrees with arb within its budget for x
...
TEST_FUNCTION_START(mp_real_exp_diophantine, state)
{
    slong iter;
    ...
    for (iter = 0; iter < 20 + 5 * flint_test_multiplier(); iter++)
    {
        ...
        _mp_real_log_primes_ensure(num, nv);
        ...
        if (!arb_get_unique_fmpz(f, l))
            TEST_FUNCTION_FAIL("floor not determined: j = %wd, nv = %wd\n",
                j, nv);
        fmpz_set_ui_array(g, _mp_real_log_primes_entry(j, nv), nv + 1);
        if (!fmpz_equal(f, g))
    ...
    for (iter = 0; iter < 60 + 60 * flint_test_multiplier(); iter++)
    {
        ...
        _mp_real_exp_diophantine(y, &test_err_out, x, n);
        u = _ulps_mp(y, n + 1, e, n);
        if (u > TEST_ERR(DOC_EXP_DIOPHANTINE_MAX_ERR))
        ...
        for (k = 0; k < 3; k++)
        {
            ...
            _mp_real_exp_diophantine_tune(y, &test_err_out, x, n, num, w);
            u = _ulps_mp(y, n + 1, e, n);
            if (u > TEST_ERR(DOC_EXP_DIOPHANTINE_MAX_ERR))
    ...
    /* one deep case: the bit-burst regime inside _mp_real_exp_reduced
