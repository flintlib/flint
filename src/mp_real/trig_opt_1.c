...
/* GENERATED by dev/tune_mp_real.py (trig, n = 1, r = 4) -- do not
   edit directly; retune or re-emit with
       python3 dev/tune_mp_real.py trig 1 --pin 4 --emit */
...
static void
_mp_real_tan_series_1(nn_ptr res, nn_srcptr x)
{
...
void
_mp_real_trig_opt_1(nn_ptr ysin, nn_ptr ycos, nn_ptr ytan,
    nn_srcptr x)
{
    slong num, nc;
    const int r = 4;
    ...
    nn_srcptr tab;
    tab = _mp_real_atans_tab(1, r, &nc);

    TMP_START;
    used = TMP_ALLOC(MP_REAL_BITWISE_REDUCE_USED_ALLOC(r) * sizeof(slong));

    t[0] = x[0] >> 1;
    num = _mp_real_bitwise_reduce(t, 1, r, 1, tab, nc, used);
    _mp_real_tan_rotate_tab[1](wx, wy, used, num);

    _mp_real_tan_series_1(t, t);          /* u = tan(t') */
    U = t[0];
...
void
_mp_real_sin_cos_opt_1(nn_ptr ysin, nn_ptr ycos, nn_srcptr x)
{
    _mp_real_trig_opt_1(ysin, ycos, NULL, x);
}

void
_mp_real_tan_opt_1(nn_ptr res, nn_srcptr x)
{
    _mp_real_trig_opt_1(NULL, NULL, res, x);
}
