...
/* _mp_real_exp_notab: exp(x) for any x in [0, 1) without table-based
   ...
   the guard limbs); _mp_real_exp_reduced runs on t = x 2^-h at the
   ...
   where the automatic dispatch inside _mp_real_exp_reduced switches to
   ...
   forced-depth worker below and profile/p-mp_real. */

static const short _mp_real_exp_notab_n_tab[] = { 40, 80, 150, 300 };
static const char _mp_real_exp_notab_r_tab[] = { 32, 16, 32, 16, 32 };

FLINT_FORCE_INLINE void
_mp_real_exp_notab_r_noerr(nn_ptr y, nn_srcptr x, slong n, int r)
{
    ...
    if (h == 0)
    {
        ...
        _mp_real_exp_reduced(y, NULL, x, n, (flint_bitcnt_t) z, 0);
        return;
    }
    ...
    _mp_real_exp_reduced(Y, NULL, t, wn, (flint_bitcnt_t) (z + h), 0);
...
void
_mp_real_exp_notab_r(nn_ptr y, ulong * err, nn_srcptr x, slong n, int r)
{
    _mp_real_exp_notab_r_noerr(y, x, n, r);
    if (err != NULL)
        *err = 128;
}

FLINT_FORCE_INLINE void
_mp_real_exp_notab_noerr(nn_ptr y, nn_srcptr x, slong n)
{
    slong j;
    int r = _mp_real_exp_notab_r_tab[
        sizeof(_mp_real_exp_notab_n_tab)
            / sizeof(_mp_real_exp_notab_n_tab[0])];

    for (j = 0; j < (slong) (sizeof(_mp_real_exp_notab_n_tab)
        / sizeof(_mp_real_exp_notab_n_tab[0])); j++)
    {
        if (n <= _mp_real_exp_notab_n_tab[j])
        {
            r = _mp_real_exp_notab_r_tab[j];
            break;
        }
    }
    _mp_real_exp_notab_r(y, NULL, x, n, r);
}

void
_mp_real_exp_notab(nn_ptr y, ulong * err, nn_srcptr x, slong n)
{
    _mp_real_exp_notab_noerr(y, x, n);
    if (err != NULL)
        *err = 128;
}
