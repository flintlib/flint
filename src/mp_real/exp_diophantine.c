...
/* exp by diophantine (multi-prime) argument reduction (port of
   ...
   THE REDUCTION (_mp_real_log_reduce) is a greedy descent through a
   ...
   sign, but _mp_real_exp_reduced takes t >= 0.  Rather than paying for
   ...
   PRIME POWERS (_mp_real_prime_powers).  p and q are built by vector
   ...
   size; _mp_real_exp_reduced (or _mp_real_exp_notab when the residual is
   ...
   arb_exp_arf_log_reduction.  Against _mp_real_exp_bitwise_rs it is
...
void
_mp_real_log_dot(nn_ptr acc, nn_srcptr base, slong len, const slong * rel,
...
double
_mp_real_signed_get_d(nn_srcptr a, slong len, nn_ptr tmp)
{
...
slong
_mp_real_log_reduce(slong * rel, const mp_real_rel_struct * tab,
    ...
    nn_srcptr alpha, slong stride)
{
    ...
    dx = _mp_real_signed_get_d(base, wr + 1, tmp);

    /* reduce by the first alpha, which is assumed to be free */
    dalpha = _mp_real_signed_get_d(alpha, wr + 1, tmp);
    ...
    if (FLINT_ABS(n) > 10)
    {
        _mp_real_log_dot(acc, base, wr + 1, rel, num, alpha, stride);
        dx = _mp_real_signed_get_d(acc, wr + 1, tmp);
    }
...
static slong
_mp_real_prime_powers(nn_ptr res, nn_ptr tmp, slong * e_out,
...
_mp_real_exp_diophantine_tune_noerr(nn_ptr y, nn_srcptr x, slong n, slong num,
    double max_weight)
{
    ...
    _mp_real_log_primes_ensure(num, FLINT_MAX(wr, n + 1));
    ...
    _mp_real_log_primes_ensure(num, wn);   /* a no-op unless G > 1 */
    ...
    _mp_real_log_dot(t, tmp, wn + 1, rel, num,
        _mp_real_log_primes_entry(0, wn), _mp_real_log_primes_n);
    ...
    else
    {
        ...
        if (r >= 16)
            _mp_real_exp_reduced(E, NULL, t, wn, (flint_bitcnt_t) r, 0);
        else
            _mp_real_exp_notab(E, NULL, t, wn);
    }
    ...
    for (j = 0; j < num; j++)
        neg[j] = -rel[j];
    pn = _mp_real_prime_powers(p, p + (2 * cap + 4), &ep, tab->primes + 1,
        rel + 1, num - 1, cap);
    qn = _mp_real_prime_powers(q, q + (2 * cap + 4), &eq, tab->primes + 1,
        neg + 1, num - 1, cap);
    ...
    /* exp(x) B^wn = (Nm / qm) B^(eN - eq): shift the numerator by
       ...
       _mp_real_div_newton (2.4-2.9 M(n) with fft_small at every size
       ...
       quotient-only flint_mpn_tdiv_q, which is ahead there. */
    {
        ...
        {
            /* _mp_real_div_newton on the fractions num / B^numn and
            ...
            _mp_real_div_newton(Q, num, numn, q, qn, wn + 3);
            Qn = wn + 5;
            G2 = G + 3 + k;
        }
        else
...
void
_mp_real_exp_diophantine_tune(nn_ptr y, ulong * err, nn_srcptr x, slong n, slong num,
    double max_weight)
{
    _mp_real_exp_diophantine_tune_noerr(y, x, n, num, max_weight);
    if (err != NULL)
        *err = 3;
}

FLINT_FORCE_INLINE void
_mp_real_exp_diophantine_noerr(nn_ptr y, nn_srcptr x, slong n)
{
    _mp_real_exp_diophantine_tune(y, NULL, x, n, 13, (double) (FLINT_BITS * n));
}

void
_mp_real_exp_diophantine(nn_ptr y, ulong * err, nn_srcptr x, slong n)
{
    _mp_real_exp_diophantine_noerr(y, x, n);
    if (err != NULL)
        *err = 3;
}
