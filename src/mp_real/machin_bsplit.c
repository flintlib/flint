...
/* The logarithms of the first num primes and the angles 2 arg(pi_j) of
   the first num nonreal Gaussian primes as mp_real_t balls, from the
...
static void
_zuniga_atanh_inv(mp_real_t res, nn_srcptr x, slong xn, slong n)
{
    fmpz_t u, v;
    ...
    _mp_real_log_ratio_zuniga(res, u, v, n);
...
static void
_atanh_inv(mp_real_t res, nn_srcptr x, slong xn, slong n)
{
    ulong one = 1;
...
static void
_set_static(mp_real_t x, nn_srcptr tab, slong e)
{
...
/* res_i = (1/den) sum_j c_ij y_j for i < min(num, tab->num), the y_j
   ...
   error bounds e_j (_mp_real_get_fixed); the dot products are exact mpn
   ...
   freed (left zero). */
static void
_machin_combine(mp_real_struct * res, slong num, mp_real_struct * y,
    const mp_real_machin_struct * tab, slong wn)
{
    ...
    for (j = 0; j < ln; j++)
    {
        {
            ulong e;
            _mp_real_get_fixed(Y + j * wn, &e, y + j, wn);
            ey[j] = (double) e;
        }
        mp_real_clear(y + j);
        mp_real_init(y + j);
    ...
    for (i = 0; i < FLINT_MIN(num, ln); i++)
    {
        ...
        nn_srcptr crow = _mp_real_machin_c_row_raw(tab, i, &climbs, &csign);
        double err = 0.0;
        ...
        if (mpn_cmp(pos, neg, an) >= 0)
        {
            ...
            _mp_real_set_mpn_2exp(res + i, pos, an, -FLINT_BITS * wn);
        }
        else
        {
            mpn_sub_n(pos, neg, pos, an);
            _mp_real_set_mpn_2exp(res + i, pos, an, -FLINT_BITS * wn);
            mp_real_neg(res + i, res + i);
        }
        _mp_real_add_error_ulps_at(res + i, err * (1.0 + 1e-10) + 1.0, -wn);

        if (tab->den != 1)
        {
            mp_real_div(t, res + i, d, wn);
            mp_real_swap(res + i, t);
        }
    }
...
static void
_machin_job(machin_job_t * J, mp_real_struct * res, nn_srcptr x, slong xn,
    ulong p, int kind)
{
    double lx;
...
_machin_run(machin_job_t * jobs, slong num, slong n)
{
    ...
    w.jobs = jobs;
    w.n = n;
    _mp_real_parallel_tasks(_machin_worker, &w, num);
}
...
{
    slong j;
    mp_real_struct * y = flint_malloc(num * sizeof(mp_real_struct));
    for (j = 0; j < num; j++)
        mp_real_init(y + j);
    return y;
}
...
static void
_machin_series_clear(mp_real_struct * y, slong num)
{
    slong j;
    for (j = 0; j < num; j++)
        mp_real_clear(y + j);
...
static slong
_machin_guard(const mp_real_machin_struct * tab)
{
...
void
_mp_real_log_primes_vec(mp_real_struct * res, slong num, slong n)
{
    ...
    tab = _mp_real_machin_table(0, num);
    ln = tab->num;
    ...
    for (j = 0; j < ln; j++)
        _machin_job(jobs + j, y + j, tab->x + j * MP_REAL_MACHIN_X_LIMBS,
            MP_REAL_MACHIN_X_LIMBS, 1, 0);
    for (i = ln; i < num; i++)
    {
    ...
    for (i = ln; i < num; i++)
    {
        ...
        mp_real_mul_2exp_si(res + i, res + i, 1);
        ...
        for (side = 0; side < 2; side++)
        {
            ...
            for (j = 0; j < fac.num; j++)
                for (k = 0; k < i; k++)
                    if (fac.p[j] == primes[k])
                    {
                        mp_real_addmul_ui(res + i, res + i, res + k, fac.exp[j], wp);
                    }
        }

        mp_real_mul_2exp_si(res + i, res + i, -1);
        mp_real_add(res + i, res + i, res + 0, wp);
    }
    mp_real_clear(t);
    flint_free(primes);
}
...
void
_mp_real_atan_gauss_vec(mp_real_struct * res, slong num, slong n)
{
    const mp_real_machin_struct * tab;
    mp_real_struct * y;
    slong i, j, ln, wp;
    ...
    tab = _mp_real_machin_table(1, num);
    ln = tab->num;
    wp = n + _machin_guard(tab);
    ...
    for (i = ln; i < num; i++)
    {
        ...
        if (f[1] < 0)
            mp_real_neg(res + i, res + i);
        mp_real_add(res + i, res + i, res + f[0], wp);
    }
    flint_free(fol);
