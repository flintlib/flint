...
/* _mp_real_sin_cos_notab: sin(x) and cos(x) for any x in [0, 1)
   without table-based argument reduction.  As in _mp_real_exp_notab
   the argument is halved h = max(0, r(n) - z) times (z the leading
   zero bits of x, the shift exact within the guard limbs) and
   _mp_real_sin_cos_reduced runs at depth z + h; the angle is then
   ...
   through flint_mpn_sqrtrem at small sizes and _mp_real_sqrt_newton above
...
_mp_real_sin_cos_notab_r_noerr(nn_ptr ysin, nn_ptr ycos, nn_srcptr x,
    slong n, int r)
{
    ...
    if (h == 0)
    {
        ...
        _mp_real_sin_cos_reduced(sg, sg + (n + 1), NULL, x, n,
            (flint_bitcnt_t) z, 0);
    ...
    _mp_real_sin_cos_reduced(s, g, NULL, t, wn, (flint_bitcnt_t) (z + h), 0);
    ...
    else if (wn < MP_REAL_SIN_COS_NOTAB_SQRT_NEWTON_CUTOFF)
    {
    ...
    else
    {
        /* Newton at pure limb granularity: take w's top limbs at a
           ...
           _mp_real_sqrt_newton's accepted range; then
       sin B^wn = sqrt(vhat) B^((vh + wn) / 2) and the placement is
        ...
        _mp_real_sqrt_newton(rt, vf, nin, wn + 1);
...
void
_mp_real_sin_cos_notab_r(nn_ptr ysin, nn_ptr ycos, ulong * err, nn_srcptr x,
    slong n, int r)
{
    _mp_real_sin_cos_notab_r_noerr(ysin, ycos, x, n, r);
    if (err != NULL)
        *err = 128;
}

FLINT_FORCE_INLINE void
_mp_real_sin_cos_notab_noerr(nn_ptr ysin, nn_ptr ycos, nn_srcptr x, slong n)
{
    _mp_real_sin_cos_notab_r(ysin, ycos, NULL, x, n,
        (n <= MP_REAL_SIN_COS_NOTAB_BURST_CUTOFF) ? 32 : 24);
}

void
_mp_real_sin_cos_notab(nn_ptr ysin, nn_ptr ycos, ulong * err, nn_srcptr x, slong n)
{
    _mp_real_sin_cos_notab_noerr(ysin, ycos, x, n);
    if (err != NULL)
        *err = 128;
}
