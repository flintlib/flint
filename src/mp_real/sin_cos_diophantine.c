...
/* sin and cos by diophantine (multi-prime) argument reduction (the trigonometric
   ...
   _mp_real_log_reduce over the relation table (rel_tab.c) picks
   integers c_j with
   ...
   (sin |t|, 1 - cos |t|) come from _mp_real_sin_cos_reduced.  A is
   ...
   output ulp.  the returned bound *err = 4.

   MEASURED (development VM, fft_small).  The reduced sine and cosine
   are 90% of the time: at 2677 limbs _mp_real_sin_cos_reduced costs
   11.8 ms at r = 105 against 7.5 ms for _mp_real_exp_reduced, and the
   ...
   evaluations.  The alg choice of _mp_real_sin_cos_reduced at r ~ 100
...
static void
_mp_real_gauss_powers(nn_ptr zr, slong * zrn, nn_ptr zi, slong * zin,
...
/* tan = |zi| / |zr| from the two fixed-point parts (same scale, wn
   ...
   needs), and _mp_real_div_newton divides the two fractions; when the
    ...
    slong zrn, slong wn)
{
    ...
    if (a[m - 1] != 0)
        _mp_real_div_newton(q, b, m, a, m, wn + 2);       /* q = tan */
    else
        _mp_real_div_newton(q, b, m, a, m - 1, wn + 3);   /* q = tan / B */
...
/* C = floor(|zr| B^sh / N), S = floor(|zi| B^sh / N) for both parts
   at once (sh as in _div_by_N): one reciprocal of N (_mp_real_inv_newton
    ...
    slong zin, slong sh, nn_srcptr N, slong Nn, slong wn)
{
    ...
    _mp_real_inv_newton(q, Nw, m, m);      /* q_int = B^(2m) / Nw_int, m + 2 limbs */
...
static void
_mp_real_trig_diophantine(nn_ptr ysin, nn_ptr ycos, nn_ptr ytan, nn_srcptr x,
    slong n, slong num, double max_weight)
{
    const mp_real_rel_struct * tab;
    slong * rel;
    ...
    FLINT_ASSERT(num >= 2 && num <= MP_REAL_ATAN_GAUSS_MAX);
    ...
    _mp_real_atan_gauss_ensure(num, FLINT_MAX(wr, n + 1));
    ...
    _mp_real_atan_gauss_ensure(num, wn);
    ...
    tmp[wn] = 0;
    _mp_real_log_dot(t, tmp, wn + 1, rel, num,
        _mp_real_atan_gauss_entry(0, wn), _mp_real_atan_gauss_n);
    ...
    Ai = Ar + (2 * cap + 4);
    _mp_real_gauss_powers(Ar, &arn, Ai, &ain, &eA, _mp_real_gaussian_primes + 2,
        rel + 1, num - 1, cap);
...
_mp_real_sin_cos_diophantine_tune_noerr(nn_ptr ysin, nn_ptr ycos, nn_srcptr x,
    slong n, slong num, double max_weight)
{
    _mp_real_trig_diophantine(ysin, ycos, NULL, x, n, num, max_weight);
}

void
_mp_real_sin_cos_diophantine_tune(nn_ptr ysin, nn_ptr ycos, ulong * err, nn_srcptr x,
    slong n, slong num, double max_weight)
{
    _mp_real_sin_cos_diophantine_tune_noerr(ysin, ycos, x, n, num, max_weight);
    if (err != NULL)
        *err = 4;
}
...
/* 32 primes at four times the precision as weight: the reduced sine
   ...
   tuning notes above) */
#define MP_REAL_TRIG_DIOPHANTINE_NUM 32
#define MP_REAL_TRIG_DIOPHANTINE_WEIGHT 4.0

FLINT_FORCE_INLINE void
_mp_real_sin_cos_diophantine_noerr(nn_ptr ysin, nn_ptr ycos, nn_srcptr x, slong n)
{
    _mp_real_trig_diophantine(ysin, ycos, NULL, x, n, MP_REAL_TRIG_DIOPHANTINE_NUM,
        MP_REAL_TRIG_DIOPHANTINE_WEIGHT * (double) (FLINT_BITS * n));
}

void
_mp_real_sin_cos_diophantine(nn_ptr ysin, nn_ptr ycos, ulong * err, nn_srcptr x, slong n)
{
    _mp_real_sin_cos_diophantine_noerr(ysin, ycos, x, n);
    if (err != NULL)
        *err = 4;
}

FLINT_FORCE_INLINE void
_mp_real_tan_diophantine_noerr(nn_ptr res, nn_srcptr x, slong n)
{
    _mp_real_trig_diophantine(NULL, NULL, res, x, n, MP_REAL_TRIG_DIOPHANTINE_NUM,
        MP_REAL_TRIG_DIOPHANTINE_WEIGHT * (double) (FLINT_BITS * n));
}

void
_mp_real_tan_diophantine(nn_ptr res, ulong * err, nn_srcptr x, slong n)
{
    _mp_real_tan_diophantine_noerr(res, x, n);
    if (err != NULL)
        *err = 4;
}

FLINT_FORCE_INLINE void
_mp_real_tan_diophantine_tune_noerr(nn_ptr res, nn_srcptr x, slong n, slong num,
    double max_weight)
{
    _mp_real_trig_diophantine(NULL, NULL, res, x, n, num, max_weight);
}

void
_mp_real_tan_diophantine_tune(nn_ptr res, ulong * err, nn_srcptr x, slong n, slong num,
    double max_weight)
{
    _mp_real_tan_diophantine_tune_noerr(res, x, n, num, max_weight);
    if (err != NULL)
        *err = 4;
}
