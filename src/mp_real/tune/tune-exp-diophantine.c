...
/* Tune _mp_real_exp_diophantine: for a size n, sweep the number of
...
precomp_diophantine(slong num, slong n)
{
    ...
    t0 = wall_us();
    _mp_real_log_primes_ensure(num, n + 1);
    t1 = wall_us();
    return t1 - t0;
}
...
precomp_bitwise(slong n)
{
    ...
    t0 = wall_us();
    _mp_real_exp_bitwise_rs(y, NULL, x, n, 0);
    t1 = wall_us();
    flint_free(x);
    flint_free(y);
    return t1 - t0;
...
static double
time_call(void (*f)(nn_ptr, ulong *, nn_srcptr, slong, slong, double),
...
static void
f_bitwise(nn_ptr y, ulong * err, nn_srcptr x, slong n, slong num, double w)
{
    (void) num; (void) w;
    _mp_real_exp_bitwise_rs(y, err, x, n, 0);
}
...
{
    const mp_real_rel_struct * tab = _mp_real_rel_table(0, num);
    slong wr, j;
    slong * rel;
    nn_ptr base, t, tmp;
    double r, eps_min;
    ...
    {
        if (rel[j] > 0)
            mpn_submul_1(t, _mp_real_log_primes_entry(j, n + 1), n + 2, rel[j]);
        else if (rel[j] < 0)
            mpn_addmul_1(t, _mp_real_log_primes_entry(j, n + 1), n + 2, -rel[j]);
    }
...
main(int argc, char * argv[])
{
    ...
    flint_printf("bitwise (r = %d): precomputation %.0f us, per call %.1f us\n\n",
        _mp_real_exp_bitwise_rs_default_r(n), pre_b, tb);
    ...
    for (in = 0; in < nnum; in++)
    {
        ...
        _mp_real_rel_table(0, num);   /* table generation outside the timing */
        ...
        for (iw = 0; iw < nwf; iw++)
        {
            ...
            double t = time_call(_mp_real_exp_diophantine_tune, y, xs, n, nx, num, w);
            double r = 0;
            for (k = 0; k < nx; k++)
