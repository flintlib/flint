...
/* Tune the LARGE-n default reduction parameters of the bitwise
   ...
   tuned per size, at arbitrary r, by dev/tune_mp_real.py -- this
   ...
   _mp_real_sin_cos_bitwise_rs and _mp_real_tan_bitwise_rs share the
   half-angle reduction and reconstruction, hence a single table
   (_mp_real_trig_bitwise_rs_r_tab); it is tuned on sin_cos, the
   ...
   pasted directly into src/mp_real/exp_bitwise_rs.c,
   src/mp_real/log1p_bitwise_rs.c, src/mp_real/atan_bitwise_rs.c and
   src/mp_real/tan_bitwise_rs.c. */
...
static double
time_call(void (*func)(nn_ptr, ulong *, nn_srcptr, slong, int), nn_ptr y,
...
/* _mp_real_sin_cos_bitwise_rs computes two outputs; adapt it to the
...
static void
sin_cos_wrapper(nn_ptr y, ulong * err, nn_srcptr x, slong n, int r)
{
    ...
    _mp_real_sin_cos_bitwise_rs(y, sincos_scratch, err, x, n, r);
}
...
static int
wins(void (*func)(nn_ptr, ulong *, nn_srcptr, slong, int), nn_ptr y,
...
tune_func(const char * name,
          void (*func)(nn_ptr, ulong *, nn_srcptr, slong, int),
          slong nmax, slong rmax)
{
    ...
    flint_printf("\n/* smallest n at which each reduction parameter"
        " becomes optimal\n   (generated by"
        " src/mp_real/tune/tune-bitwise-r.c) */\n");
...
main(int argc, char * argv[])
{
    ...
    tune_func("_mp_real_exp_bitwise_rs", _mp_real_exp_bitwise_rs,
        nmax, rmax);
    tune_func("_mp_real_log1p_bitwise_rs", _mp_real_log1p_bitwise_rs,
        nmax, rmax);
    tune_func("_mp_real_atan_bitwise_rs", _mp_real_atan_bitwise_rs,
        nmax, rmax);
    /* sin_cos and tan share the half-angle path: the sin_cos run
       below yields _mp_real_trig_bitwise_rs_{r,n}_tab, and the tangent
       run is a cross-check whose crossovers should agree */
    tune_func("_mp_real_trig_bitwise_rs", sin_cos_wrapper,
        nmax, rmax);
    tune_func("mp_real_tan_bitwise_rs_CROSSCHECK", _mp_real_tan_bitwise_rs,
        nmax, rmax);
