...
/* Tune the algorithm choice of _mp_real_exp_reduced over a grid of
   ...
   MP_REAL_EXP_BURST_TERMS between sinh and one burst, and
   MP_REAL_EXP_FULLBURST_TERMS between one burst and the full
...
time_alg(nn_ptr y, nn_srcptr t, slong wn, flint_bitcnt_t r, int alg)
{
    ...
    for (pass = 0; pass < 3; pass++)
    {
        ...
        for (;;)
        {
            timeit_start(tm);
            for (k = 0; k < reps; k++)
                _mp_real_exp_reduced(y, NULL, t, wn, r, alg);
            timeit_stop(tm);
...
int main(int argc, char ** argv)
{
    ...
    flint_printf("\npaste thresholds after inspecting the grid:\n"
        ...
        "   EXP_USE_SINH, MP_REAL_EXP_BURST_TERMS(r),\n"
        "   MP_REAL_EXP_FULLBURST_TERMS(r); edit them there)\n");
    return 0;
}
