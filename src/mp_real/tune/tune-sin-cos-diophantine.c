...
/* Tune _mp_real_sin_cos_diophantine: for a size n, sweep the number of
   Gaussian primes and the weight budget (as a factor of the precision
   ...
   Usage: tune-sin-cos-diophantine n [nx] */
...
/* precomputation of the diophantine (multi-prime) scheme, from scratch: the angle
   cache build (arb_atan_gauss_primes_vec_bsplit, doubled, floored) */
static double
precomp_diophantine(slong num, slong n)
{
    double t0, t1;
    _mp_real_atan_gauss_clear();
    t0 = wall_us();
    _mp_real_atan_gauss_ensure(num, n + 1);
    t1 = wall_us();
    return t1 - t0;
}
...
precomp_bitwise(slong n)
{
    ...
    _mp_real_atans_clear();
    t0 = wall_us();
    _mp_real_sin_cos_bitwise_rs(y, scratch_cos, NULL, x, n, 0);
    t1 = wall_us();
    flint_free(x);
    flint_free(y);
    return t1 - t0;
...
static void
f_multi(nn_ptr y, nn_srcptr x, slong n, slong num, double w)
{
    _mp_real_sin_cos_diophantine_tune(y, scratch_cos, NULL, x, n, num, w);
}
...
{
    const mp_real_rel_struct * tab = _mp_real_rel_table(1, num);
    slong wr, j;
    slong * rel;
    nn_ptr base, t, tmp;
    double r, eps_min;
    ...
    _mp_real_log_reduce(rel, tab, base, wr, w, eps_min,
    ...
    {
        if (rel[j] > 0)
            mpn_submul_1(t, _mp_real_atan_gauss_entry(j, n + 1), n + 2, rel[j]);
        else if (rel[j] < 0)
            mpn_addmul_1(t, _mp_real_atan_gauss_entry(j, n + 1), n + 2, -rel[j]);
    }
    if (t[n + 1] >> (FLINT_BITS - 1))
        mpn_neg(t, t, n + 2);   /* a negative residual is fine here */
    if (t[n + 1] != 0)
        r = -1.0;
    else
    {
...
{
    static const slong nums[] = { 8, 13, 20, 32, 48, 64 };
    static const double wfs[] = { 0.25, 0.5, 1.0, 2.0, 4.0, 8.0 };
    ...
    scratch_cos = flint_malloc((n + 1) * sizeof(ulong));
    for (k = 0; k < nx; k++)
    ...
    flint_printf("bitwise (r = %d): precomputation %.0f us, per call %.1f us\n\n",
        _mp_real_trig_bitwise_rs_default_r(n), pre_b, tb);
    ...
    for (in = 0; in < nnum; in++)
    {
        ...
        _mp_real_rel_table(1, num);   /* table generation outside the timing */
        ...
        for (iw = 0; iw < nwf; iw++)
        {
            ...
            double t = time_call(f_multi, y, xs, n, nx, num, w);
            double r = 0;
            for (k = 0; k < nx; k++)
