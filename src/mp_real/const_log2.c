...
static void
_mp_real_set_mpn2(mp_real_t x, nn_srcptr d, slong len, int negative)
{
    slong t = 0;
...
static void
log2_bsplit_basecase(mp_real_t P, mp_real_t Q, mp_real_t T, slong a, slong b,
    int need_p)
{
...
#undef L2_MUL1
    ...
    if (need_p)
        _mp_real_set_mpn2(P, G, lg, 0);
    _mp_real_set_mpn2(Q, Qs, lq, 0);
    _mp_real_set_mpn2(T, S, ls, 0);
}
...
static void log2_bsplit(mp_real_t P, mp_real_t Q, mp_real_t T, slong a, slong b,
    slong n, int need_p);
...
static void
log2_bsplit(mp_real_t P, mp_real_t Q, mp_real_t T, slong a, slong b, slong n,
    int need_p)
{
    if (b - a <= L2_BS_BLK)
    {
    ...
    else
    {
        ...
        if (par && !above)
        {
            ...
            R.need_p = need_p;
            _mp_real_parallel_pair(_log2_bsplit_worker, &L,
...
_mp_real_const_log2_compute(mp_real_t res, slong n)
{
    ...
    mp_real_div(res, T, Q, n);

    /* series tail */
    mp_real_add_rel_error_2exp_si(res, -(FLINT_BITS * n + 96));
