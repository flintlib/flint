...
/* Profile a fixed-point elementary function against arb across
   ...
   and of the mp_real function, and the speedup ratio.
   ...
   p-mp_real exp_notab 1000000), and the number of distinct inputs
...
typedef struct
{
    const char * name;
    int outputs;                       /* 1 or 2 */
    void (*fixed1)(nn_ptr, ulong *, nn_srcptr, slong, int);
    void (*fixed2)(nn_ptr, nn_ptr, ulong *, nn_srcptr, slong, int);
...
static void p_exp_notab(nn_ptr y, ulong * err, nn_srcptr x, slong n, int r)
{ (void) r; _mp_real_exp_notab(y, err, x, n); }
static void p_sin_cos_notab(nn_ptr s, nn_ptr c, ulong * err, nn_srcptr x,
    slong n, int r)
{ (void) r; _mp_real_sin_cos_notab(s, c, err, x, n); }
static void p_exp_diophantine(nn_ptr y, ulong * err, nn_srcptr x, slong n, int r)
{ (void) r; _mp_real_exp_diophantine(y, err, x, n); }
static void p_sin_cos_diophantine(nn_ptr s, nn_ptr c, ulong * err, nn_srcptr x,
    slong n, int r)
{ (void) r; _mp_real_sin_cos_diophantine(s, c, err, x, n); }
static void p_tan_diophantine(nn_ptr y, ulong * err, nn_srcptr x, slong n, int r)
{ (void) r; _mp_real_tan_diophantine(y, err, x, n); }
static int p_notab_r(slong n) { (void) n; return 0; }

static const which_t table[] = {
    { "exp", 1, _mp_real_exp_bitwise_rs, NULL, a_exp, NULL,
      _mp_real_exp_bitwise_rs_default_r },
    { "log1p", 1, _mp_real_log1p_bitwise_rs, NULL, a_log1p, NULL,
      _mp_real_log1p_bitwise_rs_default_r },
    { "atan", 1, _mp_real_atan_bitwise_rs, NULL, a_atan, NULL,
      _mp_real_atan_bitwise_rs_default_r },
    { "tan", 1, _mp_real_tan_bitwise_rs, NULL, a_tan, NULL,
      _mp_real_trig_bitwise_rs_default_r },
    { "sin_cos", 2, NULL, _mp_real_sin_cos_bitwise_rs, NULL, a_sin_cos,
      _mp_real_trig_bitwise_rs_default_r },
    ...
    for (pass = 0; pass < 3; pass++)
    {
        ...
        while (1)
        {
            ...
            for (it = 0; it < reps; it++)
                ...
                {
                    if (w->outputs == 1)
                        w->fixed1(y1, NULL, &xs[0][0] + (size_t) k * n, n, 0);
                    else
                        w->fixed2(y1, y2, NULL, &xs[0][0] + (size_t) k * n, n, 0);
                }
            timeit_stop(tm);
            if (tm->wall >= 30)
                break;
...
main(int argc, char * argv[])
{
    ...
    if (w == NULL)
    {
        flint_printf("usage: p-mp_real exp|log1p|atan|sin_cos|tan"
    ...
    flint_printf("%6s %8s %8s %5s %12s %12s %8s\n",
        "n", "bits", "digits", "r", "arb (ns)", "mp_real (ns)", "ratio");
         ...
         n = (n < 12) ? n + 1 : FLINT_MAX(n + 1, n * 4 / 3))
    {
        ...
        if (w->outputs == 1)
            w->fixed1(y1, NULL, &xs[0][0], n, 0);
        else
            w->fixed2(y1, y2, NULL, &xs[0][0], n, 0);
