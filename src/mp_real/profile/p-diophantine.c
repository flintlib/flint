...
/* Compares the argument-reduction strategies for exp and sin_cos at
   a range of precisions: the table-free routines (_mp_real_exp_notab,
   _mp_real_sin_cos_notab), the bitwise reductions with their tuned
   table depth r (_mp_real_exp_bitwise_rs, _mp_real_sin_cos_bitwise_rs),
   and the diophantine reductions with several prime counts
   (_mp_real_exp_diophantine_tune, _mp_real_sin_cos_diophantine_tune).
...
run(slong n, double wfac, int reps, flint_rand_t st)
{
    ...
    /* warm up before the first timing */
    {
        ...
        while (wall() - t0 < 0.05)
        {
            _mp_real_exp_notab(y1, NULL, xs, n);
            _mp_real_sin_cos_notab(y1, y2, NULL, xs, n);
        }
    }
    ...
    flint_printf("    %-30s %9.2f %9.3f   (r = %d)\n", "bitwise", (pre - warm) / te,
        warm / te, _mp_real_exp_bitwise_rs_default_r(n));
    for (i = 0; i < 4; i++)
    {
        char name[64];
        _mp_real_rel_table(0, nums[i]);   /* the relation table is not timed */
        _mp_real_log_primes_clear();
        t0 = wall();
    ...
    flint_printf("    %-30s %9.2f %9.3f   (r = %d)\n", "bitwise", (pre - warm) / te,
        warm / te, _mp_real_trig_bitwise_rs_default_r(n));
    for (i = 0; i < 4; i++)
    {
        char name[64];
        _mp_real_rel_table(1, nums[i]);
