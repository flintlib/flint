...
TEST_TEMPLATE_FUNCTION_START(T, mat_scalar_mul_ui, state)
{
    slong rep;
    ...
    for (rep = 0; rep < 1000 * flint_test_multiplier(); rep++)
    {
        ...
        c = n_randtest_not_zero(state); // avoid overflow when subtracting 1
        TEMPLATE(T, mat_randtest)(A, state, ctx);
        TEMPLATE(T, mat_randtest)(B, state, ctx);

        TEMPLATE(T, mat_scalar_mul_ui)(C, A, c, ctx);
        c--;
        TEMPLATE(T, mat_scalar_mul_ui)(D, A, c, ctx);
        ...
        /* Aliasing */
        TEMPLATE(T, mat_scalar_mul_ui)(C, A, c, ctx);
        TEMPLATE(T, mat_scalar_mul_ui)(A, A, c, ctx);
