...
TEST_TEMPLATE_FUNCTION_START(T, mat_scalar_mul_si, state)
{
    slong rep;
    ...
    for (rep = 0; rep < 1000 * flint_test_multiplier(); rep++)
    {
        ...
        do {
            c = z_randtest(state);
        } while (c == WORD_MIN); // avoid overflow when subtracting 1
        TEMPLATE(T, mat_randtest)(A, state, ctx);
        TEMPLATE(T, mat_randtest)(B, state, ctx);

        TEMPLATE(T, mat_scalar_mul_si)(C, A, c, ctx);
        c--;
        TEMPLATE(T, mat_scalar_mul_si)(D, A, c, ctx);
        ...
        /* Aliasing */
        TEMPLATE(T, mat_scalar_mul_si)(C, A, c, ctx);
        TEMPLATE(T, mat_scalar_mul_si)(A, A, c, ctx);
