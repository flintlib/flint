    ...
    slong nb_steps, slong prec)
{
    ...
    for (k = nb_steps - 1; k >= 0; k--)
    {
        ...
        acb_theta_agm_sqrt(th, th, roots + k * n, n, prec);
    }
...
{    
    ...
    for (k = nb_steps - 1; k >= 0; k--)
    {
        ...
        acb_theta_agm_mul(next + n, cur, cur + n, g, prec);
        acb_theta_agm_mul(next + 2 * n, cur, cur + 2 * n, g, prec);
        _acb_vec_scalar_mul_2exp_si(next + n, next + n, 2 * n, g);
        acb_theta_agm_sqrt(next + n, next + n, roots + k * 2 * n, 2 * n, prec);
        
        /* Duplication formula with divisions */
        acb_theta_agm_sqr(next, cur + n, g, prec);
        _acb_vec_scalar_mul_2exp_si(next, next, n, g);
        for (a = 0; a < n; a++)
        {
            acb_div(&next[a], &next[a], &next[2 * n + a], prec);
        }
...
acb_theta_uql_const(acb_ptr th, const acb_mat_t tau, slong prec)
{
    ...
    hprec = acb_theta_ql_roots(r, NULL, 0, tau, nb_steps, prec);
    if (hprec >= 0)
    {
    ...
    {        
        hprec = acb_theta_uql_roots(r, t, NULL, 0, tau, nb_steps, prec);
        if (hprec >= 0)
        {
            agm_aux(th, r, t, tau, nb_steps, hprec);
        }
        else
        {
