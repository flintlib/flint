...
TEST_FUNCTION_START(acb_theta_one_notransform, state)
{
    slong iter;
    ...
    for (iter = 0; iter < 20 * flint_test_multiplier(); iter++)
    {
        ...
        acb_theta_one_notransform(th, z, nb, tau, ab, prec);

        acb_theta_all_notransform(all, z, nb, tau, 0, prec);
        for (k = 0; k < nb; k++)
        {
            acb_set(&test[k], &all[k * n2 + ab]);
        }

        /* flint_printf("g = %wd, prec = %wd, nb = %wd, ab = %wd, tau, z:\n", g, prec, nb, ab);
