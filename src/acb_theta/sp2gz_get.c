...
void
sp2gz_get_b(fmpz_mat_t res, const fmpz_mat_t mat)
{
    slong g = sp2gz_dim(mat);
    slong j, k;
...
void
sp2gz_get_c(fmpz_mat_t res, const fmpz_mat_t mat)
{
    slong g = sp2gz_dim(mat);
    slong j, k;
...
void
sp2gz_get_d(fmpz_mat_t res, const fmpz_mat_t mat)
{
    slong g = sp2gz_dim(mat);
    slong j, k;
    ...
    for (j = 0; j < g; j++)
    {
        for (k = 0; k < g; k++)
        {
            fmpz_set(fmpz_mat_entry(res, j, k),
                     fmpz_mat_entry(mat, j + g, k + g));
        }
    }
}
