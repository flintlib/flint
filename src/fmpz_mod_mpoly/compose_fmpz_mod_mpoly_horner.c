...
/* A = A * X^pow */
static int _fmpz_mod_mpoly_pmul(fmpz_mod_mpoly_t A, const fmpz_mod_mpoly_t X,
                  const fmpz_t pow, fmpz_mod_mpoly_t T, const fmpz_mod_mpoly_ctx_t ctx)
{
    ulong p;
    ...
    if (!fmpz_fits_si(pow))
    {
        if (!fmpz_mod_mpoly_pow_fmpz(T, X, pow, ctx))
        {
    ...
    {
        if (!fmpz_mod_mpoly_pow_ui(T, X, p, ctx))
        {
            fmpz_mod_mpoly_zero(A, ctx);
            return 0;
        }
                     ...
                     const fmpz_mod_mpoly_ctx_t ctxB, const fmpz_mod_mpoly_ctx_t ctxAC)
{
    ...
    fmpz_mod_mpoly_struct * regs;
    fmpz_mod_mpoly_t temp;
    slong * rtypes;
    ...
    {
       FLINT_ASSERT(Blen == 1);
       fmpz_mod_mpoly_set_fmpz(A, B->coeffs + 0, ctxAC);
       return 1;
    }
    ...
    regs   = (fmpz_mod_mpoly_struct *) TMP_ALLOC(nvars*sizeof(fmpz_mod_mpoly_struct));
    for (i = 0; i < nvars; i++)
        fmpz_mod_mpoly_init(regs + i, ctxAC);
    fmpz_mod_mpoly_init(temp, ctxAC);
...
HornerForm:
    ...
    else if (totalcounts == 1)
    {
        ...
        if (!fmpz_mod_mpoly_pow_fmpz(regs + rp, C[v], Buexp + nvars*f + v, ctxAC))
        {
            success = 0;
        }
        fmpz_mod_mpoly_scalar_mul_fmpz(regs + rp, regs + rp, Bcoeff + f, ctxAC);

        if (Blist[f] != -WORD(1)) /* if f has a second term */
        {
            /* this term should be a scalar */
            FLINT_ASSERT(fmpz_is_zero(Buexp + nvars*Blist[f] + v));
            fmpz_mod_mpoly_add_fmpz(regs + rp, regs + rp,  Bcoeff + Blist[f], ctxAC);
        }
...
HornerForm2:
        ...
        if (rtypes[rp - 1] == -WORD(1) && rtypes[rp] == -WORD(1))
        {
            ...
            if (!_fmpz_mod_mpoly_pmul(regs + rp - 1, C[(stack + sp)->v_var],
                                             (stack + sp)->v_exp, temp, ctxAC))
            {
                success = 0;
            }
            fmpz_mod_mpoly_add(temp, regs + rp - 1, regs + rp, ctxAC);
            fmpz_mod_mpoly_swap(temp, regs + rp - 1, ctxAC);
        }
        else if (rtypes[rp - 1] == -WORD(1) && rtypes[rp] != -WORD(1))
        {
            ...
            fmpz_mod_mpoly_add_fmpz(regs + rp - 1, regs + rp - 1,
                                                   Bcoeff + rtypes[rp], ctxAC);
        }
        else if (rtypes[rp - 1] != -WORD(1) && rtypes[rp] == -WORD(1))
        {
            /* quotient is a scalar, remainder is a polynomial */
            if (!fmpz_mod_mpoly_pow_fmpz(temp, C[(stack + sp)->v_var],
                                                   (stack + sp)->v_exp, ctxAC))
            {
                success = 0;
            }
            fmpz_mod_mpoly_scalar_mul_fmpz(temp, temp, Bcoeff + rtypes[rp - 1], ctxAC);
            fmpz_mod_mpoly_add(regs + rp - 1, temp, regs + rp, ctxAC);
        }
        else
        {
    ...
    else
    {
        ...
        if (!_fmpz_mod_mpoly_pmul(regs + rp, C[(stack + sp)->v_var],
                                            (stack + sp)->v_exp, temp, ctxAC))
        {
            success = 0;
        }
...
HornerFormReturn:
    ...
    if (rtypes[rp] == -WORD(1))
    {
        fmpz_mod_mpoly_swap(A, regs + rp, ctxAC);
    }
    else
    {
        fmpz_mod_mpoly_set_fmpz(A, Bcoeff + rtypes[rp], ctxAC);
    }

cleanup:

    for (i = 0; i < nvars; i++)
        fmpz_mod_mpoly_clear(regs + i, ctxAC);
    fmpz_mod_mpoly_clear(temp, ctxAC);
