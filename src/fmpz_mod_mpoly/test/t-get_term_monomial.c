...
TEST_FUNCTION_START(fmpz_mod_mpoly_get_term_monomial, state)
{
    int i, j;
    ...
    for (i = 0; i < 100 * flint_test_multiplier(); i++)
    {
        ...
        fmpz_mod_mpoly_repack_bits(h, f,
                                f->bits + n_randint(state, 2*FLINT_BITS), ctx);

        for (j = fmpz_mod_mpoly_length(f, ctx) - 1; j >= 0; j--)
        {
            fmpz_mod_mpoly_get_term_monomial(g, f, j, ctx);
            fmpz_mod_mpoly_repack_bits(g, g,
                                  g->bits + n_randint(state, FLINT_BITS), ctx);
            fmpz_mod_mpoly_get_term_coeff_fmpz(d, f, j, ctx);
            fmpz_mod_mpoly_get_coeff_fmpz_monomial(c, h, g, ctx);
