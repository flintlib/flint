...
void
arb_mat_vector_mul_row(arb_ptr res, arb_srcptr row, const arb_mat_t A, slong prec)
{
    ...
    for (k = 0; k < nrow; k++)
    {
        arb_set(arb_mat_entry(r, 0, k), &row[k]);
    }
...
void
arb_mat_vector_mul_col(arb_ptr res, const arb_mat_t A, arb_srcptr col, slong prec)
{    
    slong nrow = arb_mat_nrows(A);
    slong ncol = arb_mat_ncols(A);
    arb_mat_t c, p;
    ...
    for (k = 0; k < ncol; k++)
    {
        arb_set(arb_mat_entry(c, k, 0), &col[k]);
    }
