...
TEST_FUNCTION_START(qqbar_pow_si, state)
{
    slong iter;
    ...
    for (iter = 0; iter < 100 * 0.1 * flint_test_multiplier(); iter++)
    {
        ...
        m = n_randint(state, 7) - 4;
        n = n_randint(state, 7) - 4;

        while ((m < 0 || n < 0) && qqbar_is_zero(x))
    ...
    for (iter = 0; iter < 100 * 0.1 * flint_test_multiplier(); iter++)
    {
        ...
        while (n < 0 && qqbar_is_zero(x))
            qqbar_randtest(x, state, 2, 10);

        while (n < 0 && qqbar_is_zero(y))
            qqbar_randtest(y, state, 2, 10);

        qqbar_mul(xyn, x, y);
        qqbar_pow_si(xyn, xyn, n);
