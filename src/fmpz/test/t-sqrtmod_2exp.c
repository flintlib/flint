...
TEST_FUNCTION_START(fmpz_sqrtmod_2exp, state)
{
    slong i;
    ...
    for (i = 0; i < 5000 * flint_test_multiplier(); i++)
    {
        ...
        low = fmpz_fdiv_ui(a, 8);
        expected = (low & 1) && ((N == 1) || (N == 2 && (low & 3) == 1) || (N >= 3 && low == 1));

        res = fmpz_sqrtmod_2exp(b, a, N);

        if (res != expected)
            TEST_FUNCTION_FAIL("return value: N = %wd\na = %{fmpz}\n", (slong) N, a);
        ...
        if (res)
        {
            ...
            if (!fmpz_is_zero(c) || fmpz_sgn(b) < 0 || fmpz_cmp(b, m) >= 0)
                TEST_FUNCTION_FAIL("N = %wd\na = %{fmpz}\nb = %{fmpz}\n", (slong) N, a, b);

            fmpz_set(c, a);
            fmpz_sqrtmod_2exp(c, c, N);
            if (!fmpz_equal(b, c))
