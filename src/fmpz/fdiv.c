...
fmpz_fdiv_q(fmpz_t f, const fmpz_t g, const fmpz_t h)
{
    ...
    else                        /* g is large */
    {
        ...
        {
            if (_fmpz_div_qr_small_divisor(f, NULL, g, c2, -1))
                return;
            mf = _fmpz_promote(f);
        ...
        {
            mf = _fmpz_promote(f);
            flint_mpz_fdiv_q(mf, COEFF_TO_PTR(c1), COEFF_TO_PTR(c2));
            _fmpz_demote_val(f);  /* division by h may result in small value */
        }
    }
}
...
fmpz_fdiv_qr(fmpz_t f, fmpz_t s, const fmpz_t g, const fmpz_t h)
{
    ...
    else                        /* g is large */
    {
		...
		if (!COEFF_IS_MPZ(c2))  /* h is small */
        {
            if (_fmpz_div_qr_small_divisor(f, s, g, c2, -1))
                return;
        ...
        else                    /* both are large */
        {
            ...
            flint_mpz_fdiv_qr(mf, ms, COEFF_TO_PTR(c1), COEFF_TO_PTR(c2));
...
fmpz_fdiv_q_si(fmpz_t f, const fmpz_t g, slong h)
{
    ...
    {
        if (_fmpz_div_qr_small_divisor(f, NULL, g, h, -1))
            return;
...
fmpz_fdiv_q_ui(fmpz_t f, const fmpz_t g, ulong h)
{
    ...
    {
        if (_fmpz_div_qr_small_divisor_ui(f, NULL, NULL, g, h, 0, -1))
            return;
        mpz_ptr mf = _fmpz_promote(f);
...
fmpz_fdiv_r(fmpz_t f, const fmpz_t g, const fmpz_t h)
{
    ...
    else                        /* g is large */
    {
        ...
        if (!COEFF_IS_MPZ(c2))  /* h is small */
        {
            if (_fmpz_div_qr_small_divisor(NULL, f, g, c2, -1))
                return;
        ...
        {
            mf = _fmpz_promote(f);
            flint_mpz_fdiv_r(mf, COEFF_TO_PTR(c1), COEFF_TO_PTR(c2));
            _fmpz_demote_val(f);    /* division by h may result in small value */
        }
    }
}
...
fmpz_fdiv_ui(const fmpz_t g, ulong h)
{
    ...
    {
        ulong rem;
        if (_fmpz_div_qr_small_divisor_ui(NULL, NULL, &rem, g, h, 0, -1))
            return rem;
