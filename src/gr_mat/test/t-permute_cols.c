...
TEST_GR_FUNCTION_START(gr_mat_permute_cols, state, count_success, count_domain, count_unable)
{
    ...
    for (iter = 0; iter < 100 * flint_test_multiplier(); iter++)
    {
        ...
        status |= gr_mat_permute_cols(matt, perm_store, perm_act, ctx);
        ...
        for (j = 0; j < n; j++)
        {
            ...
            for (i = 0; i < m; i++)
            {
                if (gr_equal(GR_MAT_ENTRY(matt, i, j, ctx->sizeof_elem), GR_MAT_ENTRY(mat, i, perm_act[j], ctx->sizeof_elem), ctx) == T_FALSE)
                {
                    flint_printf("FAIL (2):\n");
