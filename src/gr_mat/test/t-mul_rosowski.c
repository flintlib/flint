...
TEST_FUNCTION_START(gr_mat_mul_rosowski, state)
{
    slong iter;
    ...
    for (iter = 0; iter < 1000; iter++)
    {
        ...
        a = n_randint(state, 12);
        b = n_randint(state, 2) ? a : n_randint(state, 12);
        c = n_randint(state, 2) ? a : n_randint(state, 12);
        ...
        if (a == b && b == c && n_randint(state, 2))
        {
            ...
            status |= gr_mat_mul_rosowski(C, A, A, ctx);
        }
        else if (b == c && n_randint(state, 2))
        {
            status |= gr_mat_set(C, A, ctx);
            status |= gr_mat_mul_rosowski(C, C, B, ctx);
        }
        else if (a == b && n_randint(state, 2))
        {
            status |= gr_mat_set(C, B, ctx);
            status |= gr_mat_mul_rosowski(C, A, C, ctx);
        }
        else
        {
            status |= gr_mat_mul_rosowski(C, A, B, ctx);
        }
