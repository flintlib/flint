...
gr_mat_div_scalar(gr_mat_t res, const gr_mat_t mat, gr_srcptr x, gr_ctx_t ctx)
{
    ...
    if (c != 0)
        for (i = 0; i < r; i++)
            status |= _gr_vec_div_scalar(res->rows[i], mat->rows[i], c, x, ctx);
...
gr_mat_div_scalar_other(gr_mat_t res, const gr_mat_t mat, gr_srcptr x, gr_ctx_t x_ctx, gr_ctx_t ctx)
{
    ...
    if (c != 0)
        for (i = 0; i < r; i++)
            status |= _gr_vec_div_scalar_other(res->rows[i], mat->rows[i], c, x, x_ctx, ctx);
...
gr_mat_div_ui(gr_mat_t res, const gr_mat_t mat, ulong x, gr_ctx_t ctx)
{
    ...
    if (c != 0)
        for (i = 0; i < r; i++)
            status |= _gr_vec_div_scalar_ui(res->rows[i], mat->rows[i], c, x, ctx);
...
gr_mat_div_si(gr_mat_t res, const gr_mat_t mat, slong x, gr_ctx_t ctx)
{
    ...
    if (c != 0)
        for (i = 0; i < r; i++)
            status |= _gr_vec_div_scalar_si(res->rows[i], mat->rows[i], c, x, ctx);
...
gr_mat_div_fmpz(gr_mat_t res, const gr_mat_t mat, const fmpz_t x, gr_ctx_t ctx)
{
    ...
    if (c != 0)
        for (i = 0; i < r; i++)
            status |= _gr_vec_div_scalar_fmpz(res->rows[i], mat->rows[i], c, x, ctx);
...
gr_mat_div_fmpq(gr_mat_t res, const gr_mat_t mat, const fmpq_t x, gr_ctx_t ctx)
{
    ...
    if (c != 0)
        for (i = 0; i < r; i++)
            status |= _gr_vec_div_scalar_fmpq(res->rows[i], mat->rows[i], c, x, ctx);
