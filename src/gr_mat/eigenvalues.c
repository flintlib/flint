...
int
gr_mat_eigenvalues(gr_vec_t lambda, fmpz_vec_t mult, const gr_mat_t mat, int FLINT_UNUSED(flags), gr_ctx_t ctx)
{
    int status;
    gr_poly_t cp;
    gr_poly_init(cp, ctx);
...
int
gr_mat_eigenvalues_other(gr_vec_t lambda, fmpz_vec_t mult, const gr_mat_t mat, gr_ctx_t mat_ctx, int FLINT_UNUSED(flags), gr_ctx_t ctx)
{
    int status;
    gr_poly_t cp;
