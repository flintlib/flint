...
int main()
{
    ...
    slong lenA, lenB;
    double t1, t2, __;
    double speedup_mul, speedup_sqr, speedup_mullow, speedup_sqrlow;
    slong i;
    ...
    for (i = 0; i < 6; i++)
    {
        ...
        flint_printf("  bits    n   mul    sqr    mullow sqrlow\n");
        ...
        for (n = 1; n <= 8192; n *= 2)
        {
            ...
            C = gr_heap_init_vec(lenA + lenB - 1, ctx);
            ...
            C2 = gr_heap_init_vec(lenA + lenB - 1, ctx2);
            ...
            flint_printf("%5wd %5wd   %.3f  %.3f  %.3f  %.3f\n", bits, n, speedup_mul, speedup_sqr, speedup_mullow, speedup_sqrlow);

            (void) __;
            gr_heap_clear_vec(A, lenA, ctx);
            gr_heap_clear_vec(B, lenB, ctx);
            gr_heap_clear_vec(C, lenA + lenB - 1, ctx);

            gr_heap_clear_vec(A2, lenA, ctx2);
            gr_heap_clear_vec(B2, lenB, ctx2);
            gr_heap_clear_vec(C2, lenA + lenB - 1, ctx2);
