...
TEST_FUNCTION_START(mpn_mod_poly_mullow_karatsuba, state)
{
    ...
    for (iter = 0; iter < 1000 * flint_test_multiplier(); iter++)
    {
        ...
        gr_poly_test_mullow((gr_method_poly_binary_trunc_op) _mpn_mod_poly_mullow_karatsuba_deep, state, 10, 40, ctx);
        gr_poly_test_mullow((gr_method_poly_binary_trunc_op) _mpn_mod_poly_mullow_karatsuba_shallow, state, 10, 40, ctx);
        gr_ctx_clear(ctx);
    }
