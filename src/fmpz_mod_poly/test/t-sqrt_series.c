...
TEST_FUNCTION_START(fmpz_mod_poly_sqrt_series, state)
{
    int i, result;
    ...
    for (i = 0; i < 100 * flint_test_multiplier(); i++)
    {
        ...
        if (fmpz_cmp_ui(ctx->n, 2) != 0)
        {
            ...
            fmpz_mod_poly_sqrt_series(g, h, m, ctx);
            fmpz_mod_poly_mullow(r, g, g, m, ctx);
            fmpz_mod_poly_truncate(h, m, ctx);

            result = (fmpz_mod_poly_equal(r, h, ctx));
            if (!result)
            {
                flint_printf("FAIL:\n");
    ...
    for (i = 0; i < 100 * flint_test_multiplier(); i++)
    {
        ...
        if (fmpz_cmp_ui(ctx->n, 2) != 0)
        {
            ...
            fmpz_mod_poly_sqrt_series(g, h, m, ctx);
            fmpz_mod_poly_sqrt_series(h, h, m, ctx);

            result = (fmpz_mod_poly_equal(g, h, ctx));
            if (!result)
            {
                flint_printf("FAIL:\n");
                fmpz_mod_poly_print(h, ctx), flint_printf("\n\n");
