...
TEST_FUNCTION_START(fmpz_mod_poly_resultant_euclidean, state)
{
    int i, result;
    fmpz_mod_ctx_t ctx;
    ...
    for (i = 0; i < 100 * flint_test_multiplier(); i++)
    {
        ...
        fmpz_mod_poly_randtest(g, state, n_randint(state, 100), ctx);

        fmpz_mod_poly_resultant_euclidean(x, f, g, ctx);
        fmpz_mod_poly_resultant_euclidean(y, g, f, ctx);
    ...
    for (i = 0; i < 100 * flint_test_multiplier(); i++)
    {
        ...
        fmpz_mod_poly_resultant_euclidean(y, f, g, ctx);
        fmpz_mod_poly_resultant_euclidean(z, h, g, ctx);
