...
/*
    ...
    The quotient limbs are estimated from two limbs by divrem21_preinv
    with the top limb of the inverse computed by flint_mpn_preinvn and
    corrected by one comparison. (The schoolbook division of
    flint_mpn_divrem_preinv1 would need the exact 3/2 inverse of
    flint_mpn_preinv1, of which that limb is occasionally one less.)
*/
                                  ...
                                  mp_srcptr b, mp_size_t n, mp_limb_t dinv)
{
    ...
    for (i = m - 1; i >= n; i--)
    {
        divrem21_preinv(q, a[i], a[i - 1], dinv);
