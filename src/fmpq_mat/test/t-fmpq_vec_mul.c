...
TEST_FUNCTION_START(fmpq_mat_fmpq_vec_mul, state)
{
    slong i;
    ...
    for (i = 0; i < 200 * flint_test_multiplier(); i++)
    {
        ...
        _fmpq_vec_randtest(a, state, alen, n_randint(state, 200) + 1);
        ...
        aa = FLINT_ARRAY_ALLOC(alen, fmpq*);
        for (j = 0; j < alen; j++)
        {
            aa[j] = FLINT_ARRAY_ALLOC(1, fmpq);
            fmpq_init(aa[j]);
            fmpq_set(aa[j], a + j);
        }

        fmpq_mat_fmpq_vec_mul(c, a, alen, B);
        fmpq_mat_fmpq_vec_mul_ptr(cc, (const fmpq * const *)aa, alen, B);

        /* supposed to match mul of the chopped or zero-extended a */
        for (j = 0; j < m && j < alen; j++)
            fmpq_set(fmpq_mat_entry(A, 0, j), a + j);
