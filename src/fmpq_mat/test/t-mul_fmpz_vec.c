...
TEST_FUNCTION_START(fmpq_mat_mul_fmpz_vec, state)
{
    slong i;
    ...
    for (i = 0; i < 100 * flint_test_multiplier(); i++)
    {
        ...
        fmpq_mat_mul_fmpz_vec(c, A, b, blen);
        fmpq_mat_mul_fmpz_vec_ptr(cc, A, (const fmpz * const *)bb, blen);

        /* supposed to match mul of the chopped or zero-extended b */
        for (j = 0; j < n && j < blen; j++)
            fmpq_set_fmpz(fmpq_mat_entry(B, j, 0), b + j);
