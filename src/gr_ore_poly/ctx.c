...
static const char * default_var = "Dx";
...
ore_poly_write(gr_stream_t out, gr_ore_poly_t poly, gr_ctx_t ctx)
{
    ...
    return gr_ore_poly_write(out, poly, GR_ORE_POLY_CTX(ctx)->var, ctx);
}
...
void
gr_ore_poly_ctx_init(gr_ore_poly_ctx_t ctx, gr_ctx_t base_ring, slong base_var, const ore_poly_which_operator_t operator)
{
    ...
    GR_ORE_POLY_CTX(ctx)->which_operator = operator;
...
{
    gr_ore_poly_ctx_init(ctx, base_ring, 0, ore_poly_which_operator_randtest(state));
}
