...
int gr_ore_poly_ctx_write(gr_stream_t out, gr_ore_poly_ctx_t ctx)
{
    ...
    gr_ctx_write(out, ORE_POLY_ELEM_CTX(ctx));
    return GR_SUCCESS;
}
...
int _gr_ore_poly_ctx_set_gen_name(gr_ctx_t ctx, const char * s)
{
    ...
    if (ORE_POLY_CTX(ctx)->var == default_var)
        ORE_POLY_CTX(ctx)->var = NULL;

    ORE_POLY_CTX(ctx)->var = flint_realloc(ORE_POLY_CTX(ctx)->var, len + 1);
    memcpy(ORE_POLY_CTX(ctx)->var, s, len + 1);
    return GR_SUCCESS;
}
...
gr_ore_poly_ctx_clear(gr_ore_poly_ctx_t ctx)
{
    if (ORE_POLY_CTX(ctx)->var != default_var)
    {
        flint_free(ORE_POLY_CTX(ctx)->var);
    }
}
...
{
    return gr_ctx_is_ring(ORE_POLY_ELEM_CTX(ctx));
}

truth_t
gr_ore_poly_ctx_is_zero_ring(gr_ore_poly_ctx_t ctx)
{
    return gr_ctx_is_zero_ring(ORE_POLY_ELEM_CTX(ctx));
}
...
{
    return gr_ctx_is_threadsafe(ORE_POLY_ELEM_CTX(ctx));
}
...
gr_ore_poly_i(gr_ore_poly_t res, gr_ore_poly_ctx_t ctx)
{
    ...
    status = gr_i(res->coeffs, ORE_POLY_ELEM_CTX(ctx));
...
gr_ore_poly_gens_recursive(gr_vec_t vec, gr_ore_poly_ctx_t ctx)
{
    ...
    gr_vec_init(vec1, 0, ORE_POLY_ELEM_CTX(ctx));
    status = gr_gens_recursive(vec1, ORE_POLY_ELEM_CTX(ctx));
    n = vec1->length;
    ...
    for (i = 0; i < n; i++)
        status |= gr_ore_poly_set_other(gr_vec_entry_ptr(vec, i, ctx),
                gr_vec_entry_srcptr(vec1, i, ORE_POLY_ELEM_CTX(ctx)),
                ORE_POLY_ELEM_CTX(ctx),
                ctx);

    status |= gr_ore_poly_gen(gr_vec_entry_ptr(vec, n, ctx), ctx);

    gr_vec_clear(vec1, ORE_POLY_ELEM_CTX(ctx));
...
gr_ore_poly_ctx_init(gr_ore_poly_ctx_t ctx, gr_ctx_t base_ring, slong base_var, const ore_algebra_t which_algebra)
{
    ...
    ORE_POLY_CTX(ctx)->base_ring = (gr_ctx_struct *) base_ring;
    ORE_POLY_CTX(ctx)->degree_limit = WORD_MAX;
    ORE_POLY_CTX(ctx)->var = (char *) default_var;
    ORE_POLY_CTX(ctx)->which_algebra = which_algebra;
    ORE_POLY_CTX(ctx)->base_var = base_var;
