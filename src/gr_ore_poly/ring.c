...
int
_gr_ore_poly_write(gr_stream_t out, gr_srcptr poly, slong n, const char * x, gr_ore_poly_ctx_t ctx)
{
    return _gr_poly_write(out, poly, n, x, GR_ORE_POLY_ELEM_CTX(ctx));
}

int
gr_ore_poly_write(gr_stream_t out, const gr_ore_poly_t poly, const char * x, gr_ore_poly_ctx_t ctx)
{
    return _gr_poly_write(out, poly->coeffs, poly->length, x, GR_ORE_POLY_ELEM_CTX(ctx));
}

int
_gr_ore_poly_get_str(char ** res, gr_srcptr f, slong len, const char * x, gr_ore_poly_ctx_t ctx)
{
    return _gr_poly_get_str(res, f, len, x, GR_ORE_POLY_ELEM_CTX(ctx));
}

int
gr_ore_poly_get_str(char ** res, const gr_ore_poly_t f, const char * x, gr_ore_poly_ctx_t ctx)
{
    return _gr_poly_get_str(res, f->coeffs, f->length, x, GR_ORE_POLY_ELEM_CTX(ctx));
}
...
gr_ore_poly_print(const gr_ore_poly_t poly, gr_ore_poly_ctx_t ctx)
{
    ...
    return gr_ore_poly_write(out, poly, "Dx", ctx);
}

int
gr_ore_poly_set_str(gr_ore_poly_t res, const char * s, const char * x, gr_ore_poly_ctx_t ctx)
{
    return gr_poly_set_str((gr_poly_struct *) res, s, x, GR_ORE_POLY_ELEM_CTX(ctx));
}

int
_gr_ore_poly_set_str(gr_ptr res, const char * s, const char * x, slong len, gr_ore_poly_ctx_t ctx)
{
    return _gr_poly_set_str(res, s, x, len, GR_ORE_POLY_ELEM_CTX(ctx));
}
...
{
    if (x_ctx == GR_ORE_POLY_ELEM_CTX(ctx)) {
        return gr_poly_set_scalar((gr_poly_struct *) poly, x, GR_ORE_POLY_ELEM_CTX(ctx));
    } else if (x_ctx == ctx) {
        return gr_poly_set((gr_poly_struct *) poly, x, ctx);
...
{
    if (x_ctx == GR_ORE_POLY_ELEM_CTX(ctx)) {
        return gr_poly_mul_scalar((gr_poly_struct *) res, (gr_poly_struct *) poly, x, GR_ORE_POLY_ELEM_CTX(ctx));
    } else if (x_ctx == ctx) {
        return gr_poly_mul((gr_poly_struct *) res, (const gr_poly_struct *) poly, x, GR_ORE_POLY_ELEM_CTX(ctx));
    } else if (x_ctx->which_ring == GR_CTX_FMPZ) {
