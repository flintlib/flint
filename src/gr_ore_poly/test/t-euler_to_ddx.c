...
TEST_GR_FUNCTION_START(gr_ore_poly_euler_to_ddx, state, count_success, count_domain, count_unable)
{
    for (slong iter = 0; iter < 1000 * flint_test_multiplier(); iter++)
    {
        ...
        /* round trip ddx_to_euler(euler_to_ddx(P_e)) = x^(len-1) P_e */
        ...
        {
            flint_printf("FAIL: euler_to_ddx round trip\n");
            flint_abort();
        }
        ...
        for (j = 0; j < 4; j++)
        {
            ...
            /* euler_to_ddx leaves the operator unchanged: P_d(f) = P_e(f) */
            if (status == GR_SUCCESS && gr_equal(g_d, g_e, cctx) == T_FALSE)
            {
                flint_printf("FAIL: euler_to_ddx application identity\n");
                flint_abort();
            }
        }
        ...
        {
            flint_printf("FAIL: euler_to_ddx unexpected failure\n");
            flint_abort();
        }
