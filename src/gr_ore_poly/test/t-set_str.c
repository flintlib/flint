...
TEST_FUNCTION_START(gr_ore_poly_set_str, state)
{
    ...
    for (iter = 0; iter < 1000 * flint_test_multiplier(); iter++)
    {
        ...
        gr_ctx_init_gr_ore_poly(ore_ctx, ctx, 0, ORE_OPERATOR_STANDARD_DERIVATIVE);
        ...
        {
            status |= gr_ore_poly_get_str(&s, f, "XX", ore_ctx);
            status |= gr_ore_poly_set_str(g, s, "XX", ore_ctx);
            glen = -1;
        ...
        {
            status |= _gr_ore_poly_get_str(&s, f->coeffs, f->length, "XX", ore_ctx);
            glen = n_randint(state, 10);
            gr_ore_poly_fit_length(g, glen, ore_ctx);
            status |= _gr_ore_poly_set_str(g->coeffs, s, "XX", glen, ore_ctx);
