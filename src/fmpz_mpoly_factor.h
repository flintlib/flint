...
{
    fmpz_set(fmpq_numref(c), f->constant); fmpz_set(fmpq_denref(c), f->constant_den);
}

FMPZ_MPOLY_FACTOR_INLINE
void fmpz_mpoly_factor_get_base(fmpz_mpoly_t p, const fmpz_mpoly_factor_t f, slong i, const fmpz_mpoly_ctx_t ctx)
{
    FLINT_ASSERT(i < (ulong) f->num); fmpz_mpoly_set(p, f->poly + i, ctx);
}

FMPZ_MPOLY_FACTOR_INLINE
void fmpz_mpoly_factor_swap_base(fmpz_mpoly_t p, fmpz_mpoly_factor_t f, slong i, const fmpz_mpoly_ctx_t ctx)
{
    FLINT_ASSERT(i < (ulong) f->num); fmpz_mpoly_swap(p, f->poly + i, ctx);
}
...
int fmpz_mpoly_factor_bound_si(
    fmpz_t B,
    const fmpz_t A,
    const slong * degs,
    slong nvars);
...
FMPZ_MPOLY_FACTOR_INLINE
void fmpz_mpolyv_swap(fmpz_mpolyv_t A, fmpz_mpolyv_t B, const fmpz_mpoly_ctx_t ctx)
{
    FLINT_SWAP(fmpz_mpolyv_struct, *A, *B);
}
...
{
    FLINT_SWAP(fmpz_bpoly_struct, *A, *B);
}

FMPZ_MPOLY_FACTOR_INLINE
void fmpz_bpoly_fit_length(fmpz_bpoly_t A, slong len)
{
    if (A->alloc < len) fmpz_bpoly_realloc(A, len);
}
...
{
    FLINT_SWAP(fmpz_tpoly_struct, *A, *B);
}
...
{
    FLINT_ASSERT(A->length > 0); if (fmpz_sgn(A->coeffs + 0) < 0) fmpz_mpoly_neg(A, A, ctx);
}
