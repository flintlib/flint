...
void
_arb_poly_mulmid_classical(arb_ptr res,
    ...
    arb_srcptr poly2, slong len2, slong nlo, slong nhi, slong prec)
{
    ...
    else if (poly1 == poly2 && len1 == len2)
    {
        ...
        for (i = FLINT_MAX(nlo, 1); i < FLINT_MIN(nhi, 2 * len1 - 2); i++)
        {
            ...
            arb_dot(res + i - nlo, NULL, 0, poly1 + start, 1,
                poly1 + i - start, -1, stop - start + 1, prec);
            arb_mul_2exp_si(res + i - nlo, res + i - nlo, 1);
            if (i % 2 == 0 && i / 2 < len1)
                arb_addmul(res + i - nlo, poly1 + i / 2, poly1 + i / 2, prec);
        }

        if (nhi >= 2 * len1 - 1)
            arb_sqr(res + 2 * len1 - 2 - nlo, poly1 + len1 - 1, prec);
    }
    else if (len1 == 1)
    {
        _arb_vec_scalar_mul(res, poly2 + nlo, len, poly1, prec);
    }
    else if (len2 == 1)
    {
        _arb_vec_scalar_mul(res, poly1 + nlo, len, poly2, prec);
    }
    else
    {
        slong i, top1, top2;
        ...
        for (i = nlo; i < nhi; i++)
        {
            ...
            arb_dot(res + i - nlo, NULL, 0, poly1 + i - top2, 1,
...
void
arb_poly_mulmid_classical(arb_poly_t res, const arb_poly_t poly1,
              const arb_poly_t poly2, slong nlo, slong nhi, slong prec)
{
    ...
    if (res == poly1 || res == poly2)
    {
        ...
        _arb_poly_mulmid_classical(tmp->coeffs, poly1->coeffs, xlen,
    ...
    else
    {
        ...
        _arb_poly_mulmid_classical(res->coeffs, poly1->coeffs, xlen,
            poly2->coeffs, ylen, nlo, nhi, prec);
    }
