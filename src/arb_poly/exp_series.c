    ...
    arb_srcptr h, slong len, slong prec, int inverse, slong cutoff)
{
    ...
    if (n == len && inverse)
    {
        _arb_poly_mulmid(T, f, n, g, m, m, n, prec);
        _arb_poly_mullow(g + m, g, m, T, n - m, n - m, prec);
