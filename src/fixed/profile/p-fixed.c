...
/* Profile a fixed-point elementary function against arb across
   ...
   with FUNC one of exp, log1p, atan, sin_cos, tan.  Sizes run
...
static double
time_arb(const which_t * w, arb_ptr xa, slong n)
{
    double best = 1e300;
...
main(int argc, char * argv[])
{
    ...
    if (w == NULL)
    {
        flint_printf("usage: p-fixed exp|log1p|atan|sin_cos|tan [nmax]\n");
        return 1;
    }
