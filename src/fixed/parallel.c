...
/* Thread helpers for the fixed module's series.

   _fixed_parallel_pair runs f1(a1) and f2(a2), the second on a pool
   ...
   _fixed_parallel_tasks runs f(i, args) for 0 <= i < n on
...
int
_fixed_parallel_pair(void (* f1)(void *), void * a1,
...
void
_fixed_parallel_tasks(void (* f)(slong, void *), void * args, slong n)
{
...
_pqt_left(void * arg)
{
    ...
    fball_mul(A->T, A->T, A->Q2, A->n);
    fball_mul(A->Q, A->Q, A->Q2, A->n);
}
...
{
    _pqt_struct * A = (_pqt_struct *) arg;
    fball_mul(A->T2, A->T2, A->P, A->n);
    if (A->need_p)
        fball_mul(A->P, A->P, A->P2, A->n);
}

void
_fball_pqt_merge(fball_t P, fball_t Q, fball_t T, fball_t P2, fball_t Q2,
    fball_t T2, int need_p, slong n, int par)
{
    _pqt_struct A;
    ...
    if (par)
        _fixed_parallel_pair(_pqt_left, &A, _pqt_right, &A);
    else
    {
        _pqt_left(&A);
        _pqt_right(&A);
