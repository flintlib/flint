...
/* Binary splitting for the table values log(1 + 2^-i) and
   ...
   or large i), no fball arithmetic is needed at all: the exact
   partial sum is floored by one division.

   ERRORS.  Everything above the leaves is fball arithmetic, which
...
typedef struct
{
    ...
    slong bufn;
    fball_struct * tmp; /* merge temporaries: 4 per tree level, reused
...
static void
leaf(fball_t N, fball_t D, fball_t E, slong a, slong b,
    const tab_bs_t * P, int need_d)
{
    ...
    if (need_d || !P->atanh)
        fball_set_mpn_2exp(D, BUF_D(P), dl, 0);
    if (P->atanh)
        fball_set_mpn_2exp(E, BUF_E(P), el, 0);
}

static void
bsplit(fball_t N, fball_t D, fball_t E, slong a, slong b,
    const tab_bs_t * P, int need_d, slong depth)
{
    fball_struct * N2 = P->tmp + 4 * depth, * D2 = N2 + 1, * E2 = N2 + 2,
        * T = N2 + 3;
    slong m, wp = P->wp;
    ...
    /* outputs never alias inputs (fball would stage the product in
       a temporary and copy it back); results are swapped into place */
    if (!P->atanh)
    {
        /* N = N1 D2 2^(e (b-m)) + s^(m-a) N2 D1, D = D1 D2 */
        fball_mul(T, N, D2, wp);
        fball_mul_2exp_si(T, P->e * (b - m));
        fball_mul(E2, N2, D, wp);
        if ((m - a) & 1)
            fball_sub(N, T, E2, wp);
        else
            fball_add(N, T, E2, wp);
...
static void
_fball_add_error_2exp(fball_t x, slong t)
{
...
static int
_get_lower(nn_ptr res, slong n, const fball_t v)
{
    nn_ptr y;
    double err;
    int ok = 0;
    ...
    err = fball_get_fixed(y, n + 1, v);

    if (err < ldexp(1.0, FLINT_BITS - 4))
    {
        /* |true - y| <= err guard ulps */
        if (mpn_sub_1(y, y, n + 1, (ulong) err + 1))
    ...
    ulong dstep)
{
    ...
    for (guard = TAB_BS_GUARD; ; guard *= 2)
    {
        ...
        P.tmp = flint_malloc(4 * depth * sizeof(fball_struct));
        for (k = 0; k < 4 * depth; k++)
            fball_init(P.tmp + k);
        ...
        if (!atanh)
        {
            ...
            fball_div(v, N, D, P.wp);
            fball_mul_2exp_si(v, -((slong) i + e * (Nt - 1)));
            _fball_add_error_2exp(v, -((slong) i + e * Nt));
        }
        else
        {
            ...
            if (i + 1 < FLINT_BITS)
            {
                fball_mul_ui(N, N, (UWORD(1) << (i + 1)) + 1, P.wp);
            }
            else
            {
                fball_set(v, N);
                fball_mul_2exp_si(v, (slong) i + 1);
                fball_add(N, N, v, P.wp);
            }
            fball_div(v, N, E, P.wp);
            fball_mul_2exp_si(v, 1);
            _fball_add_error_2exp(v, 2 - ((slong) i + 1) * (2 * Nt + 1));
        }
