...
void
_fball_neglog_agm(fball_t res, nn_srcptr x, slong n, slong N)
{
    slong wp = n + 3, p, e, m, j, k, kn, qe, cp;
    fball_t r2, q, t, T3, S2, a, b, c;
    nn_ptr tmp;
    ...
    fball_init(r2); fball_init(q); fball_init(t);
    fball_init(T3); fball_init(S2); fball_init(a); fball_init(b);
    fball_init(c);

    /* r^2 = x^2 2^-2e to full precision, q = r^4 to the precision it
       contributes at (2^(-4e) below the leading 1) */
    fball_set_mpn_2exp(t, x, n, -FLINT_BITS * n);
    fball_mul(r2, t, t, wp);
    fball_mul_2exp_si(r2, -2 * e);
    fball_mul(q, r2, r2, FLINT_MAX(2, wp - (4 * e) / FLINT_BITS + 1));
    qe = fball_mag_2exp(q);
    ...
    {
        slong jmax = N + 9;
        fball_struct * pw = flint_malloc((jmax + 1) * sizeof(fball_struct));
        ...
        for (m = 2; ; m++)
        {
            if (m > 2)
            {
                if (m % 2 == 0 && m > 4)
                {
                    ...
                    if (j % 2 == 0)
                        fball_mul(pw + j, pw + j / 2, pw + j / 2, cp);
                    else
                        fball_mul(pw + j, pw + j - 1, q, cp);
                }
                k += m / 2;
                ...
                if (m == 3)
                {
                    ...
                    fball_mul(pw + 2, q, q, cp);
                    fball_set(t, pw + 2);
                }
                else if (m == 4)
                    fball_mul(t, pw + 2, pw + 2, cp);       /* q^4 */
                else
                    fball_mul(t, t, pw + m / 2, cp);
            }
            if (m % 2 == 0)
                fball_add(T3, T3, t, wp);
            else
                fball_add(S2, S2, t, wp);
    ...
    fball_add_error_2exp(T3, kn * qe + 1);
    fball_add_error_2exp(S2, kn * qe + 1);
    ...
    fball_set_ui(t, 1);
    fball_add(S2, S2, t, wp);
    fball_set_mpn_2exp(a, x, n, -FLINT_BITS * n);
    fball_mul(S2, S2, a, wp);
    fball_mul_2exp_si(S2, 1 - e);           /* u = 2 r S_2 */
    fball_mul_2exp_si(T3, 1);
    fball_add(T3, T3, t, wp);               /* theta_3 */
    fball_mul(b, S2, T3, wp);               /* u theta_3 */
    fball_mul(a, S2, S2, wp);
    fball_mul(t, T3, T3, wp);
    fball_add(a, a, t, wp);
    fball_mul_2exp_si(a, -1);               /* (u^2 + theta_3^2)/2 */

    fball_agm(c, a, b, wp);

    /* (pi/4) / agm - e log 2 */
    tmp = flint_malloc(wp * sizeof(ulong));
    fixed_const_pi_div_4(tmp, wp);
    fball_set_mpn_2exp(t, tmp, wp, -FLINT_BITS * wp);
    fball_add_error(t, 1.0, -wp);
    fball_div(c, t, c, wp);
    fixed_const_log2(tmp, wp);
    fball_set_mpn_2exp(t, tmp, wp, -FLINT_BITS * wp);
    fball_add_error(t, 1.0, -wp);
    fball_submul_ui(res, c, t, (ulong) e, wp);
    flint_free(tmp);

    fball_clear(r2); fball_clear(q); fball_clear(t);
    fball_clear(T3); fball_clear(S2); fball_clear(a); fball_clear(b);
    fball_clear(c);
}

void
_fixed_neglog_agm_tune(nn_ptr y, nn_srcptr x, slong n, slong N)
{
    fball_t r;
    double bound;
    fball_init(r);
    _fball_neglog_agm(r, x, n, N);
    bound = fball_get_fixed(y, n, r);
    if (!(bound <= FIXED_NEGLOG_NEWTON_MAX_ERR))
        flint_throw(FLINT_ERROR, "fixed_neglog_agm: error bound %g ulps\n", bound);
    fball_clear(r);
}

void
fixed_neglog_agm(nn_ptr y, nn_srcptr x, slong n)
{
    _fixed_neglog_agm_tune(y, x, n, 0);
}
