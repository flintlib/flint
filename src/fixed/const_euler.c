...
/* Euler's constant by the Brent-McMillan algorithm (the refinement B3
   ...
   below EULER_LEAF terms, fballs (exact until they outgrow the working
   precision) above.
...
{
    slong i;
    fball_struct * tmp = flint_malloc(k * sizeof(fball_struct));
    for (i = 0; i < k; i++)
        fball_init(tmp + i);
    return tmp;
}
...
static void
_tmp_clear(fball_struct * tmp, slong k)
{
    slong i;
    for (i = 0; i < k; i++)
        fball_clear(tmp + i);
...
e1_init(e1_struct * s)
{
    fball_init(&s->P); fball_init(&s->D0); fball_init(&s->D1);
    fball_init(&s->T0); fball_init(&s->T1);
}

static void
e1_clear(e1_struct * s)
{
    fball_clear(&s->P); fball_clear(&s->D0); fball_clear(&s->D1);
    fball_clear(&s->T0); fball_clear(&s->T1);
}
...
static void e1_bsplit(e1_struct * s, slong a, slong b, ulong m, int need,
    slong wp, fball_struct * tmp, fball_struct * sc);
static void e2_bsplit(fball_t P, fball_t Q, fball_t T, slong a, slong b,
    ulong m, int need, slong wp, fball_struct * tmp, fball_struct * u);

typedef struct
{
    fball_struct * s;       /* e1: an e1_struct; e2: P, Q, T */
    fball_struct * tmp, * sc;
    slong a, b, wp;
    ulong m;
    int need, e2;
...
_euler_job(void * arg)
{
    ...
    fball_struct * tmp = J->tmp, * sc = J->sc;
...
/* the rule of FIXED_PAR_CAP for [a, b): 0 serial, 1 fork the halves,
...
_euler_par(slong a, slong b, ulong m, slong wp, int e2)
{
    ...
    return (est > FIXED_PAR_CAP * FLINT_BITS * (double) wp) ? 2 : 1;
}

/* the halves [a, mid), [mid, b) into s and R (1, need) on two threads */
static void
_euler_fork(fball_struct * s, fball_struct * R, slong a, slong mid,
    slong b, ulong m, int need, slong wp, fball_struct * tmp,
    fball_struct * sc, int e2)
{
    euler_job L, Rj;
    ...
    _fixed_parallel_pair(_euler_job, &L, _euler_job, &Rj);
}
...
_e1_merge_x(void * arg)
{
    ...
    fball_struct * q0 = M->sc, * q1 = M->sc + 1, * u = M->sc + 2,
        * v = M->sc + 3;
    slong wp = M->wp;

    if (M->round == 0)
    {
        fball_mul(q0, &R->D0, &R->D0, wp);
        fball_mul(u, &s->P, &R->T1, wp);
        fball_mul_2exp_si(u, M->sh);
    }
    else
    {
        fball_mul(v, q0, &s->T1, wp);
        fball_add(u, u, v, wp);
        fball_mul(v, q1, &s->T0, wp);
        fball_add(&s->T1, u, v, wp);
    }
}
...
_e1_merge_y(void * arg)
{
    e1_merge_struct * M = (e1_merge_struct *) arg;
    e1_struct * s = M->s, * R = M->R;
    fball_struct * q0 = M->sc, * q1 = M->sc + 1, * w = M->sc + 4,
        * v = M->sc + 5;
    slong wp = M->wp;

    if (M->round == 0)
    {
        fball_mul(q1, &R->D0, &R->D1, wp);
        fball_mul_2exp_si(q1, 1);
        fball_mul(w, &s->P, &R->T0, wp);
        fball_mul_2exp_si(w, M->sh);
    }
    else
    {
        fball_mul(v, q0, &s->T0, wp);
        fball_add(w, w, v, wp);
        fball_mul(v, &s->D0, &R->D1, wp);
        fball_mul(&s->D1, &s->D1, &R->D0, wp);
        fball_add(&s->D1, &s->D1, v, wp);
        fball_mul(&s->D0, &s->D0, &R->D0, wp);
    }
}
    ...
    fball_struct * sc)
{
    ...
    M.round = 0;
    _fixed_parallel_pair(_e1_merge_x, &M, _e1_merge_y, &M);
    M.round = 1;
    _fixed_parallel_pair(_e1_merge_x, &M, _e1_merge_y, &M);
    fball_swap(&s->T0, sc + 4);
    if (need)
        fball_mul(&s->P, &s->P, &R->P, wp);
}

/* the same in fballs at wp limbs; P only when need (not on the right
...
e1_bsplit(e1_struct * s, slong a, slong b, ulong m, int need, slong wp,
    fball_struct * tmp, fball_struct * sc)
{
    if (b - a <= EULER_LEAF)
    {
        e1z_struct z;
        ...
        _fball_set_fmpz(&s->P, z.P);
        _fball_set_fmpz(&s->D0, z.D0);
        _fball_set_fmpz(&s->D1, z.D1);
        _fball_set_fmpz(&s->T0, z.T0);
        _fball_set_fmpz(&s->T1, z.T1);
        e1z_clear(&z);
    }
    else
    {
        e1_struct * R = (e1_struct *) tmp;
        fball_struct * q0 = sc, * q1 = sc + 1, * u = sc + 2, * v = sc + 3;
        slong mid = a + (b - a) / 2;
        ...
        if (par == 1)
            _euler_fork((fball_struct *) s, (fball_struct *) R, a, mid, b,
        ...
        /* T0 = P1 T2_0 + Q2_0 T1_0 */
        fball_mul(u, &s->P, &R->T0, wp);
        fball_mul_2exp_si(u, sh);
        fball_mul(v, q0, &s->T0, wp);
        fball_add(&s->T0, u, v, wp);

        /* D = D1 D2 */
        fball_mul(u, &s->D0, &R->D1, wp);
        fball_mul(v, &s->D1, &R->D0, wp);
        fball_add(&s->D1, u, v, wp);
...
static void
e2_bsplit(fball_t P, fball_t Q, fball_t T, slong a, slong b, ulong m,
    int need, slong wp, fball_struct * tmp, fball_struct * u)
{
    if (b - a <= EULER_LEAF)
    {
        fmpz_t zP, zQ, zT;
        ...
        _fball_set_fmpz(P, zP);
        _fball_set_fmpz(Q, zQ);
        _fball_set_fmpz(T, zT);
        fmpz_clear(zP);
        fmpz_clear(zQ);
        fmpz_clear(zT);
    }
    ...
    {
        fball_struct * P2 = tmp, * Q2 = tmp + 1, * T2 = tmp + 2;
        slong mid = a + (b - a) / 2;
        ...
        _fball_pqt_merge(P, Q, T, P2, Q2, T2, need, wp, par == 2);

        if (T2->alloc > EULER_KEEP)
        {
            fball_clear(P2); fball_init(P2);
            fball_clear(Q2); fball_init(Q2);
            fball_clear(T2); fball_init(T2);
        }
    }
}
...
_euler_log_worker(slong j, void * arg)
{
    ...
    if (W->L->np == 1)
    {
        fball_const_log2(W->v + i, W->wp);
        return;
    }
    fmpz_init_set_ui(u, W->L->u[i]);
    fmpz_init_set_ui(v, W->L->v[i]);
    _fball_log_ratio_zuniga(W->v + i, u, v, W->wp);
    fmpz_clear(u);
    fmpz_clear(v);
}
...
static void
_euler_log(fball_t res, ulong m, const euler_logset_t * L, slong wp)
{
    ...
    W.L = L;
    W.wp = wp;
    _fixed_parallel_tasks(_euler_log_worker, &W, cnt);
    ...
    for (i = 0; i < L->np; i++)
    {
        ...
        {
            if (w[i] < 0)
                fball_submul_ui(res, res, v + i, FLINT_UABS(w[i]), wp);
            else
                fball_addmul_ui(res, res, v + i, FLINT_UABS(w[i]), wp);
        }
        fball_clear(v + i);
    }
...
void
_fball_const_euler(fball_t res, slong n, int set)
{
    ...
    fball_mul(t, &S.T0, &S.D1, wp);
    fball_mul_2exp_si(t, 1);
    fball_mul(u, &S.T1, &S.D0, wp);
    fball_sub(t, t, u, wp);
    fball_mul(u, &S.D0, &S.D0, wp);                 /* Q0 = D0^2 */
    fball_mul(A, u, u, wp2);                        /* Q0^2 */
    fball_add(&S.T0, &S.T0, u, wp);                 /* X */
    fball_mul(B, &S.T0, &S.T0, wp2);                /* X^2 */
    fball_mul(u, &S.T0, &S.D0, wp);
    fball_mul_2exp_si(u, 1);
    e1_clear(&S);
    fball_div(res, t, u, wp);
    fball_clear(t);
    fball_clear(u);
    fball_init(t);
    fball_init(u);
    ...
    {
        fball_struct PQT[3];    /* consecutive, for the threaded split */
        ...
        _tmp_clear(tmp, k);
        fball_clear(PQT);
        fball_mul(A, A, PQT + 2, wp2);
        fball_mul(B, B, PQT + 1, wp2);
        fball_clear(PQT + 1);
        fball_clear(PQT + 2);
        fball_div(t, A, B, wp2);
        fball_sub(res, res, t, wp);
    }
    ...
    _fball_add_error_2exp(res, (slong) ceil(4.6 - 11.5415 * (double) m));
...
void
fball_const_euler(fball_t res, slong n)
{
    if (n + 1 < EULER_TAB_LIMBS)
    {
        /* gamma in [1/2, 1): the table as a fraction, one ulp */
        fball_set_mpn_2exp(res, arb_hypgeom_gamma_tab_limbs + EULER_TAB_LIMBS,
            EULER_TAB_LIMBS, -FLINT_BITS * EULER_TAB_LIMBS);
        fball_add_error(res, 1.0, -EULER_TAB_LIMBS);
        return;
    }

    _fball_const_euler(res, n, -1);
}

/* fixed_const_euler: verified floor truncations of gamma from a
   per-thread cache, exactly as fixed_const_log2 (see const_pi.c) */

static FLINT_TLS_PREFIX nn_ptr _fixed_euler_cache = NULL;
static FLINT_TLS_PREFIX slong _fixed_euler_n = 0;
static FLINT_TLS_PREFIX int _fixed_euler_cleanup_registered = 0;

void
_fixed_const_euler_clear(void)
{
    flint_free(_fixed_euler_cache);
    _fixed_euler_cache = NULL;
    _fixed_euler_n = 0;
}

static void
_fixed_euler_cleanup(void)
{
    _fixed_const_euler_clear();
    _fixed_euler_cleanup_registered = 0;
}

void
fixed_const_euler(nn_ptr y, slong n)
{
    FLINT_ASSERT(n >= 1);

    if (n > _fixed_euler_n)
    {
        slong nc = FLINT_MAX(FLINT_MAX(n, 2 * _fixed_euler_n), 16);
        slong guard = 3;
        ...
        flint_free(_fixed_euler_cache);
        _fixed_euler_cache = e;
        _fixed_euler_n = nc;

        if (!_fixed_euler_cleanup_registered)
        {
            flint_register_cleanup_function(_fixed_euler_cleanup);
            _fixed_euler_cleanup_registered = 1;
        }
    }

    flint_mpn_copyi(y, _fixed_euler_cache + (_fixed_euler_n - n), n);
}
