...
/* k-th roots and reciprocal k-th roots (k >= 4; the square and cube
   ...
   written in fball arithmetic, which handles the exponents and the
...
static void
_pow_ui(fball_t res, const fball_t x, ulong m, slong n)
{
    if (m == 1)
        fball_set(res, x);
    else
...
static void
_root_step(fball_t res, const fball_t v, const fball_t z0, ulong k,
    slong n, int r, int recip, slong rz0, const root_coeffs_struct * rc)
{
    ...
    if (rz0 < -8)
    {
        ...
        slong ub = FLINT_MAX(kb + 1 + rz0, fball_rel_2exp(zk_first ? T : S) + 1) + 1;
        ...
        fball_set_ui(u, 1);
        if (zk_first)
            fball_submul_bounded(u, u, v, T, E, pu);
        else
            fball_submul_bounded(u, u, S, z0, E, pu);
    }
    else
    {
        if (zk_first)
    ...
    uexp = fball_mag_2exp(u);

    if (uexp > -2)
        flint_throw(FLINT_ERROR, "fball_root_ui: iteration failed\n");
    ...
    /* the series: res = base (1 + sum_{j<r} c_j u^j) as
       ...
       precision) as a shift, one fball_div_ui or the chain of the
       generic denominator, and one full product applies the base. */
    {
        const fball_struct * base = recip ? z0 : S;
        fball_struct pw[ROOT_MAX_ORDER];
        fball_t W;
        ...
        for (j = 1; j < r; j++)
        {
            ...
            else
            {
                fball_init(pw + j);
                if (j % 2 == 0)
                    fball_mul(pw + j, (j / 2 == 1) ? u : pw + j / 2,
                        (j / 2 == 1) ? u : pw + j / 2, cp);
                else
                    fball_mul(pw + j, pw + j - 1, u, cp);
                uj = pw + j;
            }
            ...
            {
                if (j == 1)
                    fball_mul_ui(W, uj, rc->c[j], cp);
                else
                    fball_addmul_ui(W, W, uj, rc->c[j], cw);
                continue;
            }
            else
            {
                ...
                fball_set_ui(coef, 1);
                for (i = j + 1; i < r; i++)
                    fball_mul_ui(coef, coef, (ulong) i, 32);
                for (i = 0; i < r - 1 - j; i++)
                    fball_mul_ui(coef, coef, k, 32);
                for (i = 0; i < j; i++)
                    fball_mul_ui(coef, coef, bk + (ulong) i * k, 32);
                fball_mul(T, uj, coef, cp);
                fball_clear(coef);
            }
            if (j == 1)
                fball_swap(W, T);
        ...
        if (rc->Dfits && rc->D == 1)
            ...
            fball_mul_2exp_si(W, -(slong) FLINT_BIT_COUNT(rc->D) + 1);
        else if (rc->Dfits)
            fball_div_ui(W, W, rc->D, cw);
        else
        {
            fball_div_ui(W, W, rc->fac, cw);
            for (j = 1; j < r; j++)
                fball_div_ui(W, W, k, cw);
        }
        fball_mul(T, base, W, p);
        fball_add(res, base, T, p);
        fball_clear(W);

        /* the tail: below 2 c_r |u|^r |base| with c_r <= 1 */
        fball_add_error_2exp(res, r * uexp + fball_mag_2exp(base) + 1);
    }
...
static void
_seed(fball_t z, double md, slong ep, ulong k)
{
    ...
    fball_set_mpn_2exp(z, &mm, 1, (slong) e0 - FLINT_BITS);
}
...
static void
_rroot_rec(fball_t z, slong * rz, const fball_t v, double md, slong ep,
...
void
_fball_root_ui_order(fball_t res, const fball_t x, ulong k, slong n,
    int r, int recip)
{
    fball_t v, z;
    ...
    if (x->size == 0 || x->negative)
        flint_throw(FLINT_ERROR, "fball_root_ui: need x > 0\n");
    if (k < 2 || k >= (UWORD(1) << 40))
        flint_throw(FLINT_ERROR, "fball_root_ui: k out of range\n");

    kb = FLINT_BIT_COUNT(k);
    if (r == 0)
        r = (kb <= 3) ? 3 : 4;
    if (r < 2 || r >= ROOT_MAX_ORDER)
        flint_throw(FLINT_ERROR, "fball_root_ui: order out of range\n");
    ...
    fball_mul_2exp_si(res, recip ? -q : q);
...
void
fball_root_ui(fball_t res, const fball_t x, ulong k, slong n)
{
    if (k == 0)
        flint_throw(FLINT_ERROR, "fball_root_ui: k = 0\n");
    ...
    else if (k == 2)
    {
        ...
        _fball_root_ui_order(res, x, k, n, 0, 0);
}

void
fball_rroot_ui(fball_t res, const fball_t x, ulong k, slong n)
{
    if (k == 0)
        flint_throw(FLINT_ERROR, "fball_rroot_ui: k = 0\n");
    ...
    else if (k == 1)
    {
        ...
        fball_init(one);
        fball_set_ui(one, 1);
        fball_div(res, one, x, n);
        fball_clear(one);
    }
    else
        _fball_root_ui_order(res, x, k, n, 0, 1);
}
