...
typedef void (* constant_func) (fball_t, slong);

static const constant_func funcs[] = {
    fball_const_e,
    fball_const_log10,
    fball_const_catalan,
    fball_const_zeta3,
    fball_const_zeta5,
    fball_const_gamma_1_3,
    fball_const_gamma_1_4,
};
...
TEST_FUNCTION_START(fixed_const_misc, state)
{
    slong iter, i;
    arb_t ref[NCONST];
    ...
    /* fball_root_ui and fball_rroot_ui against arb_root_ui */
    for (iter = 0; iter < 200 * flint_test_multiplier(); iter++)
    {
        ...
        fball_const_log2(x, n);
        fball_mul_ui(x, x, 1 + n_randint(state, 1000), n);
        fball_mul_2exp_si(x, (slong) n_randint(state, 400) - 200);

        if (recip)
            fball_rroot_ui(y, x, k, n);
        else
            fball_root_ui(y, x, k, n);
        fball_get_arb(a, x);
        fball_get_arb(b, y);
        ...
        if (!arb_overlaps(b, c))
        {
            flint_printf("FAIL: fball_root_ui k = %wu, n = %wd\n", k, n);
        ...
        if (arb_rel_accuracy_bits(b)
            < FLINT_MIN(arb_rel_accuracy_bits(a), FLINT_BITS * (n - 1))
                - ((k <= 3) ? 100 : 32))
        {
            flint_printf("FAIL: fball_root_ui accuracy, k = %wu, n = %wd\n",
                k, n);
