...
/* fixed_const_log2 and fixed_const_log2 must return EXACTLY
...
static int
_floor_matches_l2(nn_srcptr y, slong n, const arb_t exact)
{
    fmpz_t f, g;
    arb_t t;
    int ok;
...
TEST_FUNCTION_START(fixed_const_log2, state)
{
    ...
    for (iter = 0; iter < 2; iter++)
    {
        for (i = 0; i < 12; i++)
        {
            ...
            arb_const_log2(ref, FLINT_BITS * n + 192);
            fixed_const_log2(y, n);
            if (!_floor_matches_l2(y, n, ref))
