...
/* Tune fixed_sin_cos_diophantine: for a size n, sweep the number of
...
precomp_diophantine(slong num, slong n)
{
    ...
    t0 = wall_us();
    _fixed_atan_gauss_ensure(num, n + 1);
    t1 = wall_us();
    return t1 - t0;
}
...
precomp_bitwise(slong n)
{
    ...
    _fixed_atans_clear();
    t0 = wall_us();
    fixed_sin_cos_bitwise_rs(y, scratch_cos, x, n, 0);
    t1 = wall_us();
    flint_free(x);
    flint_free(y);
    return t1 - t0;
...
{
    _fixed_sin_cos_diophantine_tune(y, scratch_cos, x, n, num, w);
}
...
{
    const fixed_rel_struct * tab = fixed_rel_table(1, num);
    slong wr, j;
    slong * rel;
    nn_ptr base, t, tmp;
    double r, eps_min;
    ...
    {
        if (rel[j] > 0)
            mpn_submul_1(t, _fixed_atan_gauss_entry(j, n + 1), n + 2, rel[j]);
        else if (rel[j] < 0)
            mpn_addmul_1(t, _fixed_atan_gauss_entry(j, n + 1), n + 2, -rel[j]);
    }
...
main(int argc, char * argv[])
{
    ...
    flint_printf("bitwise (r = %d): precomputation %.0f us, per call %.1f us\n\n",
        fixed_trig_bitwise_rs_default_r(n), pre_b, tb);
    ...
    for (in = 0; in < nnum; in++)
    {
        ...
        fixed_rel_table(1, num);   /* table generation outside the timing */
