...
/* atan(p/q) and atanh(p/q), 0 < p < q given as mpn integers, by binary
   splitting in fball arithmetic (the replacement for
   ...
   ERRORS.  Merges and the final division are fball operations; the
...
typedef struct
{
    ...
    slong bufn;
    fball_struct * Qpow;    /* Qpow[i] = Q^(L 2^i) */
    fball_struct * Ppow;    /* Ppow[i] = P^(L 2^i) (unless p = 1) */
    fball_struct * tmp;     /* merge temporaries, 5 per level */
...
static void bsplit_den(fball_t N, fball_t D, fball_t E, slong a0, slong k,
    slong lev, const atf_t * A, int need_d);
static void bsplit(fball_t N, fball_t D, slong a0, slong k, slong lev,
    const atf_t * A);

/* a subtree as a job for _fixed_parallel_pair (E == NULL: the power
...
_atf_job(void * arg)
{
    ...
    if (J->own)
    {
        ...
        A2.tmp = flint_malloc(nt * sizeof(fball_struct));
        for (i = 0; i < nt; i++)
            fball_init(A2.tmp + i);
        A = &A2;
    }
...
_atf_par(slong k, const atf_t * A)
{
    ...
    return ((double) (k * A->L) * A->pt
        > FIXED_PAR_CAP * FLINT_BITS * (double) A->wp) ? 2 : 1;
}

/* the halves of the 2h leaves from a0 at level lev */
static void
_atf_halves(fball_struct * N, fball_struct * D, fball_struct * E,
    fball_struct * N2, fball_struct * D2, fball_struct * E2, slong a0,
    slong h, slong lev, int need_d, int den, int fork, const atf_t * A)
{
    if (fork)
    {
        ...
        _fixed_parallel_pair(_atf_job, &L, _atf_job, &R);
    }
    else if (den)
    {
...
typedef struct
{
    fball_struct * N, * D, * E, * N2, * D2, * E2, * T, * U;
    const fball_struct * qpow, * ppow;
    int need_d;
    slong wp;
}
atf_merge_struct;
...
_atf_merge_x(void * arg)
{
    ...
    if (M->E != NULL)
    {
        fball_mul(M->T, M->N, M->E2, M->wp);
        fball_mul(M->E, M->E, M->E2, M->wp);
    }
    else
    {
        fball_mul(M->T, M->N, M->D2, M->wp);
        fball_mul(M->T, M->T, M->qpow, M->wp);
    }
}
...
{
    atf_merge_struct * M = (atf_merge_struct *) arg;
    fball_mul(M->U, M->N2, M->D, M->wp);
    if (M->ppow != NULL)
        fball_mul(M->U, M->U, M->ppow, M->wp);
    if (M->need_d)
        fball_mul(M->D, M->D, M->D2, M->wp);
}
...
{
    if (par)
        _fixed_parallel_pair(_atf_merge_x, M, _atf_merge_y, M);
    else
    {
        _atf_merge_x(M);
        _atf_merge_y(M);
    ...
    if (sub)
        fball_sub(M->N, M->T, M->U, M->wp);
    else
        fball_add(M->N, M->T, M->U, M->wp);
}
...
static void
bsplit_den(fball_t N, fball_t D, fball_t E, slong a0, slong k, slong lev,
    const atf_t * A, int need_d)
{
    if (k == 1)
    {
        ...
        fball_set_mpn_2exp(N, Nb, nl, 0);
        fball_set_mpn_2exp(D, Db, dl, 0);
        fball_set_mpn_2exp(E, Eb, el, 0);
    }
    else
    {
        fball_struct * N2 = A->tmp + 5 * lev, * D2 = N2 + 1, * T = N2 + 2,
            * U = N2 + 3, * E2 = N2 + 4;
        slong h = k / 2, wp = A->wp;
...
/* the range of k leaves starting at leaf index a0 (k a power of two),
   ...
   at A->tmp + 5 lev */
static void
bsplit(fball_t N, fball_t D, slong a0, slong k, slong lev,
    const atf_t * A)
{
    if (k == 1)
    {
    ...
    else
    {
        fball_struct * N2 = A->tmp + 5 * lev, * D2 = N2 + 1, * T = N2 + 2,
            * U = N2 + 3;
        slong h = k / 2, wp = A->wp;
        fball_struct * qpow = A->Qpow + (lev - 1);   /* Q^(L h) */
...
static void
_powtab(fball_struct * pow, nn_srcptr base, slong bn, slong L, slong J,
    slong n)
{
    nn_ptr w, v, w0;
    slong wl, vl, t, i;
    ...
    fball_set_mpn_2exp(pow, w, wl, 0);
    for (i = 1; i < J; i++)
        fball_mul(pow + i, pow + i - 1, pow + i - 1, n);
...
void
fball_atan_frac_bsplit(fball_t res, nn_srcptr p, slong pn, nn_srcptr q,
    slong qn, int hyperbolic, slong n)
{
    ...
    if (qn < pn || (qn == pn && mpn_cmp(p, q, pn) >= 0))
        flint_throw(FLINT_ERROR, "fball_atan_frac_bsplit: need p < q\n");
    ...
    if (!(lx >= 0.015))
        flint_throw(FLINT_ERROR,
            "fball_atan_frac_bsplit: p/q too close to 1\n");
    ...
    if (K == 1)
    {
        ...
        fball_set_mpn_2exp(N, Nb, nl, 0);
        fball_set_mpn_2exp(D, Eb, el, 0);
        fball_set_mpn_2exp(T, pq, pqn, 0);
        fball_mul(N, N, T, n);
        fball_div(res, N, D, n);
    }
    else
    {
        ...
        A.Qpow = flint_malloc(2 * J * sizeof(fball_struct));
        A.Ppow = A.Qpow + J;
        A.tmp = flint_malloc(5 * (J + 1) * sizeof(fball_struct));
        for (i = 0; i < 2 * J; i++)
            fball_init(A.Qpow + i);
        for (i = 0; i < 5 * (J + 1); i++)
            fball_init(A.tmp + i);
        ...
        if (powtab)
        {
            ...
            fball_mul(T, A.Qpow + (J - 1), A.Qpow + (J - 1), n);
            fball_mul(D, D, T, n);
        }
        else
        {
    ...
    }
    {
        ...
        fball_add_error(res, ldexp(1.0, (int) (t - qq * FLINT_BITS)), qq);
    }
