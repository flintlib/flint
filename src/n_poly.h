...
N_POLY_INLINE void n_poly_fit_length(n_poly_t A, slong len) { if (len > A->alloc) n_poly_realloc(A, len); }
...
N_POLY_INLINE void n_poly_swap(n_poly_t A, n_poly_t B) { FLINT_SWAP(n_poly_struct, *A, *B); }
...
N_POLY_INLINE slong n_poly_degree(const n_poly_t A) { FLINT_ASSERT(A->length >= 0); return A->length - 1; }
N_POLY_INLINE mp_limb_t n_poly_lead(const n_poly_t A) { FLINT_ASSERT(A->length > 0); return A->coeffs[A->length - 1]; }

N_POLY_INLINE int n_poly_is_zero(const n_poly_t poly) { return poly->length == 0; }
N_POLY_INLINE int n_poly_is_one(const n_poly_t A) { return A->length == 1 && A->coeffs[0] == 1; }

N_POLY_INLINE void n_poly_zero(n_poly_t res) { res->length = 0; }
N_POLY_INLINE void n_poly_one(n_poly_t A) { n_poly_fit_length(A, 1); A->length = 1; A->coeffs[0] = 1; }
...
N_POLY_INLINE void n_poly_mod_taylor_shift(n_poly_t g, mp_limb_t c, nmod_t mod) { _nmod_poly_taylor_shift(g->coeffs, c, g->length, mod); }

N_POLY_INLINE ulong n_poly_get_coeff(const n_poly_t poly, slong j) { return (j >= poly->length) ? 0 : poly->coeffs[j]; }
...
N_POLY_INLINE
void _n_poly_mod_scalar_mul_nmod(n_poly_t A, const n_poly_t B, mp_limb_t c, nmod_t mod)
{
...
N_POLY_INLINE void _n_poly_mod_scalar_mul_nmod_inplace(n_poly_t A, mp_limb_t c, nmod_t mod) { _nmod_vec_scalar_mul_nmod(A->coeffs, A->coeffs, A->length, c, mod); }

void n_poly_mod_scalar_mul_ui(n_poly_t A, const n_poly_t B, mp_limb_t c, nmod_t ctx);

mp_limb_t n_poly_mod_eval_step2(n_poly_t Acur, const n_poly_t Ainc, nmod_t mod);

N_POLY_INLINE mp_limb_t n_poly_mod_evaluate_nmod(const n_poly_t A, mp_limb_t c, nmod_t mod) { return _nmod_poly_evaluate_nmod(A->coeffs, A->length, c, mod); }
...
void n_poly_mod_shift_left_scalar_addmul(n_poly_t A, slong k, mp_limb_t c, nmod_t mod);

void n_poly_mod_addmul_linear(n_poly_t A, const n_poly_t B, const n_poly_t C, mp_limb_t d1, mp_limb_t d0, nmod_t mod);

void n_poly_mod_scalar_addmul_nmod(n_poly_t A, const n_poly_t B, const n_poly_t C, mp_limb_t d0, nmod_t ctx);
...
void n_poly_mod_eval2_pow(mp_limb_t * vp, mp_limb_t * vm, const n_poly_t P, n_poly_t alphapow, nmod_t ctx);

mp_limb_t n_poly_mod_div_root(n_poly_t Q, const n_poly_t A, mp_limb_t c, nmod_t ctx);
...
N_POLY_INLINE nmod_t fq_nmod_ctx_mod(const fq_nmod_ctx_t ctx) { return ctx->modulus->mod; }
...
N_POLY_INLINE int n_fq_is_one(const mp_limb_t * a, const fq_nmod_ctx_t ctx) { return _n_fq_is_one(a, fq_nmod_ctx_degree(ctx)); }
...
void n_fq_gen(mp_limb_t * a, const fq_nmod_ctx_t ctx);

N_POLY_INLINE
void _n_fq_set(mp_limb_t * a /* length d */, const mp_limb_t * b /* length d */, slong d)
{
    slong i = 0;
    do {
        a[i] = b[i];
...
N_POLY_INLINE
void _n_fq_swap(mp_limb_t * a /* length d */, mp_limb_t * b /* length d */, slong d)
{
    slong i = 0;
    do {
...
N_POLY_INLINE
int _n_fq_equal(mp_limb_t * a /* length d */, const mp_limb_t * b /* length d */, slong d)
{
    slong i = 0;
    do {
        if (a[i] != b[i])
...
int n_fq_equal_fq_nmod(const mp_limb_t * a, const fq_nmod_t b, const fq_nmod_ctx_t ctx);

int n_fq_is_canonical(const mp_limb_t * a, const fq_nmod_ctx_t ctx);

void n_fq_randtest_not_zero(mp_limb_t * a, flint_rand_t state, const fq_nmod_ctx_t ctx);

char * n_fq_get_str_pretty(const mp_limb_t * a, const fq_nmod_ctx_t ctx);
...
void n_fq_get_fq_nmod(fq_nmod_t a, const mp_limb_t * b, const fq_nmod_ctx_t ctx);
void n_fq_set_fq_nmod(mp_limb_t * a, const fq_nmod_t b, const fq_nmod_ctx_t ctx);

void n_fq_get_n_poly(n_poly_t a, const mp_limb_t * b, const fq_nmod_ctx_t ctx);

void _n_fq_set_n_poly(mp_limb_t * a, const mp_limb_t * bcoeffs, slong blen, const fq_nmod_ctx_t ctx);

void n_fq_add_si(mp_limb_t * a, const mp_limb_t * b, slong c, const fq_nmod_ctx_t ctx);
void n_fq_add_fq_nmod(mp_limb_t * a, const mp_limb_t * b, const fq_nmod_t c, const fq_nmod_ctx_t ctx);

void n_fq_sub_fq_nmod(mp_limb_t * a, const mp_limb_t * b, const fq_nmod_t c, const fq_nmod_ctx_t ctx);

N_POLY_INLINE
void _n_fq_add(mp_limb_t * a /* length d */, const mp_limb_t * b /* length d */, const mp_limb_t * c /* length d */, slong d, nmod_t mod)
{
    FLINT_ASSERT(d > 0);
...
N_POLY_INLINE
void _n_fq_sub(mp_limb_t * a /* length d */, const mp_limb_t * b /* length d */, const mp_limb_t * c /* length d */, slong d, nmod_t mod)
{
    FLINT_ASSERT(d > 0);
...
N_POLY_INLINE
void _n_fq_neg(mp_limb_t * a, const mp_limb_t * b, slong d, nmod_t mod)
{
    FLINT_ASSERT(d > 0);
...
N_POLY_INLINE
void n_fq_add(mp_limb_t * a /* length d */, const mp_limb_t * b /* length d */, const mp_limb_t * c /* length d */, const fq_nmod_ctx_t ctx)
{
...
N_POLY_INLINE
void n_fq_sub(mp_limb_t * a /* length d */, const mp_limb_t * b /* length d */, const mp_limb_t * c /* length d */, const fq_nmod_ctx_t ctx)
{
    slong d = fq_nmod_ctx_degree(ctx);
    FLINT_ASSERT(d > 0);
...
N_POLY_INLINE
void _n_fq_mul_ui(mp_limb_t * a /* length d */, const mp_limb_t * b /* length d */, mp_limb_t c, slong d, nmod_t mod)
{
    if (c >= mod.n)
        NMOD_RED(c, c, mod);
...
void _n_fq_madd2(mp_limb_t * a /* length 2d-1 */, const mp_limb_t * b /* length d */, const mp_limb_t * c /* length d */, const fq_nmod_ctx_t ctx, mp_limb_t * t /* length 2d */);

void _n_fq_mul2(mp_limb_t * t /* length 2d-1 */, const mp_limb_t * b /* length d */, const mp_limb_t * c /* length d */, const fq_nmod_ctx_t ctx);

#define N_FQ_REDUCE_ITCH 2
void _n_fq_reduce(mp_limb_t * a, mp_limb_t * b, slong blen, const fq_nmod_ctx_t ctx, mp_limb_t * t);

/* same itch as reduce */
N_POLY_INLINE
void _n_fq_reduce2(mp_limb_t * a /* length d */, mp_limb_t * b /* length 2d-1 */, const fq_nmod_ctx_t ctx, mp_limb_t * t)          /* length 2d */
{
...
N_POLY_INLINE
void _n_fq_mul(mp_limb_t * a /* length d */, const mp_limb_t * b /* length d */, const mp_limb_t * c /* length d */, const fq_nmod_ctx_t ctx, mp_limb_t * t)          /* length 4d */
{
    slong d = fq_nmod_ctx_degree(ctx);
...
N_POLY_INLINE
void _n_fq_addmul(mp_limb_t * a /* length d */, const mp_limb_t * b /* length d */, const mp_limb_t * c /* length d */, const mp_limb_t * e /* length d */, const fq_nmod_ctx_t ctx, mp_limb_t * t)          /* length 4d */
{
    slong d = fq_nmod_ctx_degree(ctx);
...
void _n_fq_reduce2_lazy1(mp_limb_t * a /* length 6d, 2d used */, slong d, nmod_t ctx);
void _n_fq_reduce2_lazy2(mp_limb_t * a /* length 6d, 4d used */, slong d, nmod_t ctx);
void _n_fq_reduce2_lazy3(mp_limb_t * a /* length 6d */, slong d, nmod_t ctx);

void _n_fq_madd2_lazy1(mp_limb_t * a /* length 6d, 2d used */, const mp_limb_t * b /* length d */, const mp_limb_t * c /* length d */, slong d);
void _n_fq_madd2_lazy2(mp_limb_t * a /* length 6d, 4d used */, const mp_limb_t * b /* length d */, const mp_limb_t * c /* length d */, slong d);
void _n_fq_madd2_lazy3(mp_limb_t * a /* length 6d */, const mp_limb_t * b /* length d */, const mp_limb_t * c /* length d */, slong d);

void _n_fq_mul2_lazy1(mp_limb_t * a /* length 6d, 2d used */, const mp_limb_t * b /* length d */, const mp_limb_t * c /* length d */, slong d);
void _n_fq_mul2_lazy2(mp_limb_t * a /* length 6d, 4d used */, const mp_limb_t * b /* length d */, const mp_limb_t * c /* length d */, slong d);
void _n_fq_mul2_lazy3(mp_limb_t * a /* length 6d */, const mp_limb_t * b /* length d */, const mp_limb_t * c /* length d */, slong d);

#define N_FQ_INV_ITCH 1
void _n_fq_inv(mp_limb_t * a, const mp_limb_t * b, const fq_nmod_ctx_t ctx, mp_limb_t * t);

#define N_FQ_MUL_INV_ITCH FLINT_MAX(N_FQ_MUL_ITCH, N_FQ_INV_ITCH)

void _n_fq_pow_ui(mp_limb_t * a, const mp_limb_t * b, ulong e, const fq_nmod_ctx_t ctx);
void n_fq_pow_ui(mp_limb_t * a, const mp_limb_t * b, ulong e, const fq_nmod_ctx_t ctx);
void n_fq_pow_fmpz(mp_limb_t * a, const mp_limb_t * b, const fmpz_t e, const fq_nmod_ctx_t ctx);

void n_fq_mul_fq_nmod(mp_limb_t * a, const mp_limb_t * b, const fq_nmod_t c, const fq_nmod_ctx_t ctx);
void n_fq_mul(mp_limb_t * a, const mp_limb_t * b, const mp_limb_t * c, const fq_nmod_ctx_t ctx);

void n_fq_inv(mp_limb_t * a, const mp_limb_t * b, const fq_nmod_ctx_t ctx);

void n_fq_addmul(mp_limb_t * a, const mp_limb_t * b, const mp_limb_t * c, const mp_limb_t * d, const fq_nmod_ctx_t ctx);
...
N_POLY_INLINE void n_fq_poly_one(n_fq_poly_t A, const fq_nmod_ctx_t ctx) { _n_fq_poly_one(A, fq_nmod_ctx_degree(ctx)); }
...
void n_fq_poly_get_coeff_n_fq(mp_limb_t * c, const n_poly_t A, slong e, const fq_nmod_ctx_t ctx);

void n_fq_poly_set_coeff_fq_nmod(n_poly_t A, slong j, const fq_nmod_t c, const fq_nmod_ctx_t ctx);
void n_fq_poly_set_coeff_n_fq(n_poly_t A, slong j, const mp_limb_t * c, const fq_nmod_ctx_t ctx);

void n_fq_poly_scalar_mul_ui(n_poly_t A, const n_poly_t B, ulong c, const fq_nmod_ctx_t ctx);
void n_fq_poly_scalar_mul_n_fq(n_poly_t A, const n_poly_t B, const mp_limb_t * c, const fq_nmod_ctx_t ctx);

void n_fq_poly_scalar_addmul_n_fq(n_fq_poly_t A, const n_fq_poly_t B, const n_fq_poly_t C, const mp_limb_t * d, const fq_nmod_ctx_t ctx);

void n_fq_poly_shift_left_scalar_submul(n_poly_t A, slong k, const mp_limb_t * c, const fq_nmod_ctx_t ctx);

void n_fq_poly_evaluate_fq_nmod(fq_nmod_t e, const n_poly_t A, const fq_nmod_t c, const fq_nmod_ctx_t ctx);
void n_fq_poly_evaluate_n_fq(mp_limb_t * e, const n_poly_t A, const mp_limb_t * c, const fq_nmod_ctx_t ctx);

void n_fq_poly_get_fq_nmod_poly(fq_nmod_poly_t A, const n_poly_t B, const fq_nmod_ctx_t ctx);

void n_fq_poly_set_n_fq(n_poly_t A, const mp_limb_t * c, const fq_nmod_ctx_t ctx);
...
void _n_fq_poly_mul_(mp_limb_t * A, const mp_limb_t * B, slong Blen, const mp_limb_t * C, slong Clen, const fq_nmod_ctx_t ctx, n_poly_stack_t St);
...
N_POLY_INLINE void n_fq_poly_divrem_(n_poly_t Q, n_poly_t R, const n_poly_t A, const n_poly_t B, const fq_nmod_ctx_t ctx, n_poly_stack_t St) { n_fq_poly_divrem_divconquer_(Q, R, A, B, ctx, St); }
...
void n_fq_poly_eval_pow(mp_limb_t * ev, const n_fq_poly_t A, n_fq_poly_t alphapow, const fq_nmod_ctx_t ctx);
...
N_POLY_INLINE void n_bpoly_zero(n_bpoly_t A) { A->length = 0; }
void n_bpoly_one(n_bpoly_t A);

N_POLY_INLINE int n_bpoly_is_zero(const n_bpoly_t A) { return A->length == 0; }

void _n_bpoly_set(n_bpoly_t A, const n_bpoly_t B);
N_POLY_INLINE void n_bpoly_set(n_bpoly_t A, const n_bpoly_t B) { if (A != B) _n_bpoly_set(A, B); }

int n_bpoly_equal(const n_bpoly_t A, const n_bpoly_t B);

void n_bpoly_set_coeff_nonzero(n_bpoly_t A, slong e0, slong e1, mp_limb_t c);
...
N_POLY_INLINE slong n_bpoly_degree0(const n_bpoly_t A) { return A->length - 1; }
...
void n_bpoly_mod_taylor_shift_gen0(n_bpoly_t A, mp_limb_t c, nmod_t ctx);
void n_bpoly_mod_taylor_shift_gen1(n_bpoly_t A, const n_bpoly_t B, mp_limb_t c, nmod_t ctx);
...
void n_bpoly_mod_interp_lift_2sm_poly(slong * deg1, n_bpoly_t T, const n_poly_t A, const n_poly_t B, mp_limb_t alpha, nmod_t mod);
...
void n_fq_bpoly_get_coeff_n_fq(mp_limb_t * c, const n_bpoly_t A, slong e0, slong e1, const fq_nmod_ctx_t ctx);

void n_fq_bpoly_set_coeff_n_fq(n_fq_bpoly_t A, slong e0, slong e1, const mp_limb_t * c, const fq_nmod_ctx_t ctx);
...
void n_fq_bpoly_scalar_mul_n_fq(n_fq_bpoly_t A, const mp_limb_t * c, const fq_nmod_ctx_t ctx);
...
void n_fq_bpoly_taylor_shift_gen0_n_fq(n_fq_bpoly_t A, const mp_limb_t * alpha, const fq_nmod_ctx_t ctx);
...
void nmod_pow_cache_start(mp_limb_t b, n_poly_t pos_direct, n_poly_t pos_bin, n_poly_t neg_direct);

mp_limb_t nmod_pow_cache_mulpow_ui(mp_limb_t a, ulong e, n_poly_t pos_direct, n_poly_t pos_bin, n_poly_t neg_direct, nmod_t ctx);
mp_limb_t nmod_pow_cache_mulpow_neg_ui(mp_limb_t a, ulong e, n_poly_t pos_direct, n_poly_t pos_bin, n_poly_t neg_direct, nmod_t ctx);
mp_limb_t nmod_pow_cache_mulpow_fmpz(mp_limb_t a, const fmpz_t e, n_poly_t pos_direct, n_poly_t pos_bin, n_poly_t neg_direct, nmod_t ctx);
...
void n_fq_pow_cache_start_n_fq(const mp_limb_t * b, n_poly_t pos_direct, n_poly_t pos_bin, n_poly_t neg_direct, const fq_nmod_ctx_t ctx);

void n_fq_pow_cache_mulpow_ui(mp_limb_t * r, const mp_limb_t * a, ulong e, n_poly_t pos_direct, n_poly_t pos_bin, n_poly_t neg_direct, const fq_nmod_ctx_t ctx);
void n_fq_pow_cache_mulpow_neg_ui(mp_limb_t * r, const mp_limb_t * a, ulong e, n_poly_t pos_direct, n_poly_t pos_bin, n_poly_t neg_direct, const fq_nmod_ctx_t ctx);
void n_fq_pow_cache_mulpow_fmpz(mp_limb_t * r, const mp_limb_t * a, const fmpz_t e, n_poly_t pos_direct, n_poly_t pos_bin, n_poly_t neg_direct, const fq_nmod_ctx_t ctx);
...
N_POLY_INLINE slong nmod_eval_interp_eval_length(nmod_eval_interp_t E) { return 1 + E->radix*E->d; }
...
N_POLY_INLINE void nmod_evals_zero(n_poly_t a) { a->length = 0; }
N_POLY_INLINE void n_fq_evals_zero(n_fq_poly_t a) { a->length = 0; }
...
N_POLY_INLINE void n_polyun_fit_length(n_polyun_t A, slong len) { if (len > A->alloc) n_polyun_realloc(A, len); }
...
void n_fq_poly_product_roots_n_fq(n_poly_t M, const mp_limb_t * H, slong length, const fq_nmod_ctx_t ctx, n_poly_stack_t St);
...
mp_limb_t _nmod_zip_eval_step(mp_limb_t * cur, const mp_limb_t * inc, const mp_limb_t * coeffs, slong length, nmod_t ctx);

void _n_fq_zip_eval_step(mp_limb_t * res, mp_limb_t * cur, const mp_limb_t * inc, const mp_limb_t * coeffs, slong length, const fq_nmod_ctx_t ctx);
void _n_fqp_zip_eval_step(mp_limb_t * res, mp_limb_t * cur, const mp_limb_t * inc, const mp_limb_t * coeffs, slong length, slong d, nmod_t mod);

int _nmod_zip_vand_solve(mp_limb_t * coeffs, const mp_limb_t * monomials, slong mlength, const mp_limb_t * evals, slong elength, const mp_limb_t * master, mp_limb_t * scratch, nmod_t ctx);
int _n_fq_zip_vand_solve(mp_limb_t * coeffs, const mp_limb_t * monomials, slong mlength, const mp_limb_t * evals, slong elength, const mp_limb_t * master, mp_limb_t * scratch, const fq_nmod_ctx_t ctx);
int _n_fqp_zip_vand_solve(mp_limb_t * coeffs, const mp_limb_t * monomials, slong mlength, const mp_limb_t * evals, slong elength, const mp_limb_t * master, mp_limb_t * scratch, const fq_nmod_ctx_t ctx);
...
N_POLY_INLINE void n_poly_stack_give_back(n_poly_stack_t S, slong k) { FLINT_ASSERT(S->top >= k); S->top -= k; }
N_POLY_INLINE slong n_poly_stack_size(const n_poly_stack_t S) { return S->top; }
...
N_POLY_INLINE void n_bpoly_stack_give_back(n_bpoly_stack_t S, slong k) { FLINT_ASSERT(S->top >= k); S->top -= k; }
N_POLY_INLINE slong n_bpoly_stack_size(const n_bpoly_stack_t S) { return S->top; }
...
N_POLY_INLINE void n_polyun_stack_give_back(n_polyun_stack_t S, slong k) { FLINT_ASSERT(S->top >= k); S->top -= k; }
N_POLY_INLINE slong n_polyun_stack_size(const n_polyun_stack_t S) { return S->top; }
