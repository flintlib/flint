...
{
    if (len > A->alloc) n_poly_realloc(A, len);
}
...
{
    FLINT_ASSERT(A->length >= 0); return A->length - 1;
}
N_POLY_INLINE
mp_limb_t n_poly_lead(const n_poly_t A)
{
    FLINT_ASSERT(A->length > 0); return A->coeffs[A->length - 1];
}
...
{
    n_poly_fit_length(A, 1); A->length = 1; A->coeffs[0] = 1;
}
...
void n_bpoly_mod_taylor_shift_gen1(
    n_bpoly_t A,
    const n_bpoly_t B,
    mp_limb_t c,
    nmod_t ctx);
...
{
    if (len > A->alloc) n_polyun_realloc(A, len);
}
