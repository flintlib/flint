...
int
_gr_poly_shiftless_decomposition_factor(
        ...
        gr_ctx_t ctx)
{
    ...
    for (slong j = 0; j < slfac->length; j++)
    {
        ...
        status |= _gr_vec_sub_scalar(shifts->entries + sizeof(fmpz),
                                     shifts->entries + sizeof(fmpz),
                                     shifts->length - 1,
                                     shifts->entries, ZZ);
        ...
        gr_ctx_t ctx)
{
    ...
    status |= gr_factor(polc, fac, mult, pol, 0, pctx);
    gr_poly_fit_length(polc, 1, ctx);
    gr_swap(c, polc->coeffs, ctx);
    _gr_poly_normalise(polc, ctx);
    status |= _gr_poly_shiftless_decomposition_factor(
            slfac, slshifts, slmult, fac, mult, ctx);
