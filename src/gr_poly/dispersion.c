...
int
_gr_poly_dispersion_factor(fmpz_t disp, gr_vec_t disp_set,
			  ...
			  gr_ctx_t ctx)
{
    ...
    if (f == g)
    {
	status |= _gr_poly_dispersion_factor(disp, disp_set, ffac, ffac, ctx);
    }
    else
    {
	...
	status |= _gr_poly_dispersion_factor(disp, disp_set, ffac, gfac, ctx);
    }
