...
TEST_FUNCTION_START(gr_poly_taylor_shift_horner, state)
{
    slong iter;
    ...
    for (iter = 0; iter < 1000; iter++)
    {
        ...
        status |= gr_poly_taylor_shift_horner(Fc, F, c, ctx);
        status |= gr_poly_taylor_shift_horner(Fcd1, Fc, d, ctx);
        status |= gr_poly_set(Fcd2, F, ctx); /* also test aliasing */
        status |= gr_poly_taylor_shift_horner(Fcd2, Fcd2, cd, ctx);
