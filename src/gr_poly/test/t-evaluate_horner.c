...
TEST_FUNCTION_START(gr_poly_evaluate_horner, state)
{
    slong iter;
    ...
    for (iter = 0; iter < 1000; iter++)
    {
        ...
        status |= gr_poly_evaluate_horner(Fx, F, x, ctx);
        status |= gr_set(Gx, x, ctx); /* test aliasing */
        status |= gr_poly_evaluate_horner(Gx, G, Gx, ctx);
        status |= gr_add(FxGx, Fx, Gx, ctx);
        status |= gr_poly_evaluate_horner(FGx, FG, x, ctx);
