...
TEST_FUNCTION_START(gr_poly_compose_divconquer, state)
{
    slong iter;
    ...
    for (iter = 0; iter < 1000; iter++)
    {
        ...
        status |= gr_poly_compose_divconquer(Fx, F, x, ctx);
        status |= gr_poly_set(Gx, x, ctx); /* test aliasing */
        status |= gr_poly_compose_divconquer(Gx, G, Gx, ctx);
        status |= gr_poly_add(FxGx, Fx, Gx, ctx);
        status |= gr_poly_compose_divconquer(FGx, FG, x, ctx);
