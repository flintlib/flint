...
TEST_FUNCTION_START(gr_poly_evaluate_rectangular, state)
{
    slong iter;
    ...
    for (iter = 0; iter < 1000; iter++)
    {
        ...
        status |= gr_poly_evaluate_rectangular(Fx, F, x, ctx);
        status |= gr_set(Gx, x, ctx); /* test aliasing */
        status |= gr_poly_evaluate_rectangular(Gx, G, Gx, ctx);
        status |= gr_add(FxGx, Fx, Gx, ctx);
        status |= gr_poly_evaluate_rectangular(FGx, FG, x, ctx);
