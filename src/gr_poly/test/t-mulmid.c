...
TEST_FUNCTION_START(gr_poly_mulmid, state)
{
    slong iter;
    ...
    for (iter = 0; iter < 1000; iter++)
    {
        ...
        if (aliasing == 0)
        {
            status |= gr_poly_mulmid(res1, a, b, nlo, nhi, ctx);
        }
        else if (aliasing == 1)
        {
            status |= gr_poly_set(res1, a, ctx);
            status |= gr_poly_mulmid(res1, res1, b, nlo, nhi, ctx);
        }
        else if (aliasing == 2)
        {
            status |= gr_poly_set(res1, b, ctx);
            status |= gr_poly_mulmid(res1, a, res1, nlo, nhi, ctx);
        }
        else if (aliasing == 3)
        {
            status |= gr_poly_mulmid(res1, a, a, nlo, nhi, ctx);
        }
        else if (aliasing == 4)
        {
            status |= gr_poly_set(res1, a, ctx);
            status |= gr_poly_mulmid(res1, res1, res1, nlo, nhi, ctx);
        }
