...
int
test_sqrt_series(flint_rand_t state, int which)
{
    gr_ctx_t ctx;
    slong n;
    gr_poly_t A, B, C;
    ...
    {
        case 0:
            status |= gr_poly_sqrt_series(B, A, n, ctx);
            break;
        case 1:
            status |= gr_poly_set(B, A, ctx);
            status |= gr_poly_sqrt_series(B, B, n, ctx);
            break;
        case 2:
            status |= gr_poly_sqrt_series_basecase(B, A, n, ctx);
            break;
        case 3:
            status |= gr_poly_set(B, A, ctx);
            status |= gr_poly_sqrt_series_basecase(B, B, n, ctx);
            break;
        case 4:
            status |= gr_poly_sqrt_series_newton(B, A, n, n_randint(state, 20), ctx);
            break;
        case 5:
            status |= gr_poly_set(B, A, ctx);
            status |= gr_poly_sqrt_series_newton(B, B, n, n_randint(state, 20), ctx);
            break;
        case 6:
            status |= gr_poly_sqrt_series_miller(B, A, n, ctx);
            break;
        case 7:
            status |= gr_poly_set(B, A, ctx);
...
TEST_FUNCTION_START(gr_poly_sqrt_series, state)
{
    slong iter;
