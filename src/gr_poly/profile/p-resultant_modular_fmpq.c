                    ...
                    flint_bitcnt_t bits, gr_ctx_t ctx, gr_ctx_t cctx)
{
    ...
    for (i = 0; i < leny; i++)
    {
        ...
        for (k = 0; k < lenx; k++)
        {
            ...
            fmpq_randtest_not_zero(t, state, bits);
            GR_MUST_SUCCEED(gr_set_fmpq(v, t, cctx));
...
int main(void)
{
    ...
    flint_printf("res_y(f, g) over Q[x][y], with f and g of length leny in y,\n");
    flint_printf("coefficients of length lenx in x, whose numerators and\n");
    flint_printf("denominators have `bits` bits.\n\n");
    ...
    /* the ranges are smaller than over Z: it is the subresultant baseline
       that grows out of hand over Q, not the modular algorithm */
    for (leny = 4; leny <= 8; leny *= 2)
    {
        for (lenx = 4; lenx <= 8; lenx *= 2)
        {
            for (bits = 8; bits <= 256; bits *= 4)
            {
                gr_ctx_t cctx, ctx;
                gr_poly_t f, g;
                gr_ptr x, y;
