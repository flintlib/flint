...
TEST_FUNCTION_START(ca_mat_rref, state)
{
    slong iter;
    ...
    for (iter = 0; iter < 1000 * 0.1 * flint_test_multiplier(); iter++)
    {
        ...
        {
            ca_mat_set(R, A, ctx);
            success = ca_mat_rref(&rank, R, R, ctx);
        }
        else
        {
            success = ca_mat_rref(&rank, R, A, ctx);
        }
    ...
    for (iter = 0; iter < 1000 * 0.1 * flint_test_multiplier(); iter++)
    {
        ...
        success = ca_mat_rref(&rank1, R, A, ctx);

        if (success)
        {
            success = ca_mat_rref(&rank2, R2, B, ctx);
