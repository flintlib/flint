...
TEST_FUNCTION_START(fmpz_mod_mpoly_q_mul_fmpq, state)
{
    slong iter;
    ...
    for (iter = 0; iter < 10000 * 0.1 * flint_test_multiplier(); iter++)
    {
        ...
        fmpz_mod_mpoly_scalar_mul_fmpz(fmpz_mod_mpoly_q_numref(C), fmpz_mod_mpoly_q_numref(A), fmpq_numref(c), ctx);
        fmpz_mod_mpoly_scalar_mul_fmpz(fmpz_mod_mpoly_q_denref(C), fmpz_mod_mpoly_q_denref(A), fmpq_denref(c), ctx);
