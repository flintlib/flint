...
TEST_FUNCTION_START(fmpz_poly_interpolate_multi_mod, state)
{
    int i, result;
    ...
    for (i = 0; i < 1000 * flint_test_multiplier(); i++)
    {
        ...
        for (k = 0; k < nbad && npoints; k++)
        {
            ...
            fmpz_mul_ui(x + j, x + j, adversarial_primes[n_randint(state, 100)]);
        }

        fmpz_poly_evaluate_fmpz_vec(y, P, x, npoints);

        result = fmpz_poly_interpolate_multi_mod(Q, x, y, npoints);
        if (!result)
        {
