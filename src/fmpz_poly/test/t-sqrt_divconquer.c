...
TEST_FUNCTION_START(fmpz_poly_sqrt_divconquer, state)
{
    int i;
    ...
    for (i = 0; i < 200 * flint_test_multiplier(); i++)
    {
        ...
        square1 = fmpz_poly_sqrt_divconquer(b, a);
        square2 = fmpz_poly_sqrt_divconquer(a, a);
    ...
    for (i = 0; i < 200 * flint_test_multiplier(); i++)
    {
        ...
        fmpz_poly_randtest(a, state, 1 + n_randint(state, 100),
            1 + n_randint(state, 200));
        fmpz_poly_sqr(b, a);
        square = fmpz_poly_sqrt_divconquer(c, b);
