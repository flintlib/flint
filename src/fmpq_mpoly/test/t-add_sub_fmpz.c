...
TEST_FUNCTION_START(fmpq_mpoly_add_sub_fmpz, state)
{
    int i, j, result;
    ...
    for (i = 0; i < 10 * flint_test_multiplier(); i++)
    {
        ...
        for (j = 0; j < 10; j++)
        {
            ...
            fmpq_mpoly_add_fmpz(g, f, c, ctx);
            fmpq_mpoly_assert_canonical(g, ctx);

            fmpq_mpoly_sub_fmpz(h, g, c, ctx);
    ...
    for (i = 0; i < 10 * flint_test_multiplier(); i++)
    {
        ...
        for (j = 0; j < 10; j++)
        {
            ...
            fmpq_mpoly_add_fmpz(f, f, c, ctx);
            fmpq_mpoly_assert_canonical(f, ctx);

            fmpq_mpoly_sub_fmpz(f, f, c, ctx);
            fmpq_mpoly_assert_canonical(f, ctx);
