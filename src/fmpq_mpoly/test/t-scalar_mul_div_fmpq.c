...
TEST_FUNCTION_START(fmpq_mpoly_scalar_mul_div_fmpq, state)
{
    int i, j, result;
    ...
    for (i = 0; i < 10 * flint_test_multiplier(); i++)
    {
        ...
        for (j = 0; j < 10; j++)
        {
            ...
            fmpq_mpoly_scalar_mul_fmpq(g, f, c, ctx);
            fmpq_mpoly_assert_canonical(g, ctx);

            fmpq_mpoly_scalar_div_fmpq(h, g, c, ctx);
    ...
    for (i = 0; i < 10 * flint_test_multiplier(); i++)
    {
        ...
        for (j = 0; j < 10; j++)
        {
            ...
            fmpq_mpoly_scalar_mul_fmpq(f, f, c, ctx);
            fmpq_mpoly_assert_canonical(f, ctx);

            fmpq_mpoly_scalar_div_fmpq(f, f, c, ctx);
            fmpq_mpoly_assert_canonical(f, ctx);
