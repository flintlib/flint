...
{
    FLINT_SWAP(fq_zech_bpoly_struct, *A, *B);
}

FQ_ZECH_MPOLY_FACTOR_INLINE
void fq_zech_bpoly_normalise(fq_zech_bpoly_t A, const fq_zech_ctx_t ctx)
{
    while (A->length > 0 && fq_zech_poly_is_zero(A->coeffs + A->length - 1, ctx)) A->length--;
}

FQ_ZECH_MPOLY_FACTOR_INLINE
void fq_zech_bpoly_fit_length(fq_zech_bpoly_t A, slong len, const fq_zech_ctx_t ctx)
{
    if (len > A->alloc) fq_zech_bpoly_realloc(A, len, ctx);
}
...
{
    if (e0 >= A->length) fq_zech_zero(c, ctx); else fq_zech_poly_get_coeff(c, A->coeffs + e0, e1, ctx);
}
...
void fq_zech_bpoly_set_poly_var0(
    fq_zech_bpoly_t A,
    const fq_zech_poly_t B,
    const fq_zech_ctx_t ctx);
void fq_zech_bpoly_set_poly_var1(
    fq_zech_bpoly_t A,
    const fq_zech_poly_t B,
    const fq_zech_ctx_t ctx);
...
{
    FLINT_SWAP(fq_zech_tpoly_struct, *A, *B);
}
...
{
    A->coeffs = NULL; A->exps = NULL; A->length = 0; A->alloc = 0;
}
...
{
    FLINT_ASSERT(A->alloc >= 0); if (len > A->alloc) fq_zech_polyu_realloc(A, len, ctx);
}

FQ_ZECH_MPOLY_FACTOR_INLINE
void fq_zech_polyu_swap(fq_zech_polyu_t A, fq_zech_polyu_t B, const fq_zech_ctx_t ctx)
{
    FLINT_SWAP(fq_zech_polyu_struct, *A, *B);
}
...
void fq_zech_mpoly_factor_realloc(
...
void fq_zech_mpoly_factor_set(
    fq_zech_mpoly_factor_t a,
    const fq_zech_mpoly_factor_t b,
    const fq_zech_mpoly_ctx_t ctx);
...
void fq_zech_mpoly_factor_append_ui(
    fq_zech_mpoly_factor_t f,
    const fq_zech_mpoly_t A,
    ulong e,
    const fq_zech_mpoly_ctx_t ctx);
void fq_zech_mpoly_factor_append_fmpz(
    fq_zech_mpoly_factor_t f,
    const fq_zech_mpoly_t A,
    const fmpz_t e,
    const fq_zech_mpoly_ctx_t ctx);

int fq_zech_mpoly_factor_squarefree(
    fq_zech_mpoly_factor_t f,
    const fq_zech_mpoly_t A,
    const fq_zech_mpoly_ctx_t ctx);
...
FQ_ZECH_MPOLY_FACTOR_INLINE
void fq_zech_mpoly_factor_swap(fq_zech_mpoly_factor_t A, fq_zech_mpoly_factor_t B, const fq_zech_mpoly_ctx_t ctx)
{
    FLINT_SWAP(fq_zech_mpoly_factor_struct, *A, *B);
}

FQ_ZECH_MPOLY_FACTOR_INLINE
void fq_zech_mpoly_factor_one(fq_zech_mpoly_factor_t a, const fq_zech_mpoly_ctx_t ctx)
{
    	fq_zech_one(a->constant, ctx->fqctx); a->num = 0;
}

int fq_zech_mpoly_factor_expand(
    fq_zech_mpoly_t A,
    const fq_zech_mpoly_factor_t f,
    const fq_zech_mpoly_ctx_t ctx);
...
FQ_ZECH_MPOLY_FACTOR_INLINE
void fq_zech_mpolyv_swap(fq_zech_mpolyv_t A, fq_zech_mpolyv_t B, const fq_zech_mpoly_ctx_t ctx)
{
    FLINT_SWAP(fq_zech_mpolyv_struct, *A, *B);
}
...
int _fq_zech_mpoly_eval_rest_fq_zech_poly(
    fq_zech_poly_struct * E, slong * starts,
    slong * ends,
    slong * stops,
    ulong * es,
