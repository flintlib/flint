    ...
    const fmpz_mod_mpoly_ctx_t ctx)
{
    ...
    fmpz_mod_mpoly_t Ac, Bc, Gc, Gamma, Al_lc, Bl_lc;
    ulong * shift, * stride;
    slong * perm;
    slong * tmp, * Al_degs, * Bl_degs, * Gamma_degs;
    ...
    tmp = FLINT_ARRAY_ALLOC(4*ctx->minfo->nvars, slong);
    Al_degs   = tmp + 1*ctx->minfo->nvars;
    Bl_degs   = tmp + 2*ctx->minfo->nvars;
    Gamma_degs = tmp + 3*ctx->minfo->nvars;
    ...
    fmpz_mod_mpoly_degrees_si(Bl_degs, Bl, lctx);
    fmpz_mod_mpoly_degrees_si(Gamma_degs, Gamma, lctx);

    success = fmpz_mod_mpolyl_gcd_zippel2_smprime(Gl, NULL, Abarl, Bbarl,
...
TEST_FUNCTION_START(fmpz_mod_mpoly_factor_gcd_zippel2, state)
{
    slong i, j, tmul = 10;
