...
static slong _gr_mpoly_sub(
    ...
    gr_ctx_t fctx)
{
    gr_method_binary_op sub = GR_BINARY_OP(fctx, SUB);
    gr_method_unary_op set = GR_UNARY_OP(fctx, SET);
    gr_method_unary_op neg = GR_UNARY_OP(fctx, NEG);
    ...
    while (i < Blen && j < Clen)
    {
        ...
        {
            mpoly_monomial_set(Aexps + k*N, Bexps + i*N, N);
            status |= sub(GR_ENTRY(Acoeffs, k, sz), GR_ENTRY(Bcoeffs, i, sz), GR_ENTRY(Ccoeffs, j, sz), fctx);
    ...
    const mpoly_ctx_t mctx, gr_ctx_t cctx)
{
    ...
    if (B->length == 0)
        return gr_mpoly_neg(A, C, mctx, cctx);
