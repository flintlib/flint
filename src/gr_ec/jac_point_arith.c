...
*/
int
gr_ec_jac_point_mul_2exp_si(gr_ec_jac_point_t res, const gr_ec_jac_point_t P, slong k,
        gr_ec_ctx_t ctx)
{
    int status;
    slong i;
    ...
    status = gr_ec_jac_point_set(res, P, ctx);

    for (i = 0; i < k && status == GR_SUCCESS; i++)
        status = gr_ec_jac_point_dbl(res, res, ctx);

    return status;
}

int
gr_ec_jac_point_mul_2exp_fmpz(gr_ec_jac_point_t res, const gr_ec_jac_point_t P,
        const fmpz_t k, gr_ec_ctx_t ctx)
{
    ...
    if (gr_ec_jac_point_is_inf(P, ctx) == T_TRUE)
        return gr_ec_jac_point_zero(res, ctx);

    if (!fmpz_fits_si(k))
        return GR_UNABLE;

    return gr_ec_jac_point_mul_2exp_si(res, P, fmpz_get_si(k), ctx);
}
