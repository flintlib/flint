...
TEST_FUNCTION_START(mpf_vec_sub, state)
{
    int i, result;
    ...
    for (i = 0; i < 1000 * flint_test_multiplier(); i++)
    {
        ...
        _mpf_vec_sub(c, a, b, len);
        _mpf_vec_sub(a, a, b, len);
    ...
    for (i = 0; i < 1000 * flint_test_multiplier(); i++)
    {
        ...
        b = _mpf_vec_init(len, 200);
        c = _mpf_vec_init(len, 200);
        d = _mpf_vec_init(len, 200);
        _mpf_vec_randtest(a, state, len, 200);
        _mpf_vec_randtest(b, state, len, 200);

        _mpf_vec_add(c, a, b, len);
        _mpf_vec_sub(d, c, b, len);

        result = (_mpf_vec_equal(d, a, len));
        if (!result)
        {
            flint_printf("FAIL:\n");
            fflush(stdout);
