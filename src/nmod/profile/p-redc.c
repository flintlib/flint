...
ulong pbits_arr[] = { FLINT_BITS - 8, FLINT_BITS, 0 };
ulong ebits_arr[] = { 2, 4, 8, 12, 16, 32, 48, 64, 0 };

__attribute__((noinline)) void _loop_redc_set_nmod(nn_ptr z, nn_srcptr x, slong N, nmod_redc_ctx_t ctx)
{
    slong i;
    for (i = 0; i < N; i++)
...
__attribute__((noinline)) void _loop_redc_get_nmod(nn_ptr z, nn_srcptr x, slong N, nmod_redc_ctx_t ctx)
{
    slong i;
    for (i = 0; i < N; i++)
...
__attribute__((noinline)) void _loop_nmod_mul(nn_ptr z, nn_srcptr x, nn_srcptr y, slong N, nmod_t mod)
{
    slong i;
    for (i = 0; i < N; i++)
...
__attribute__((noinline)) void _loop_nmod_mul_fullword(nn_ptr z, nn_srcptr x, nn_srcptr y, slong N, nmod_t mod)
{
    slong i;
    for (i = 0; i < N; i++)
...
__attribute__((noinline)) void _loop_nmod_redc_mul(nn_ptr z, nn_srcptr x, nn_srcptr y, slong N, nmod_redc_ctx_t ctx)
{
    slong i;
    for (i = 0; i < N; i++)
...
__attribute__((noinline)) void _loop_nmod_redc_fast_mul(nn_ptr z, nn_srcptr x, nn_srcptr y, slong N, nmod_redc_ctx_t ctx)
{
    slong i;
    for (i = 0; i < N; i++)
...
__attribute__((noinline)) void _loop_nmod_add(nn_ptr z, nn_srcptr x, nn_srcptr y, slong N, nmod_t mod)
{
    slong i;
    for (i = 0; i < N; i++)
...
__attribute__((noinline)) void _loop_nmod_redc_add(nn_ptr z, nn_srcptr x, nn_srcptr y, slong N, nmod_redc_ctx_t ctx)
{
    slong i;
    for (i = 0; i < N; i++)
...
__attribute__((noinline)) void _loop_nmod_redc_fast_add(nn_ptr z, nn_srcptr x, nn_srcptr y, slong N, nmod_redc_ctx_t ctx)
{
    slong i;
    for (i = 0; i < N; i++)
...
__attribute__((noinline)) void _loop_nmod_redc_fast_mul_two(nn_ptr z, nn_srcptr x, slong N, nmod_redc_ctx_t ctx)
{
    slong i;
    for (i = 0; i < N; i++)
...
#define PRINTF2(f, t) flint_printf("%30s    %g\n", f, t / N / 1e-9)
    ...
    for (ipbits = 0; (pbits = pbits_arr[ipbits]) != 0; ipbits++)
    {
        ...
        flint_printf("\n%10s  %24s  %24s  %24s  %24s\n", "ebits",
            "_nmod_pow_ui_binexp", "_nmod_redc_pow_ui", "_nmod_redc_fast_pow_ui", "nmod_pow_ui");
        ...
        for (iebits = 0; (ebits = ebits_arr[iebits]) != 0; iebits++)
        {
            ...
            for (i = 0; i < N; i++)
            {
                s += _nmod_redc_pow_ui(x[i], exps[i], ctx);
            }
            ...
            if (nmod_redc_can_use_fast(ctx))
            {
                ...
                for (i = 0; i < N; i++)
                {
                    s += _nmod_redc_fast_pow_ui(x[i], exps[i], ctx);
                }
