...
typedef enum {
    ORE_OPERATOR_STANDARD_DERIVATIVE,
    ORE_OPERATOR_EULER_DERIVATIVE,
    /* TODO: Shift operator, etc. */
}
ore_poly_which_operator_t;

#define ORE_POLY_NUM_OPERATORS 2

GR_ORE_POLY_INLINE
ore_poly_which_operator_t ore_poly_which_operator_randtest(flint_rand_t state)
{
   return (ore_poly_which_operator_t) n_randint(state, ORE_POLY_NUM_OPERATORS);
}
...
typedef struct
{
    ...
    char * var;
    slong base_var;
    ore_poly_which_operator_t which_operator;
...
void gr_ctx_init_gr_ore_poly(gr_ctx_t ctx, gr_ctx_t base_ring, slong base_var, const ore_poly_which_operator_t operator);

void gr_ore_poly_ctx_init(gr_ore_poly_ctx_t ctx, gr_ctx_t base_ring, slong base_var, const ore_poly_which_operator_t operator);
...
int _gr_ore_poly_write(gr_stream_t out, gr_srcptr poly, slong len, const char * d, gr_ore_poly_ctx_t ctx);
int gr_ore_poly_write(gr_stream_t out, const gr_ore_poly_t poly, const char * d, gr_ore_poly_ctx_t ctx);
int _gr_ore_poly_get_str(char ** res, gr_srcptr f, slong len, const char * d, gr_ore_poly_ctx_t ctx);
int gr_ore_poly_get_str(char ** res, const gr_ore_poly_t poly, const char * d, gr_ore_poly_ctx_t ctx);
int _gr_ore_poly_set_str(gr_ptr res, const char * s, const char * d, slong len, gr_ore_poly_ctx_t ctx);
