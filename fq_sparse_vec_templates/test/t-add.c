...
main(void)
{
    ...
    for (rep = 0; rep < 1000; rep++)
    {
        ...
        TEMPLATE(T, sparse_vec_add)(w, u, v, ctx);
        TEMPLATE(T, sparse_vec_sub)(x, w, v, ctx);

        if (!TEMPLATE(T, sparse_vec_equal)(u, x, 0, ctx))
        {
            flint_printf("FAIL: u != u+v-v\n");
            abort();
        }

        TEMPLATE(T, sparse_vec_add)(u, u, v, ctx);
        ...
        TEMPLATE(T, sparse_vec_sub)(u, u, v, ctx);
        if (!TEMPLATE(T, sparse_vec_equal)(u, x, 0, ctx))
        {
