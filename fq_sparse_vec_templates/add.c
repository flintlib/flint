                ...
                const TEMPLATE(T, ctx_t) ctx)
{
    ...
    if(u->nnz == 0) TEMPLATE(T, sparse_vec_set) (w, v, 0, ctx);
    ...
    else
    {
        ...
        while (ue >= u->entries && ve >= v->entries)
        {
            ...
            if (ue->ind == ve->ind) 
            {
                TEMPLATE(T, add) (we->val, (ue--)->val, (ve--)->val, ctx);
                if (!TEMPLATE(T, is_zero) (we->val, ctx)) we--;
            }
            else if (ue->ind == we->ind) TEMPLATE(T, set) ((we--)->val, (ue--)->val, ctx);
            else if (ve->ind == we->ind) TEMPLATE(T, set) ((we--)->val, (ve--)->val, ctx);
        }
        while (ue >= u->entries) we->ind = ue->ind, TEMPLATE(T, set) ((we--)->val, (ue--)->val, ctx);
        while (ve >= v->entries) we->ind = ve->ind, TEMPLATE(T, set) ((we--)->val, (ve--)->val, ctx);
