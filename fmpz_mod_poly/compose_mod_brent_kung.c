                         ...
                         const fmpz_mod_poly_t poly3, const fmpz_mod_ctx_t ctx)
{
    ...
    if (len3 == 0)
        flint_throw(FLINT_DIVZERO, "fmpz_mod_poly_compose_mod_brent_kung\n");

    if (len1 >= len3)
        flint_throw(FLINT_DIVZERO, "Degree of first polynomial must be smaller than modulus in fmpz_mod_poly_compose_brent_kung\n");
