...
\end{lstlisting}
FLINT can also handle source build of GMP/MPIR and MPFR. Though
...
If you wish to make use of the threaded functions in Flint, the best
scaling will happen with the \code{--pooled} and \code{--single}
options. Note that to use the pooled allocator, one must call the
macro \code{FLINT_POOLED_INIT();} before any Flint code is called.
The purpose of the \code{--pooled} option is to automatically insert
this macro in all of the Flint test code.
