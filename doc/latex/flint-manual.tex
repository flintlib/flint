...
\code{ulong} and \code{mp_limb_signed_t} types, respectively.
...
Each coefficient of an \code{nmod_poly_t} is of type \code{ulong}
...
\code{ulong}'s, representing a dense matrix in row-major order.
...
irreducible polynomial of degree~$n$, as a polynomial in
$\mathbf{F}_p[X]$ of degree less than $n$.  The underlying data
structure is just an \code{ulong}.
...
\code{ulong}; however, it is recommended that $p^n < 2^{20}$ due
...
The basic type is the \code{ulong} as defined by MPIR. Functions
which take a precomputed inverse either have the suffix \code{preinv}
and take an \code{ulong} precomputed inverse as computed by
...
\code{fmpzxx}. The latter takes an argument of type \code{ulong} and one of
