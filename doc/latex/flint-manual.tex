...
Additionally, one can register custom allocation and freeing functions, by calling the
\code{flint_set_memory_functions} function. This function takes 4 function pointers to 
\code{flint_malloc}, \code{flint_realloc}, \code{flint_calloc} and \code{flint_free}, respectively.
 If a null pointer is passed in, the default function is called.
...
<<<<<<< HEAD
The \code{fmpz_mod_poly_factor} module is included automatically with
\code{fmpz_mod_poly.h}. One should not try to include
=======
