...
.. function:: void thread_pool_init(thread_pool_t T, slong size)
    ...
    Initialise `T` and create `size` sleeping threads that are available to work.
    If `size \le 0` no threads are created and future calls to
    `thread_pool_request` will return `0` (unless :func:`thread_pool_set_size` has been called).
