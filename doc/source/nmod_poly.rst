...
.. function:: int nmod_poly_crt_compile(nmod_poly_crt_t P, nmod_poly_struct * const * moduli, slong len);
    ...
    Configure ``P`` for repeated chinese remaindering of ``moduli``. The number of moduli, ``len``, should be positive, and the modulus (``mod.n``) of the moduli is expected to be prime.
