...
.. function:: umul_ppmm(high_prod, low_prod, multipler, multiplicand)

    Multiplies two single limb integers ``MULTIPLER`` and 
    ``MULTIPLICAND``, and generates a two limb product in 
    ``HIGH_PROD`` and ``LOW_PROD``.

.. function:: smul_ppmm(high_prod, low_prod, multipler, multiplicand)

    As for ``umul_ppmm()`` but the numbers are signed.

.. function:: udiv_qrnnd(quotient, remainder, high_numerator, low_numerator, denominator)
...
.. function:: sdiv_qrnnd(quotient, remainder, high_numerator, low_numerator, denominator)
...
.. function:: count_leading_zeros(count, x)
...
.. function:: count_trailing_zeros(count, x)

    As for ``count_leading_zeros()``, but counts from the least 
    significant end. If ``x`` is zero then count is undefined.

.. function:: add_ssaaaa(high_sum, low_sum, high_addend_1, low_addend_1, high_addend_2, low_addend_2)
...
.. function:: add_sssaaaaaa(high_sum, mid_sum, low_sum, high_addend_1, mid_addend_1, low_addend_1, high_addend_2, mid_addend_2, low_addend_2)

    Adds two three limb integers. Carry out is lost.

.. function:: sub_ddmmss(high_difference, low_difference, high_minuend, low_minuend, high_subtrahend, low_subtrahend)
...
.. function:: sub_dddmmmsss(high_diff, mid_diff, low_diff, high_minuend_1, mid_minuend_1, low_minuend_1, high_subtrahend_2, mid_subtrahend_2, low_subtrahend_2)

    Subtracts two three limb integers. Borrow out is lost.

.. function:: byte_swap(x)

    Swap the order of the bytes in the word `x`, i.e. most significant byte
    becomes least significant byte, etc.

.. function:: invert_limb(invxl, xl)
...
.. function:: udiv_qrnnd_preinv(q, r, nh, nl, d, di)
