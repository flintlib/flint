...
The valuation `v` and the precision `N` are measured in powers of `p` (digits),
...
.. macro:: RADIX_PADIC_EXACT
    ...
    `N` value marking an exact element.

.. macro:: RADIX_PADIC_PREC_INF

    Value representing an infinite precision.

.. macro:: RADIX_PADIC_ERR_MAX

    Largest representable error.
...
.. macro:: RADIX_PADIC_VAL(x)

.. macro:: RADIX_PADIC_N(x)

.. macro:: RADIX_PADIC_CTX_RADIX(ctx)
           RADIX_PADIC_CTX_PREC_ABS(ctx)
