...
.. function:: void fq_mat_scalar_mul(fq_mat_t B, const fq_mat_t A, const fq_t c, const fq_ctx_t ctx)
    ...
    Set ``B = A*c`` where ``A`` is an ``fq_mat_t`` and ``c``
    is a scalar respectively of type ``fq_t``, ``fmpz_t``, ``slong``,
