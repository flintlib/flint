...
slong qsieve_merge_sort(qs_t qs_inf)
{
   ...
   for (i = columns + num_unmerged - WORD(1); i >= dups; i--)
   {
      if (!columns) comp = -1;
         ...
         comp = qsieve_relations_cmp2(matrix + columns - WORD(1), qsort_arr[num_unmerged - WORD(1)]);
...
slong qsieve_merge_relations(qs_t qs_inf)
{
   ...
   if (num_unmerged)
   {
      ...
      qsort(qsort_arr, num_unmerged, sizeof(la_col_t *), qsieve_relations_cmp);

      return qsieve_merge_sort(qs_inf);
   }
...
slong qsieve_insert_relation(qs_t qs_inf, fmpz_t Y)
{
   ...
   if (qs_inf->num_unmerged == qs_inf->qsort_rels)
      return qsieve_merge_relations(qs_inf);
