...
int check_rref_form(slong * perm, TEMPLATE(T, mat_t) A, slong rank, const TEMPLATE(T, ctx_t) ctx)
{
        ...
        for (j = 0; j < A->c; j++)
        {
            if (!TEMPLATE(T, is_zero)(TEMPLATE(T, mat_entry)(A, i, j), ctx))
            {
                ...
                for (k = 0; k < rank; k++)
                {
                    if (i == k)
                    {
                        if (!TEMPLATE(T, is_one)(TEMPLATE(T, mat_entry)(A, k, j), ctx))
                            return 0;
                    }
                    else
                    {
                        if (!TEMPLATE(T, is_zero)(TEMPLATE(T, mat_entry)(A, k, j), ctx))
                            return 0;
                    }
                }
    ...
    for (i = 0; i < 100; i++)
    {
        ...
        if (equal)
        {
            ...
            for (j = rank2; j < 2 * rank2; j++)
                for (k = 0; k < n; k++)
                {
                    equal = equal && TEMPLATE(T, is_zero)(TEMPLATE(T, mat_entry)(D, j, k), ctx);
                }
        }
