                           ...
                           const TEMPLATE(T, ctx_t) ctx)
{
    ...
    else if (nullity)
    {
        ...
        {
            while (TEMPLATE(T, is_zero)(TEMPLATE(T, mat_entry)(tmp, i, j), ctx))
            {
                nonpivots[k] = j;
                k++;
                j++;
        ...
        for (i = 0; i < nullity; i++)
        {
            for (j = 0; j < rank; j++)
            {
                TEMPLATE(T, neg)(TEMPLATE(T, mat_entry)(X, pivots[j], i),
                                 TEMPLATE(T, mat_entry)(tmp, j, nonpivots[i]),
                                 ctx);
            }

            TEMPLATE(T, one)(TEMPLATE(T, mat_entry)(X, nonpivots[i], i), ctx);
        }
    }
