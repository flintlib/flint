...
mp_limb_t
mpqs_knuth_schroeppel(mpqs_t mpqs_inf)
{
    ...
    if (fmpz_is_even(mpqs_inf->n)) /* check 2 is not a factor */
        return 2;

    /* initialise weights for each multiplier k depending on kn mod 8 */
    nmod8 = fmpz_fdiv_ui(mpqs_inf->n, 8); /* n modulo 8 */
    ...
    */
    max = FLINT_MIN(mpqs_inf->ks_primes, mpqs_inf->num_primes - 2);
    ...
    for (num_primes = 0; num_primes < max; num_primes++)
    {
        ...
        nmod = fmpz_fdiv_ui(mpqs_inf->n, p);
