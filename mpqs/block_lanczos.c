...
void mpqs_reduce_matrix(mpqs_t mpqs_inf, slong *nrows, slong *ncols, la_col_t *cols) {
	...
	do {
		...
		if (reduced_cols > reduced_rows + mpqs_inf->extra_rels) {
			for (i = reduced_rows + mpqs_inf->extra_rels;
					i < reduced_cols; i++) {
			...
			reduced_cols = reduced_rows + mpqs_inf->extra_rels;
		}
...
/*-------------------------------------------------------------------*/
void mpqs_mul_MxN_Nx64(slong vsize, slong dense_rows,
...
/*-------------------------------------------------------------------*/
void mpqs_mul_trans_MxN_Nx64(slong dense_rows, slong ncols,
...
/*-----------------------------------------------------------------------*/
void mpqs_combine_cols(slong ncols, 
...
uint64_t * mpqs_block_lanczos(flint_rand_t state, slong nrows, 
	...
	mpqs_mul_MxN_Nx64(vsize, dense_rows, ncols, B, v[0], scratch);
	mpqs_mul_trans_MxN_Nx64(dense_rows, ncols, B, scratch, v[0]);
	...
	while (1) {
		...
		mpqs_mul_MxN_Nx64(vsize, dense_rows, ncols, B, v[0], scratch);
		mpqs_mul_trans_MxN_Nx64(dense_rows, ncols, B, scratch, vnext);
	...
	mpqs_mul_MxN_Nx64(vsize, dense_rows, ncols, B, x, v[1]);
	mpqs_mul_MxN_Nx64(vsize, dense_rows, ncols, B, v[0], v[2]);

	mpqs_combine_cols(ncols, x, v[0], v[1], v[2]);

	/* verify that these really are linear dependencies of B */

	mpqs_mul_MxN_Nx64(vsize, dense_rows, ncols, B, x, v[0]);
