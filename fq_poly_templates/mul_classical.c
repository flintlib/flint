                       ...
                       const TEMPLATE(T, struct) * op2, long len2, const TEMPLATE(T, ctx_t) ctx)
{
    ...
    else
    {
        ...
        _TEMPLATE(T, TEMPLATE(poly_scalar_mul, T))(rop + len1, op2 + 1, len2 - 1, op1 + len1 - 1,
                               ctx);

        /* out[i+j] += in1[i]*in2[j] */
        for (i = 0; i < len1 - 1; i++)
            _TEMPLATE(T, TEMPLATE(poly_scalar_addmul, T))(rop + i + 1, op2 + 1, len2 - 1, op1 + i,
                                      ctx);
    }
}
                      ...
                      const TEMPLATE(T, ctx_t) ctx)
{
    ...
    else
    {
        ...
        _TEMPLATE(T, poly_mul_classical)(rop->coeffs, op1->coeffs, op1->length,
                               op2->coeffs, op2->length, ctx);
    }
