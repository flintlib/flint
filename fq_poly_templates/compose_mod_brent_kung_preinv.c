...
_TEMPLATE(T, poly_compose_mod_brent_kung_preinv)(TEMPLATE(T, struct) * res,
                                       ...
                                       const TEMPLATE(T, struct) * poly3inv,
                                       slong len3inv, const TEMPLATE(T, ctx_t) ctx)
{
    ...
    for (i = 2; i < m; i++)
    {
        _TEMPLATE(T, poly_mulmod_preinv)(tmp, A->rows[i - 1], n, poly2, n, poly3, len3,
    ...
    _TEMPLATE(T, poly_mulmod_preinv)(h, A->rows[m - 1], n, poly2, n, poly3, len3,
                           poly3inv, len3inv, ctx);
                                      ...
                                      const TEMPLATE(T, ctx_t) ctx)
{
    ...
    if (len1 >= len3)
    {
        TEMPLATE_PRINTF("Exception: %s_poly_compose_brent_kung: the degree of the", T);
    ...
    if (res == poly3 || res == poly1)
    {
        ...
        TEMPLATE(T, poly_compose_mod_brent_kung_preinv)(tmp, poly1, poly2, poly3,
                                              poly3inv, ctx);
    ...
    _TEMPLATE(T, poly_compose_mod_brent_kung_preinv)(res->coeffs, poly1->coeffs, len1,
