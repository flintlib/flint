                       ...
                       const TEMPLATE(T, struct) * op, slong len, const TEMPLATE(T, ctx_t) ctx)
{
    ...
    else
    {
        ...
        _TEMPLATE(T, TEMPLATE(poly_scalar_mul, T))(rop + len, op + 1, len - 1, op + len - 1, ctx);

        for (i = 1; i < len - 1; i++)
            _TEMPLATE(T, TEMPLATE(poly_scalar_addmul, T))(rop + i + 1, op + 1, i - 1, op + i, ctx);
...
void
TEMPLATE(T, poly_sqr_classical)(TEMPLATE(T, poly_t) rop, const TEMPLATE(T, poly_t) op, const TEMPLATE(T, ctx_t) ctx)
{
    ...
    if (rop == op)
    {
        ...
        _TEMPLATE(T, poly_sqr_classical)(t->coeffs, op->coeffs, op->length, ctx);
    ...
    else
    {
        ...
        _TEMPLATE(T, poly_sqr_classical)(rop->coeffs, op->coeffs, op->length, ctx);
    }
