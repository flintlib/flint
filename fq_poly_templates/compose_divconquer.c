                            ...
                            const TEMPLATE(T, ctx_t) ctx)
{
    ...
    if (len1 <= 2 || len2 <= 1)
    {
        if (len1 == 1)
            ...
            _TEMPLATE(T, TEMPLATE(poly_evaluate, T))(rop, op1, len1, op2, ctx);
        else                    /* len1 == 2 */
    ...
    h = (TEMPLATE(T, struct) **) flint_malloc(((len1 + 1) / 2) * sizeof(TEMPLATE(T, struct) *));
    h[0] = v;
    for (i = 0; i < (len1 - 1) / 2; i++)
    {
        h[i + 1] = h[i] + hlen[i];
    ...
    for (i = 0, j = 0; i < len1 / 2; i++, j += 2)
    {
        ...
        {
            _TEMPLATE(T, TEMPLATE(poly_scalar_mul, T))(h[i], op2, len2, op1 + j + 1, ctx);
    ...
    for (n = (len1 + 1) / 2; n > 2; n = (n + 1) / 2)
    {
        ...
        for (i = 1; i < n / 2; i++)
        {
            if (hlen[2 * i + 1] > 0)
            {
                _TEMPLATE(T, poly_mul)(h[i], pow, powlen, h[2 * i + 1], hlen[2 * i + 1],
                             ctx);
