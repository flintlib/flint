                            ...
                            const TEMPLATE(T, ctx_t) ctx)
{
    ...
    for (i = fmpz_sizeinbase(e, 2) - 2; i >= 0; i--)
    {
        ...
        _TEMPLATE(T, poly_divrem)(Q, res, T, 2 * lenf - 3, f, lenf, invf, ctx);
                           ...
                           const TEMPLATE(T, poly_t) f, const TEMPLATE(T, ctx_t) ctx)
{
    ...
    if (lenf == 0)
    {
        TEMPLATE_PRINTF("Exception: %s_poly_powmod_fmpz_binexp: divide by zero\n", T);
        abort();
    }
    ...
    if ((res == poly && !qcopy) || (res == f))
    {
        ...
        _TEMPLATE(T, poly_powmod_fmpz_binexp)(t->coeffs, q, e, f->coeffs, lenf, ctx);
    ...
    else
    {
        ...
        _TEMPLATE(T, poly_powmod_fmpz_binexp)(res->coeffs, q, e, f->coeffs, lenf, ctx);
    }
