                         ...
                         const TEMPLATE(T, poly_t) f, const TEMPLATE(T, ctx_t) ctx)
{
    ...
    if (lenf == 0)
    {
        TEMPLATE_PRINTF("Exception: %s_poly_powmod_ui_binexp: divide by zero\n", T);
        abort();
    }
    ...
    if ((res == poly && !qcopy) || (res == f))
    {
        ...
        _TEMPLATE(T, poly_powmod_ui_binexp)(t->coeffs, q, e, f->coeffs, lenf, ctx);
    ...
    else
    {
        ...
        _TEMPLATE(T, poly_powmod_ui_binexp)(res->coeffs, q, e, f->coeffs, lenf, ctx);
    }
