                           ...
                           const TEMPLATE(T, t) cinv, const TEMPLATE(T, ctx_t) ctx)
{
    ...
    else
    {
        ...
        /* Base case */
        {
            ...
            _TEMPLATE(T, poly_div_basecase)(Qinv, W, W, 2 * n - 1, Qrev, n, cinv, ctx);
        ...
        for (i--; i >= 0; i--)
        {
            ...
            _TEMPLATE(T, poly_mullow)(Qinv + m, Qinv, m, W + m, n - m, n - m, ctx);
...
void
TEMPLATE(T, poly_inv_series_newton)(TEMPLATE(T, poly_t) Qinv, const TEMPLATE(T, poly_t) Q, slong n,
                          const TEMPLATE(T, ctx_t) ctx)
{
    ...
    if (Qinv != Q)
    {
        ...
        _TEMPLATE(T, poly_inv_series_newton)(Qinv->coeffs, Qcopy, n, cinv, ctx);
    }
    else
    {
