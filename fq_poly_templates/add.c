             ...
             const TEMPLATE(T, struct) * poly2, slong len2, const TEMPLATE(T, ctx_t) ctx)
{
    ...
    for (i = 0; i < min; i++)
        TEMPLATE(T, add)(res + i, poly1 + i, poly2 + i, ctx);
    ...
    if (poly2 != res)
        for (i = min; i < len2; i++)
            TEMPLATE(T, set)(res + i, poly2 + i, ctx);
}

void
TEMPLATE(T, poly_add)(TEMPLATE(T, poly_t) res, const TEMPLATE(T, poly_t) poly1, const TEMPLATE(T, poly_t) poly2,
            const TEMPLATE(T, ctx_t) ctx)
{
    ...
    _TEMPLATE(T, poly_add)(res->coeffs, poly1->coeffs, poly1->length,
