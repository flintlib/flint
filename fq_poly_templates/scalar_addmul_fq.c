...
void
_TEMPLATE(T, TEMPLATE(poly_scalar_addmul, T))(TEMPLATE(T, struct) * rop,
...
void
TEMPLATE(T, TEMPLATE(poly_scalar_addmul, T))(TEMPLATE(T, poly_t) rop,
                         const TEMPLATE(T, poly_t) op, const TEMPLATE(T, t) x, const TEMPLATE(T, ctx_t) ctx)
{
    if (!(TEMPLATE(T, is_zero)(x, ctx) || TEMPLATE(T, poly_is_zero)(op, ctx)))
    {
        ...
        _TEMPLATE(T, TEMPLATE(poly_scalar_addmul, T))(rop->coeffs, op->coeffs, op->length, x, ctx);
        _TEMPLATE(T, poly_set_length)(rop, FLINT_MAX(rop->length, op->length), ctx);
