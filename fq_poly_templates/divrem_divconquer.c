...
static void
__TEMPLATE(T, poly_divrem_divconquer)(TEMPLATE(T, struct) * Q, TEMPLATE(T, struct) * R,
                            ...
                            const TEMPLATE(T, t) invB, const TEMPLATE(T, ctx_t) ctx)
{
    if (lenA < 2 * lenB - 1)
    {
        ...
        TEMPLATE(T, struct) *W = _TEMPLATE(T, vec_init)((2 * n1 - 1) + lenB - 1, ctx);
        ...
        _TEMPLATE(T, poly_add)(R + n2, R + n2, n1 - 1, d2q1 + n2, n1 - 1, ctx);
    ...
    else                        /* lenA = 2 * lenB - 1 */
    {
        ...
        _TEMPLATE(T, poly_divrem_divconquer_recursive)(Q, R, W, A, B, lenB, invB, ctx);
...
void
_TEMPLATE(T, poly_divrem_divconquer)(TEMPLATE(T, struct) * Q, TEMPLATE(T, struct) * R,
                           const TEMPLATE(T, struct) * A, slong lenA,
                           const TEMPLATE(T, struct) * B, slong lenB,
                           const TEMPLATE(T, t) invB, const TEMPLATE(T, ctx_t) ctx)
{
    if (lenA <= 2 * lenB - 1)
    {
        __TEMPLATE(T, poly_divrem_divconquer)(Q, R, A, lenA, B, lenB, invB, ctx);
    }
    else                        /* lenA > 2 * lenB - 1 */
    {
        ...
        if (lenA >= lenB)
        {
            __TEMPLATE(T, poly_divrem_divconquer)(Q, W, R, lenA, B, lenB, invB, ctx);
