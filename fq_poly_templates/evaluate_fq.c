...
void
TEMPLATE(T, TEMPLATE(poly_evaluate, T))(TEMPLATE(T, t) rop, const TEMPLATE(T, poly_t) f, const TEMPLATE(T, t) a,
                    const TEMPLATE(T, ctx_t) ctx)
{
    if (rop == a)
    {
        TEMPLATE(T, t) t;
        TEMPLATE(T, init)(t, ctx);
        _TEMPLATE(T, TEMPLATE(poly_evaluate, T))(t, f->coeffs, f->length, a, ctx);
        TEMPLATE(T, swap)(rop, t, ctx);
        TEMPLATE(T, clear)(t, ctx);
    }
    else
    {
        _TEMPLATE(T, TEMPLATE(poly_evaluate, T))(rop, f->coeffs, f->length, a, ctx);
    }
}
