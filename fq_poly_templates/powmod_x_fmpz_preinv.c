                                       ...
                                       const TEMPLATE(T, ctx_t) ctx)
{
    ...
    if (lenf == 0)
    {
        flint_printf("Exception: fq_poly_powmod_x_preinv: divide by zero\n");
        abort();
    }

    if (fmpz_sgn(e) < 0)
    {
        flint_printf("Exception: fq_poly_powmod_x_preinv: "
    ...
    if ((res == f) || (res == finv))
    {
        ...
        _TEMPLATE(T, poly_powmod_x_fmpz_preinv)(tmp->coeffs, e, f->coeffs, lenf,
    ...
    else
    {
        ...
        _TEMPLATE(T, poly_powmod_x_fmpz_preinv)(res->coeffs, e, f->coeffs, lenf,
                                                finv->coeffs, finv->length, ctx);
    }
