...
void fmpq_mpoly_pushback_term_fmpq_ui(fmpq_mpoly_t poly,
                 const fmpq_t c, const ulong * exp, const fmpq_mpoly_ctx_t ctx)
{
    ...
    exp_bits = mpoly_exp_bits_required_ui(exp, ctx->zctx->minfo);
    ...
    mpoly_set_monomial_ui(zpoly->exps + N*zpoly->length, exp, zpoly->bits,
                                                             ctx->zctx->minfo);
