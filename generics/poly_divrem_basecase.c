...
void _elem_poly_divrem_basecase(elem_ptr Q, elem_ptr R, elem_srcptr A,
    long lenA, elem_srcptr B, long lenB, const ring_t ring)
{
    ...
    if (ring->type == TYPE_MOD && RING_PARENT(ring)->type == TYPE_LIMB)
    {
        if (R == A)
        {
            mp_ptr R2 = flint_malloc(sizeof(mp_limb_t) * (lenB - 1));
            _nmod_poly_divrem(Q, R2, A, lenA, B, lenB, ring->nmod);
            _nmod_vec_swap(R, R2, lenB - 1);
            flint_free(R2);
        }
        else
...
void
elem_poly_divrem_basecase(elem_poly_t Q, elem_poly_t R,
                      const elem_poly_t A, const elem_poly_t B, const ring_t ring)
{
    ...
    if (lenB == 0)
    {
        printf("Exception: division by zero in elem_poly_divrem_basecase\n");
        abort();
    }
    ...
    if (Q == A || Q == B)
    {
        elem_poly_t tmp;
        elem_init(tmp, ring);
        elem_poly_divrem_basecase(tmp, R, A, B, ring);
    ...
    if (R == B)
    {
        elem_poly_t tmp;
        elem_init(tmp, ring);
        elem_poly_divrem_basecase(Q, tmp, A, B, ring);
    ...
    _elem_poly_divrem_basecase(Q->coeffs, R->coeffs, A->coeffs, lenA, B->coeffs, lenB, ring->parent);
