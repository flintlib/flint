...
static void
get_fmpz_mat(fmpz_mat_t N, const arb_mat_t M, slong prec)
{
    slong j, k;
    ...
    for (j = 0; j < arb_mat_nrows(M); j++)
    {
        for (k = 0; k < arb_mat_ncols(M); k++)
        {
            arf_get_fmpz_fixed_si(fmpz_mat_entry(N, j, k),
                                  arb_midref(arb_mat_entry(M, j, k)), -prec);
        }
    }
}

static void
fmpz_mat_reduce(fmpz_mat_t N, fmpz_mat_t U)
{
    fmpz_lll_t fl;
...
void
arb_mat_reduce(fmpz_mat_t U, const arb_mat_t M, slong prec)
{
    fmpz_mat_t N;
