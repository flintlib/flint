...
void acb_theta_newton_eval(acb_ptr r, acb_scrptr th, const acb_theta_agm_ctx_t ctx, slong prec)
{
  ...
  for (k = 0; k < n; k++)
    {      
      acb_theta_transform_sqr_proj(transf, dupl, acb_theta_agm_ctx_matrix(ctx, k), prec);
      for (j = 1; j < n; j++) acb_div(&transf[j], &transf[j], &transf[0], prec);
      acb_one(&transf[0]);
      acb_theta_agm(&agm[k], transf, acb_theta_agm_ctx_roots(ctx, k),
		    acb_theta_agm_ctx_nb_bad_steps(ctx, k),
		    acb_theta_agm_ctx_nb_bad_steps(ctx, k) + acb_theta_agm_nb_good_steps(g, k),
		    g, prec);
      /* Add error coming from agm_good_steps */
      flint_abort();
    }
  for (k = 0; k < n-1; k++)
    {
