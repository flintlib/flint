...
static void fmpz_mat_siegel_g1(fmpz_mat_t u, slong j)
{
  switch(j)
    {
    case 0:
      ...
      break;
    default:
      flint_printf("fmpz_mat_siegel_fd: Error (invalid index: %d)\n", j);
      flint_abort();
    }
}

static void fmpz_mat_siegel_g2(fmpz_mat_t u, slong j)
{
  slong g = 2;
  fmpz_mat_t a, b, c, d;
...
void fmpz_mat_siegel_fd(fmpz_mat_t u, slong j)
{
  slong g = fmpz_mat_nrows(u)/2;
  switch(g)
    {
    case 1:
      fmpz_mat_siegel_g1(u, j);
      break;
    case 2:
      fmpz_mat_siegel_g2(u, j);
      break;
    default:
      flint_printf("fmpz_mat_siegel_fd: Error (not implemented for g = %d)\n", g);
      flint_abort();
    }
}
