    ...
    for (k = 0; k < n - 1; k++)
    {
        ...
        else
        {
            acb_theta_transform_scal_const(&im[k], acb_theta_agm_ctx_tau(ctx),
                                           acb_theta_agm_ctx_mat(ctx, k + 1),
                                           acb_theta_agm_ctx_k2(ctx, k + 1),
                                           prec);
            acb_mul(&im[k], &im[k], zeta, prec);
        }
    }
    for (k = 0; k < dim; k++)
        acb_inv(&im[k], &im[k], prec);
                       ...
                       const acb_theta_agm_ctx_t ctx, slong prec)
{
    ...
    for (k = 0; k < n; k++)
    {
        if (mag_cmp_2exp_si(arb_radref(acb_realref(&start[k])), -prec - 1) > 0
            || mag_cmp_2exp_si(arb_radref(acb_imagref(&start[k])),
                               -prec - 1) > 0)
        {
            flint_printf
                ("acb_theta_newton_start: Error (insufficient precision)\n");
            fflush(stdout);
            flint_abort();
        }
                      ...
                      const acb_theta_agm_ctx_t ctx, slong prec)
{
    ...
    for (k = 0; k < dim; k++)
    {
        if (mag_cmp_2exp_si(arb_radref(acb_realref(acb_mat_entry(h, k, 0))),
                            -nextprec - 1) > 0
            || mag_cmp_2exp_si(arb_radref(acb_imagref(acb_mat_entry(h, k, 0))),
                               -nextprec - 1) > 0)
        {
            flint_printf
                ("acb_theta_newton_step: Error (imprecise correction)\n");
    ...
    for (k = 0; k < n; k++)
    {
        ...
        if (acb_theta_agm_ctx_is_ext(ctx))
        {
            if (k > 0)
            {
                acb_add(&next[k + n], &next[k + n],
                        acb_mat_entry(h, k + n - 2, 0),
                        nextprec + log_th + ACB_THETA_AGM_GUARD);
            }
