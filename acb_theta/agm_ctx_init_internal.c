        ...
        slong g)
{
    ...
    ctx->c = flint_malloc(nb * sizeof(arf_struct));
    ctx->c_ext = flint_malloc(nb * sizeof(arf_struct));
    ctx->e = flint_malloc(nb * sizeof(arf_struct));
    ...
    for (k = 0; k < nb; k++)
    {
        ...
        arf_init(acb_theta_agm_ctx_minf(ctx, k));
        arf_init(acb_theta_agm_ctx_c(ctx, k));
        arf_init(acb_theta_agm_ctx_c_ext(ctx, k));
        arf_init(acb_theta_agm_ctx_e(ctx, k));
        
        arf_init(acb_theta_agm_ctx_rad(ctx, k));
        arf_init(acb_theta_agm_ctx_min(ctx, k));
        arf_init(acb_theta_agm_ctx_max(ctx, k));
    }
