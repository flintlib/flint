  ...
  for (iter = 0; iter < 20 * arb_test_multiplier(); iter++)
      ...
      for (k = 0; k < n; k++)
	{
      ...
      */
      ...
      for (k = 0; k < n; k++) acb_sqr(&th_sqr[k], &th[k], prec);
      arb_one(err);
      arb_mul_2exp_si(err, err, -lowprec);
      for (k = 0; k < n; k++) acb_add_error_arb(&th[k], err);
      acb_mat_scalar_mul_2exp_si(tau, tau, 1);
      acb_theta_naive_const(th_dupl, tau, prec);
      for (k = 0; k < n; k++) acb_sqr(&th_dupl[k], &th_dupl[k], prec);
      ...
      for (k = 0; k < n; k++)
	{
      ...
      if (pos && iter%2 == 0) acb_theta_agm_step_good(test, th_sqr, g, prec);
      else acb_theta_agm_step_bad(test, th_sqr, th, g, prec);
