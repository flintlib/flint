...
{
    acb_theta_agm_ctx_k2(ctx, k) = acb_theta_k2(acb_theta_agm_ctx_mat(ctx, k));
...
agm_ctx_rescale_roots(acb_theta_agm_ctx_t ctx, slong k, slong prec)
{
    ...
    else if (nb_bad > 0)
    {
        acb_inv(scal, &acb_theta_agm_ctx_roots(ctx, k)[0], prec);
        _acb_vec_scalar_mul(acb_theta_agm_ctx_roots(ctx, k),
                            acb_theta_agm_ctx_roots(ctx, k), n * nb_bad, scal,
                            prec);
    }
                   ...
                   const acb_theta_agm_ctx_t ctx, slong k, slong prec)
{
    ...
    if (is_ext)
        agm_ctx_deform_good_ext(rad, min, max, ctx, k, lowprec);
    else
        agm_ctx_deform_good(rad, min, max, ctx, k, lowprec);
...
static void
agm_ctx_get_rho_M(arf_t rho, arf_t M, const acb_theta_agm_ctx_t ctx,
                  slong prec)
{
    slong g = acb_theta_agm_ctx_g(ctx);
    ...
    if (is_ext)
        acb_theta_dupl_all(dupl, acb_theta_agm_ctx_th(ctx), g, prec);
    else
        acb_theta_dupl_all_const(dupl, acb_theta_agm_ctx_th(ctx), g, prec);

    agm_ctx_get_bounds(rho, m0, max, ctx, 0, prec);
    arf_zero(M);
    acb_abs(abs0, &dupl[0], prec);
    if (is_ext)
        acb_abs(abs1, &dupl[1 << (2 * g)], prec);
    ...
    for (k = 1; k < nb; k++)
    {
        ...
        if (is_ext)
        {
            acb_abs(abs, &dupl[acb_theta_agm_ctx_ab(ctx, k) + (1 << (2 * g))],
                    prec);
            arb_mul_arf(abs, abs, max, prec);
            arb_div_arf(abs, abs, min, prec);
...
{
    if (fmpz_cmp_si(e, WORD_MAX) > 0 || fmpz_cmp_si(e, WORD_MIN) < 0)
    {
