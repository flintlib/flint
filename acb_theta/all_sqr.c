...
static void
theta2_newton_dupl_z(acb_ptr th2, acb_srcptr z, const acb_mat_t tau, slong prec)
{
    slong g = acb_mat_nrows(tau);
    slong n = 1<<(2*g+1);
    acb_ptr zmod;
    ...
    for (k = 0; k < g; k++)
    {
	acb_printd(&zmod[k], 10); flint_printf("\n");
    }
    ...
    {
	if (acb_contains_zero(&roots[k])) r = 0;
    }
...
theta2_newton(acb_ptr th2, acb_srcptr z, const acb_mat_t tau, slong prec)
{
    ...
    for (k = 0; k < g; k++)
    {
	acb_printd(&new_z[k], 10); flint_printf("\n");
    }
